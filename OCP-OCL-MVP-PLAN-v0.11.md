# OCL v0.11 — Debugger + Trace Viewer + Trace Diff + Minimizer (Replay-native DX)
...
- `ocl trace view`, `ocl trace diff`, `ocl dbg`, `ocl minimize`.
...
**KPI-3: Minimizer**
- `ocl minimize <artifact_dir>` tạo repro nhỏ hơn:
...
- Mọi thao tác debug/diff/minimize không phá determinism:
  - replay luôn dùng chính cơ chế `ocl replay`
...
- Canonical source-of-truth cho viewer/diff/debugger:
  - `./.ocl_artifacts/<run_id>/audit.jsonl` (JSONL).
...
- `ocl trace view <artifact_dir>` (default path):
...
- `ocl dbg <artifact_dir>` interactive:
  - step forward/back
  - breakpoints by:
    - event type
    - key name
...
- Full IDE plugin.
- Source-level stepping without audit (OCL is audit-driven).
...
- `ocl trace view <audit.jsonl>` phải fail-honest với `X-TRACE-FORMAT-UNSUPPORTED` nếu thiếu marker này.
...
### 5.1 Command: `ocl trace view`
Usage:
- `ocl trace view <artifact_dir>`
- `ocl trace view <audit.jsonl>`
- `ocl trace view <legacy_trace.pipe> --legacy-pipe`
...
### 7.1 Command: `ocl trace diff`
- `ocl trace diff <artifactA> <artifactB> [--mode strict|align]`
...
### 8.1 Command: `ocl minimize`
- `ocl minimize <artifact_dir> --goal error_code:X-...`
- `ocl minimize <artifact_dir> --goal divergence --against <artifactB>`
- `ocl minimize <artifact_dir> --goal kind:INSUFFICIENT --key std.fs.read_text`
...
- Deterministic delta-debugging:
  ...
  - iteratively test subsets using `ocl replay` (locked) or `ocl replay` with cassette (quarantine)
- Stopping condition:
...
Within `./.ocl_artifacts/<run_id>/` add:
- `audit.jsonl`:
...
- Status: `DONE` (2026-03-05)
Scope:
- `ocl trace view` with filters, summary, source snippet (if available)
...
Scope:
- `ocl dbg` REPL commands (step/back/jump/break/locals/print)
Tests:
...
Scope:
- `ocl trace diff` strict + align mode
- JSON report output
...
- Scope:
  ...
  - Chuyển `ocl trace view` mặc định sang đọc artifact/audit thay vì trace pipe.
- Expected tests:
  - `cargo test --test trace_index --offline`
  - `cargo test -p ocl-cli w5_cli_trace_and_profile_non_reactor_pass --offline`
...
- Test results:
  ...
  - Regression tests (supporting only):
    ...
    - `ocl-cli` full package: 31 bài xanh, 2 bài chưa xanh do điều kiện fixture/policy lock ngoài phạm vi gate (`v5_w5_cli_view_wiring_run_pass`, `w4_cli_build_publish_fetch_verify_and_run_artifact_pass`).
  - Kết luận gate:
...
- Gate/Step:
  - 11-B
- Why:
  - Hoàn tất bề mặt `ocl trace view` theo spec v0.11 để đọc audit v2, lọc sự kiện, và hiển thị source snippet phục vụ debug-first.
- Scope:
...
- Expected tests:
  ...
  - `cargo test -p ocl-cli w5_cli_trace_and_profile_non_reactor_pass --offline`
- Exit criteria:
  - `ocl trace view` hoạt động đúng trên artifact/audit v2 với filter CLI mới.
...
- Expected tests:
  ...
  - `cargo test --test trace_index --offline`
  - `cargo test --test cli_trace_view --offline`
  - `cargo test -p ocl-cli v7_g_cli_replay_signature_match_pass --offline`
- Exit criteria:
...
- Implemented:
  ...
  - Ghi checkpoint artifact:
    - `./.ocl_artifacts/<run_id>/checkpoints/replay.checkpoints.json`.
...
- Scope:
  - Triển khai `ocl dbg <artifact_dir> [--script <file>]` trong `ocl-cli`.
...
- Expected tests:
  ...
  - `cargo fmt -- --check`
- Exit criteria:
  - `ocl dbg` chạy được command set lõi 11-D bằng script mode, output có evidence `where/locals/diffenv/last`.
...
- Scope:
  - Bổ sung subcommand `ocl trace diff <artifactA|auditA> <artifactB|auditB>`.
  - Hỗ trợ mode:
...
- Scope:
  - Bổ sung CLI command `ocl minimize`.
...
- Expected tests:
  ...
  - `cargo fmt -- --check`
- Exit criteria:
  - `ocl minimize` tạo được artifact minimized và report.
