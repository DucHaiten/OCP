...
|---|---|---|---|---|
| Value model (`null/bool/int/string/list/record/map`) | `S` | `additive` | `tests/ocl_value.rs` | `7.1-B` |
| JSON contract (`std.json.parse/stringify`) | `S` | `additive` | `tests/std_json.rs` | `7.1-C` |
| `try/else` sugar | `S` | `additive` | `tests/ocl_sugar.rs` | `7.1-F` |
| `guard` sugar + `guard_mode` | `S` | `additive` | `tests/ocl_sugar.rs`, `tests/ocl_manifest.rs` | `7.1-F`, `7.1-G` |
| Bounded loops (`repeat`, `for ... cap`) | `S` | `additive` | `tests/ocl_loops.rs` | `7.1-E` |
| Module/import loader | `S` | `additive` | `tests/ocl_modules.rs` | `7.1-D` |
| Manifest/permissions + deny/allow precedence | `S` | `additive` | `tests/ocl_manifest.rs` | `7.1-G` |
| Replay-first artifacts + signature rules | `S` | `additive` | `tests/ocl_determinism.rs`, `tests/cli_e2e.rs` | `7.1-A`, `7.1-G` |
...
- `ocl init hello-tool` tạo dự án mới **≤ 4 file** (không tính thư mục artifacts):
  - `ocl.toml`
  - `main.ocl`
  - `README.md`
...
**KPI-2: Replay-first**
- `ocl run` luôn tạo `./.ocl_artifacts/<run_id>/` gồm:
  - `audit.jsonl`
  - `signature.txt`
  - `replay.toml`
- `ocl replay <artifact_dir>` tái tạo `signature` **giống hệt** (lane locked).
...
**KPI-1: File sprawl control**
- Command:
  - `ocl init hello-tool ./tmp/hello-tool`
  - kiểm tra file bằng script lane (đếm file trong root project, không tính `.ocl_artifacts/`)
- Artifacts:
  - `tmp/hello-tool/ocl.toml`
  - `tmp/hello-tool/main.ocl`
...
**KPI-2: Replay-first**
- Command:
  - `ocl run ./tmp/hello-tool --seed 42`
...
- Pass:
  - `ocl replay` exit code = 0 và in `signature match`.
- Fail:
  - signature mismatch hoặc thiếu bất kỳ artifact bắt buộc nào.

**KPI-3: LOC reduction enabler**
- Command:
  - script lane đếm LOC của `main.ocl` (không tính comments/blank lines theo rule tool đã khóa trong lane)
- Artifacts:
...
- `ocl.toml` bắt buộc trong lane locked.
...
- `ocl trace view <audit.jsonl>` (text)
...
- Widget UI framework (OCL chỉ nên xuất draw-list)

---

## 4) Deliverables v0.7.1

1) `docs/OCL-v0.7.1.md` (tài liệu này).  
2) Workspace crates (Rust):
- `ocl-runtime-core`: span/diag/lex/parse/ast/types/typecheck/value/result4/budget/exec/runtime/audit/determinism
- `ocl-sdk`: manifest/permissions/resolver wiring
- `ocl-cli`: command surface `init/run/test/replay/trace`  
3) Pure stdlib:
...
|---|---|---|
| span/diag/lex/ast/parse/types/typecheck | `ocl-runtime-core` | `projects/ocp-ocl/crates/ocl-runtime-core/src/*` |
| value/result4/budget/exec/audit/determinism | `ocl-runtime-core` | `projects/ocp-ocl/crates/ocl-runtime-core/src/*` |
| runtime observe/commit boundary | `ocl-runtime-core` | `projects/ocp-ocl/crates/ocl-runtime-core/src/*` |
| stdlib pure execution (`std.json`, `std.core`) | `ocl-runtime-core` | `projects/ocp-ocl/crates/ocl-runtime-core/src/*` |
| manifest parser + permissions + resolver wiring | `ocl-sdk` | `projects/ocp-ocl/crates/ocl-sdk/src/*` |
| CLI command UX (`init/run/test/replay/trace`) | `ocl-cli` | `projects/ocp-ocl/crates/ocl-cli/src/main.rs` |
| public SDK exports + integration glue | `ocl-sdk` | `projects/ocp-ocl/crates/ocl-sdk/src/lib.rs` |
...
- Registry/permissions vẫn tồn tại để enforce “default deny” và test DX; resolver/path policy nằm ở `ocl-sdk`.
...
- Nếu INSUFFICIENT/DEFERRED: early exit theo config từ `ocl.toml`:
...
### 7.1 `ocl.toml` schema (v0.7.1 minimal)
...
- Lane `locked_v071` và `locked_v06` đều bắt buộc có `ocl.toml`.
...
- `ocl init hello-tool <dir>`
  - tạo `ocl.toml`, `main.ocl`, `README.md`
  - tạo `fixtures/` nếu demo cần input JSON mẫu

- `ocl run [--entry main.ocl] [--seed N]`
  - chạy theo lane trong `ocl.toml` (default `locked_v071`), emit artifacts như mục 8
...
- Nguồn chuẩn command set hiện có lấy từ `ocl --help` khi chạy ở lane `locked_v06`; mọi command/flag xuất hiện ở đây được coi là baseline compatibility.
...
- Giữ layout `.ocl_artifacts/<run_id>/...` hiện hữu.
...
Command:
- `cargo test --test ocl_diag --test ocl_audit`
...
Command:
- `cargo test --test ocl_value`

Artifacts:
- test report `ocl_value`
...
Command:
- `cargo test --test ocl_modules`

Artifacts:
- test report `ocl_modules`
...
Command:
- `cargo test --test ocl_loops`

Artifacts:
- test report `ocl_loops`
...
Scope:
- ocl.toml parse (`lane`, `limits`, `[language].guard_mode`)
- deny/allow precedence
- permission error hints
- CLI commands + hello-tool template + replay compat

Command:
- `cargo test --test ocl_manifest --test cli_e2e`

Artifacts:
- `.ocl_artifacts/<run_id>/*` bộ file bắt buộc
...
- Scope:
  ...
  - Bổ sung test suite `ocl_audit` để khóa hợp đồng audit/signature ở mức core.
  - Bổ sung artifact writer cho `ocl run` (non-reactor project path) luôn ghi:
    - `.ocl_artifacts/<run_id>/audit.jsonl`
    - `.ocl_artifacts/<run_id>/signature.txt`
    - `.ocl_artifacts/<run_id>/replay.toml`
    kể cả khi run fail.
...
- Expected tests:
  - `cargo test --test ocl_diag --test ocl_audit`
  - `cargo test -p ocl-cli v7_a_cli_run_emits_artifacts_on_success`
  - `cargo test -p ocl-cli v7_a_cli_run_emits_artifacts_on_fail`
  - `cargo test`
- Exit criteria:
  - Parse error thực tế có đủ `code + span + hint`.
  - `ocl run` (non-reactor project path) ghi đủ 3 artifact kể cả khi fail.
...
- Implemented:
  ...
  - Bổ sung suite `ocl_audit` để kiểm chứng trace/signature ổn định ở lớp core.
  - Nâng `ocl-cli` (non-reactor project run path) để luôn ghi đủ replay-first artifacts:
    - `.ocl_artifacts/<run_id>/audit.jsonl`
    - `.ocl_artifacts/<run_id>/signature.txt`
    - `.ocl_artifacts/<run_id>/replay.toml`
...
- Scope:
  ...
  - Bổ sung test suite `tests/ocl_value.rs` cho pass/fail paths của Gate B.
- Expected tests:
  - `cargo test --test ocl_value`
  - `cargo test`
- Exit criteria:
...
- Implemented:
  ...
  - Bổ sung test suite mới `tests/ocl_value.rs`.
- Files changed:
  - `src/ocp_ocl/diag.rs`
...
- Test results:
  - PASS:
    - `ocl_value`: 6/6
    - Full `cargo test`: pass toàn bộ suites hiện có (bao gồm suite mới `ocl_value`).
- Notes/risks:
...
- Scope:
  ...
  - Bổ sung diagnostics:
    ...
    - `T-IMPORT-CYCLE`
    với hint actionable.
  - Bổ sung test suite mới `tests/ocl_modules.rs`.
- Expected tests:
  - `cargo test --test ocl_modules`
  - `cargo test`
- Exit criteria:
...
- Implemented:
  ...
  - Bổ sung test suite `tests/ocl_modules.rs`:
...
- Scope:
  ...
  - Bổ sung test suite `tests/ocl_loops.rs` và cập nhật parser/diag tests liên quan.
- Expected tests:
  - `cargo test --test ocl_loops`
  - `cargo test --test ocl_parser --test ocl_exec --test ocl_diag`
  - `cargo test`
- Exit criteria:
...
- Implemented:
  ...
  - Diagnostics/tests:
    - thêm error codes mới:
      ...
      - `T-CAP-NOT-INT-LIT`
      - `X-LOOP-CAP-EXCEEDED`
    - thêm suite mới `tests/ocl_loops.rs`.
    - cập nhật `tests/ocl_parser.rs` và `tests/ocl_diag.rs`.
- Files changed:
  - `src/ocp_ocl/ast.rs`
  - `src/ocp_ocl/lex.rs`
  - `src/ocp_ocl/parse.rs`
  ...
  - `OCP-OCL-MVP-PLAN-v0.7.1.md`
- Commands run:
  - `rustfmt projects/ocp-ocl/crates/ocl-cli/src/main.rs src/ocp_ocl/exec.rs src/ocp_ocl/parse.rs src/ocp_ocl/typecheck.rs src/ocp_ocl/runner.rs tests/ocl_audit.rs tests/ocl_loops.rs`
  - `cargo fmt -- --check`
  - `cargo test --test ocl_loops`
  - `cargo test --test ocl_parser --test ocl_exec --test ocl_diag`
  - `cargo test`
...
- Scope:
  ...
  - Tests:
    - thêm suite `tests/ocl_sugar.rs`.
    - cập nhật parser/diag tests liên quan.
- Expected tests:
  - `cargo test --test ocl_sugar`
  - `cargo test --test ocl_parser --test ocl_typecheck --test ocl_exec --test ocl_diag --test ocl_sugar`
  - `cargo test`
- Exit criteria:
...
- Implemented:
  ...
  - Tests:
    - thêm `tests/ocl_sugar.rs` (invariance + type/runtime contracts).
    - cập nhật `tests/ocl_parser.rs`, `tests/ocl_diag.rs`.
    - vá `tests/ocl_confidence.rs` để tương thích `ExecConfig` có thêm `guard_mode`.
- Files changed:
  - `src/ocp_ocl/lex.rs`
  - `src/ocp_ocl/ast.rs`
  - `src/ocp_ocl/parse.rs`
...
- Test results:
  - PASS:
    - `ocl_sugar`: 6/6 pass.
    - targeted group (`ocl_parser`, `ocl_typecheck`, `ocl_exec`, `ocl_diag`, `ocl_sugar`) pass toàn bộ.
    - full `cargo test`: pass toàn bộ suites hiện có.
  - FAIL tạm thời đã xử lý:
    - full `cargo test` lần đầu fail compile ở `tests/ocl_confidence.rs` vì thiếu field `guard_mode` trong `ExecConfig`.
...
- Scope:
  ...
  - Regression bridge:
    - vá compile drift do AST/ExecConfig mới giữa `ocp-ocl` và `ocl-runtime-core`/`ocl-sdk`.
  - Tests:
    - thêm `tests/ocl_manifest.rs`.
    - thêm `tests/cli_e2e.rs`.
    - thêm test replay trong `ocl-cli/src/main.rs`.
- Expected tests:
  - `cargo test --test ocl_manifest --test cli_e2e`
  - `cargo test -p ocl-cli v7_g_cli_replay_`
...
- Gate/Step:
  - 7.1-G
- Implemented:
  - `ocl-sdk` manifest/runtime wiring:
...
- Test results:
  - PASS:
    - `ocl_manifest`: 3/3
    - `cli_e2e`: 2/2
    - `ocl-cli` replay tests: 2/2
    - full `cargo test -p ocl-cli`: 33/33
    - full `cargo test`: pass toàn bộ suites hiện có.
  - FAIL tạm thời đã xử lý:
    - compile fail do `ExecConfig` thêm `guard_mode` ở `ocl-runtime-core`/`ocl-sdk/w3`.
    - compile fail do AST/trace enum mở rộng nhưng runtime-core/sdk mapper chưa cover.
    - đã vá xong và rerun pass toàn bộ.
- Notes/risks:
  - Gate G đã đóng với replay command mức project artifact-dir; replay cho artifact package (`.oclpkg`) chưa mở trong scope này.
