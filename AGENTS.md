...
- Lệnh trên tự:
  - bỏ stage các build artifacts thường gặp (`target`, `.../ocp-cli/target`, `Rules/guard/__pycache__`),
  - chạy guard staged,
...
## 19) HAR Sandbox Lock Mode (khóa cứng phạm vi HAR)
- Mặc định bật `HAR_SANDBOX_LOCK_MODE = ON`.
- Khi mode này bật:
  - Chỉ được phép tạo/sửa/xóa file trong `HAR/`.
  - Mọi thay đổi ngoài `HAR/` được xem là vi phạm scope và phải dừng ngay.
  - Không được viện lý do “tiện tay” để sửa file ngoài `HAR/`.
- Lệnh shell/commit phải đi kèm guard:
  - `python HAR/scripts/har_path_guard.py --mode working`
  - `python HAR/scripts/har_path_guard.py --mode staged`
- Cấm chốt `DONE` nếu guard chưa pass.
- Nếu guard báo có file ngoài `HAR/`:
  - Dừng triển khai ngay,
  - Báo danh sách file vi phạm,
  - Chỉ tiếp tục khi người dùng xác nhận rõ ràng mở khóa.

### 19.1 Mở khóa tạm thời (exception hiếm)
- Chỉ được mở khóa khi người dùng ghi rõ trong yêu cầu:
  - `UNLOCK_HAR_SANDBOX`
- Cơ chế kỹ thuật cho hook (chỉ dùng tạm thời):
  - `HAR_SANDBOX_UNLOCK=1`
- Khi không có marker trên, mặc định giữ khóa.
- Sau khi hoàn tất exception, phải quay về `HAR_SANDBOX_LOCK_MODE = ON` ngay trong cùng phiên.

### 19.2 Cơ chế ép lỗi tự động (enforcement)
- Bắt buộc dùng script:
  - `HAR/scripts/har_path_guard.py`
  - `HAR/scripts/install_har_lock.ps1`
- Git hooks `pre-commit` và `pre-push` phải gọi guard để fail cứng nếu có file ngoài `HAR/`.
