...
Mục tiêu v0.14: đưa OCL lên mức “đủ tin cậy để người khác dùng” bằng:
...
- Conformance mô tả **behavior-level**:
  ...
  - lock/deps integrity.
- Người khác có thể lấy OCL runtime và tin rằng:
  - template chạy đúng,
...
- Runner:
  - `ocl test --conformance run [--suite ...]` (canonical)
  - `ocl test --conformance list` (canonical)
- Additive CLI aliases (compat):
  - `ocl conformance run` = alias của `ocl test --conformance run`
  - `ocl conformance list` = alias của `ocl test --conformance list`
- Profiles:
  - `core`
  - `packs-tool`
  - `packs-consumer`
...
- `ocl upgrade-check`:
  ...
  - lock:
    ...
    - Gates 14-A..14-E không được phụ thuộc vào `ocl upgrade-check`
...
- w9 scenario parser + `expected_signature_file` extension:
  - crate: `ocl-sdk`
  - path: `projects/ocp-ocl/crates/ocl-sdk/src/w9.rs`
- Canonical CLI `ocl test --conformance ...` + aliases `ocl conformance ...`:
  - crate: `ocl-cli`
  - path: `projects/ocp-ocl/crates/ocl-cli/src/main.rs`
- Signature reading/comparison + sandbox runner wiring for conformance:
  - crate: `ocl-sdk` (SoT cho runner behavior), CLI chỉ gọi vào SDK
  - path: `projects/ocp-ocl/crates/ocl-sdk/src/w9.rs`
- `ocl upgrade-check` command:
  - crate: `ocl-cli`
  - path: `projects/ocp-ocl/crates/ocl-cli/src/main.rs`
...
- `project/` (mini project with ocl.toml + sources)
- `expected/`
...
- For `quarantine`:
  ...
  - semantics của `replay_artifact`:
    1. copy `expected/cassette/*` -> `./.ocl_artifacts/<run_id>/cassette/*`
    2. verify `cassette_hash` khớp `replay.toml` nếu `replay.toml` có trường hash
    3. chạy `ocl replay <artifact_dir>` và so signature equality
...
- executes subset via canonical runner path `ocl test --conformance ...`
...
- canonical CLI path: `ocl test --conformance run|list`
- additive aliases: `ocl conformance run|list`
...
- shadow cases
- quarantine replay cases with golden cassettes (inject cassette -> `ocl replay`, no record in CI)
Tests:
...
Scope:
- implement `ocl upgrade-check`
- integrate trace diff for mismatch reporting
- use canonical conformance runner path (`ocl test --conformance ...`) under the hood
Tests:
- `tests/upgrade_check.rs`
Exit criteria:
...
- `cargo fmt -- --check`
- `ocl test --conformance run --suite all` (canonical in CI pipeline)
- `ocl conformance run --suite all` (alias, optional)
...
- Expected tests:
  - `cargo test --test conformance_runner`
  - `cargo test -p ocl-cli w14_cli_conformance_list_alias_json_pass -- --nocapture`
  - `cargo test -p ocl-cli w14_cli_test_conformance_list_mode_pass -- --nocapture`
  - `cargo test -p ocl-cli w14_cli_test_conformance_run_subcommand_pass -- --nocapture`
  - `cargo test -p ocl-cli w9_cli_conformance_single_scenario_pass -- --nocapture`
- Exit criteria:
...
- Scope:
  - Thêm 4 core scenarios trong `conformance.v5`:
    ...
    - `core-parse-fail`,
    - `core-exec-fail`.
  - Tạo fixture projects tương ứng trong `projects/ocp-ocl/conformance/fixtures/core-*`.
...
- Expected tests:
  - `cargo test --test conformance_core`
  - `cargo run -p ocl-cli -- test --conformance run --manifest projects/ocp-ocl/conformance/conformance.v5.toml --json`
- Exit criteria:
...
- Implemented:
  ...
  - Gắn expected signature cho `core-exec-pass`:
    - `projects/ocp-ocl/conformance/fixtures/expected/core-exec-pass.signature.txt`
  - Thêm targeted test `tests/conformance_core.rs` để khóa contract và chạy subset core trực tiếp.
- Files changed:
  - `projects/ocp-ocl/conformance/conformance.v5.toml`
  - `projects/ocp-ocl/conformance/fixtures/core-exec-pass/Ocl.toml`
  - `projects/ocp-ocl/conformance/fixtures/core-exec-pass/src/main.ocl`
  - `projects/ocp-ocl/conformance/fixtures/core-type-fail/Ocl.toml`
  - `projects/ocp-ocl/conformance/fixtures/core-type-fail/src/main.ocl`
  - `projects/ocp-ocl/conformance/fixtures/core-parse-fail/Ocl.toml`
  - `projects/ocp-ocl/conformance/fixtures/core-parse-fail/src/main.ocl`
  - `projects/ocp-ocl/conformance/fixtures/core-exec-fail/Ocl.toml`
  - `projects/ocp-ocl/conformance/fixtures/core-exec-fail/src/main.ocl`
  - `projects/ocp-ocl/conformance/fixtures/expected/core-exec-pass.signature.txt`
  - `tests/conformance_core.rs`
- Commands run:
  - `cargo test --test conformance_core`
...
- Implemented:
  ...
  - Nâng cấp runner `replay_artifact`:
    - không chỉ copy cassette, mà chạy replay thật với env replay (`OCL_QUARANTINE_MODE=replay`, `OCL_V08_CASSETTE_*`),
    - tính signature replay và ghi `.ocl_artifacts/conformance.replay/signature.txt`,
...
- Files changed:
  - `projects/ocp-ocl/crates/ocl-sdk/src/w9.rs`
  - `projects/ocp-ocl/conformance/conformance.v5.toml`
  - `projects/ocp-ocl/conformance/fixtures/packs-tool-fs-read/Ocl.toml`
  - `projects/ocp-ocl/conformance/fixtures/packs-tool-fs-read/src/main.ocl`
  - `projects/ocp-ocl/conformance/fixtures/packs-tool-fs-read/data/sample.txt`
  - `projects/ocp-ocl/conformance/fixtures/packs-consumer-ui/Ocl.toml`
  - `projects/ocp-ocl/conformance/fixtures/packs-consumer-ui/src/main.ocl`
  - `projects/ocp-ocl/conformance/fixtures/shadow-search-rr/Ocl.toml`
  - `projects/ocp-ocl/conformance/fixtures/shadow-search-rr/src/main.ocl`
  - `projects/ocp-ocl/conformance/fixtures/quarantine-wallclock-replay/Ocl.toml`
  - `projects/ocp-ocl/conformance/fixtures/quarantine-wallclock-replay/src/main.ocl`
  - `projects/ocp-ocl/conformance/fixtures/quarantine-wallclock-replay/.ocl_artifacts/conformance.replay/replay.toml`
  - `projects/ocp-ocl/conformance/fixtures/quarantine-wallclock-replay/expected/cassette/cassette.jsonl`
  - `projects/ocp-ocl/conformance/fixtures/quarantine-wallclock-replay/expected/cassette/cassette_index.json`
  - `projects/ocp-ocl/conformance/fixtures/quarantine-wallclock-replay/expected/cassette/cassette_meta.toml`
  - `projects/ocp-ocl/conformance/fixtures/expected/quarantine-wallclock-replay.signature.txt`
  - `tests/conformance_packs.rs`
- Commands run:
  - `cargo test --test conformance_packs`
  - `cargo test --test conformance_runner`
...
- Implemented:
  ...
  - Runner conformance thêm assert output supplychain:
    - xuất actual `deps.json` và `permissions_effective.json` vào `.ocl_artifacts/conformance.outputs/`
  ...
  - Tạo fixture supplychain:
    ...
    - dependency `package.oclp` có requested permissions `std_proc`,
    - expected outputs `deps.json` + `permissions_effective.json` + `signature.txt`.
  - Thêm targeted test `tests/conformance_supplychain.rs`.
- Files changed:
  - `projects/ocp-ocl/crates/ocl-sdk/src/w9.rs`
  - `projects/ocp-ocl/conformance/conformance.v5.toml`
  - `projects/ocp-ocl/conformance/fixtures/supplychain-effective-permissions/Ocl.toml`
  - `projects/ocp-ocl/conformance/fixtures/supplychain-effective-permissions/src/main.ocl`
  - `projects/ocp-ocl/conformance/fixtures/supplychain-effective-permissions/deps/toolkit/package.oclp`
  - `projects/ocp-ocl/conformance/fixtures/supplychain-effective-permissions/deps/toolkit/src/worker.ocl`
  - `projects/ocp-ocl/conformance/fixtures/supplychain-effective-permissions/expected/outputs/deps.json`
  - `projects/ocp-ocl/conformance/fixtures/supplychain-effective-permissions/expected/outputs/permissions_effective.json`
  - `projects/ocp-ocl/conformance/fixtures/supplychain-effective-permissions/expected/signature.txt`
  - `tests/conformance_supplychain.rs`
- Commands run:
  - `cargo test --test conformance_supplychain`
  - `cargo test --test conformance_runner`
...
- Scope:
  ...
  - thêm targeted suite `tests/conformance_cache.rs` gồm:
    ...
    - `ocl cache bench --json` xác nhận `signature_equal=true` và xuất artifact telemetry phi-ngữ nghĩa.
- Expected tests:
  - `cargo test --test conformance_cache`
- Exit criteria:
...
- Gate/Step: 14-F
- Why:
  - bổ sung `ocl upgrade-check` để kiểm tra nâng cấp theo contract conformance, trả report pass/fail + first divergence thay vì kiểm tra thủ công.
- Scope:
  - thêm command `ocl upgrade-check <project_dir>`.
  ...
  - chạy subset qua runner canonical `run_conformance_v1` (cùng pipeline với `ocl test --conformance`).
...
- Implemented:
  - thêm command `upgrade-check` vào CLI:
    ...
    - detect project features từ `Ocl.toml` + source `.ocl`.
    - chọn subset scenario và chạy runner canonical `run_conformance_v1`.
    - trả report `ocl.upgrade_check.v1` gồm summary + `first_divergence`.
...
- Commands run:
  ...
  - `cargo test --test conformance_cache`
  - `cargo run -p ocl-cli -- test --conformance run --manifest projects/ocp-ocl/conformance/conformance.v5.toml --json`
  - `cargo run -p ocl-cli -- upgrade-check projects/ocp-ocl/conformance/fixtures/core-exec-pass --manifest projects/ocp-ocl/conformance/conformance.v5.toml --json`
  - `cargo fmt -- --check`
- Test results:
  - Targeted tests (must-pass for gate): `PASS`
  - Regression tests (supporting only): `PASS`
...
- Expected tests:
  ...
  - `cargo test`
  - `cargo run -p ocl-cli -- test --conformance run --manifest projects/ocp-ocl/conformance/conformance.v5.toml --suite all --json`
- Exit criteria:
...
- Files changed:
  ...
  - `OCP-OCL-MVP-PLAN-v0.14.md`
- Commands run:
  - `$env:OCL_UPDATE_V14_CONTRACT_SNAPSHOT='1'; cargo test --test stability_contracts_v14`
  - `cargo test --test stability_contracts_v14`
  - `cargo test`
  - `cargo run -p ocl-cli -- test --conformance run --manifest projects/ocp-ocl/conformance/conformance.v5.toml --suite all --json`
  - `cargo fmt -- --check`
- Test results:
  - Targeted tests (must-pass for gate):
    ...
    - `cargo run -p ocl-cli -- test --conformance run --manifest ... --suite all --json`: `PASS` (15/15 scenarios)
...
- Design alignment:
  - `FULL`
- Notes/risks:
  - Lệnh chạy với `OCL_UPDATE_V14_CONTRACT_SNAPSHOT=1` là bước bảo trì baseline có chủ đích để cập nhật snapshot; bước verify chính là lần chạy lại không bật biến môi trường.
