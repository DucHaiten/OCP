...
$checks = @(
    @{
        ...
        action = {
            & cargo run -p ocl-cli -- run "projects/ocp-ocl/apps/hello-cli" --locked
        }
    },
    @{
        name = "mini_server_reactor_smoke"
        action = {
            & cargo run -p ocl-cli -- run "projects/ocp-ocl/apps/mini-server" --locked --reactor --ticks 8
        }
    },
    @{
        name = "composer_demo_verify_locked"
        action = {
            & cargo run -p ocl-cli -- test "projects/ocp-ocl/apps/composer-demo" --locked
        }
    },
    @{
...
$report = [pscustomobject]@{
    ...
    suite = "ocl_quarantine_non_blocking"
    blocking = $false
