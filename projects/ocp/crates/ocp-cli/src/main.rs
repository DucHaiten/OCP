...
use ocp_sdk::{
    ...
    build_ocppkg_with_lock, build_profile_from_trace, build_project_with_lock,
...
const OCP_PUBLIC_VERSION: &str = "v1.0.0";
const PRIMARY_CLI_COMMAND: &str = "ocp";
    ...
    match args[0].as_str() {
        "init" => {
            let Some(path) = args.get(1) else {
                eprintln!(
                    "usage: ocp init <project_dir> [--template tool-cli|tool-http|tool-proc|tool-wallclock|mini-game|shadow-preview|dep-permission] [--preset workflow_basic|agent_swarm_basic] [--locked] [--registry <index.toml>] [--signer-id <id>] [--sign-key <file>] [--trust-store <file>] [--json]"
                );
                return 2;
            };
        ...
        "check" => {
            let Some(path) = args.get(1) else {
                eprintln!("usage: ocp check <project_dir> [--json] [--locked] [--universe <id>]");
                return 2;
            };
            let json_mode = args.iter().any(|a| a == "--json");
            let locked = args.iter().any(|a| a == "--locked");
        ...
        "run" => {
            let Some(path) = args.get(1) else {
                eprintln!(
                    "usage: ocp run <project_dir> [--engine interpreter|bytecode|dual] [--reactor --ticks N --runtime deterministic|throughput --socket-listen ADDR --runtime-report FILE --replay-audit FILE] [--shadow <id> --shadow-policy forbid_commit|shadow_commit_log] [--locked] [--universe <id>] [--domain <id>] [--view <id>]"
                );
                return 2;
            };
            ...
            let path_is_artifact = path_ref
                .extension()
                .and_then(|v| v.to_str())
                .map(|v| v.eq_ignore_ascii_case("ocppkg"))
                .unwrap_or(false);
        ...
        "replay" => {
            let Some(path) = args.get(1) else {
                eprintln!("usage: ocp replay <artifact_dir>");
                return 2;
            };
            ...
            match subcmd {
                "stats" => {
                    let Some(path) = args.get(2) else {
                        eprintln!("usage: ocp cache stats <project_dir> [--json]");
                        return 2;
                    };
                    let json_mode = args.iter().any(|a| a == "--json");
                ...
                "clean" => {
                    let Some(path) = args.get(2) else {
                        eprintln!("usage: ocp cache clean <project_dir> --yes [--json]");
                        return 2;
                    };
                ...
                "bench" => {
                    let Some(path) = args.get(2) else {
                        eprintln!(
                            "usage: ocp cache bench <project_dir> [--engine interpreter|bytecode|dual] [--locked] [--json]"
                        );
                        return 2;
                    };
                    let locked = args.iter().any(|a| a == "--locked");
        ...
        "dbg" => {
            let Some(path) = args.get(1) else {
                eprintln!("usage: ocp dbg <artifact_dir> [--script <file>]");
                return 2;
            };
        ...
        "minimize" => {
            let Some(path) = args.get(1) else {
                eprintln!(
                    "usage: ocp minimize <artifact_dir> --goal <error_code:X|divergence|kind:KIND> [--key <key>] [--against <artifactB>] [--out <dir>] [--json]"
                );
                return 2;
            };
        ...
        "trace" => {
            ...
            match subcmd {
                "run" => {
                    let Some(path) = args.get(2) else {
                        eprintln!(
                            "usage: ocp trace run <project_dir> [--out <file>] [--engine interpreter|bytecode|dual] [--reactor --ticks N --runtime deterministic|throughput] [--shadow <id> --shadow-policy forbid_commit|shadow_commit_log] [--locked] [--universe <id>] [--domain <id>] [--view <id>]"
                        );
                        return 2;
                    };
                    let locked = args.iter().any(|a| a == "--locked");
                    ...
                    let out_path = parse_string_flag(args, "--out")
                        .map(PathBuf::from)
                        .unwrap_or_else(|| PathBuf::from("target/ocp/w5/trace.latest.jsonl"));
                    let engine_raw = parse_string_flag(args, "--engine")
                        .unwrap_or_else(|| "interpreter".to_string());
                    let run_engine = match engine_raw.as_str() {
                        "interpreter" => RunEngine::Interpreter,
                ...
                "view" => {
                    let Some(trace_path) = args.get(2) else {
                        eprintln!(
                            "usage: ocp trace view <artifact_dir|audit.jsonl|legacy.trace> [--tail N] [--json] [--legacy-pipe] [--type <event>] [--key <key>] [--kind <kind>] [--reason <rc>] [--module <module_id>]"
                        );
                        return 2;
                    };
                ...
                "diff" => {
                    let Some(left_path) = args.get(2) else {
                        eprintln!(
                            "usage: ocp trace diff <artifactA|auditA> <artifactB|auditB> [--mode strict|align] [--json] [--out <dir>]"
                        );
                        return 2;
                    };
                ...
                _ => {
                    eprintln!("usage: ocp trace <run|view|diff> ...");
                    2
                }
            }
        }
        "profile" => {
            ...
            match subcmd {
                "run" => {
                    let Some(path) = args.get(2) else {
                        eprintln!(
                            "usage: ocp profile run <project_dir> [--out <file>] [--engine interpreter|bytecode|dual] [--reactor --ticks N --runtime deterministic|throughput] [--shadow <id> --shadow-policy forbid_commit|shadow_commit_log] [--locked] [--universe <id>] [--domain <id>] [--view <id>]"
                        );
                        return 2;
                    };
                    let locked = args.iter().any(|a| a == "--locked");
                    ...
                    let out_path = parse_string_flag(args, "--out")
                        .map(PathBuf::from)
                        .unwrap_or_else(|| PathBuf::from("target/ocp/w5/profile.latest.json"));
                    let engine_raw = parse_string_flag(args, "--engine")
                        .unwrap_or_else(|| "interpreter".to_string());
                    let run_engine = match engine_raw.as_str() {
                        "interpreter" => RunEngine::Interpreter,
                ...
                "view" => {
                    let Some(profile_path) = args.get(2) else {
                        eprintln!("usage: ocp profile view <profile_file> [--top N] [--json]");
                        return 2;
                    };
                ...
                _ => {
                    eprintln!("usage: ocp profile <run|view> ...");
                    2
                }
            }
        }
        "fmt" => {
            ...
            let Some(path) = path else {
                eprintln!("usage: ocp fmt <project_dir> [--check]");
                return 2;
            };
        ...
        "conformance" => {
            if args.len() < 2 {
                eprintln!(
                    "usage: ocp conformance <run|list> [--manifest <file>] [--out <file>] [--runtime deterministic|throughput] [--engine interpreter|bytecode|dual] [--locked] [--universe <id>] [--trust-store <file>] [--signer-id <id>] [--sign-key <file>] [--json]"
                );
                return 2;
            }
        ...
        "upgrade-check" => {
            let Some(project_dir) = args.get(1) else {
                eprintln!(
                    "usage: ocp upgrade-check <project_dir> [--manifest <file>] [--target-runtime <id>] [--out <file>] [--runtime deterministic|throughput] [--engine interpreter|bytecode|dual] [--locked] [--universe <id>] [--json]"
                );
                return 2;
            };
            let json_mode = args.iter().any(|a| a == "--json");
            ...
            let out_path = parse_string_flag(args, "--out")
                .map(PathBuf::from)
                .unwrap_or_else(|| {
                    PathBuf::from("target/ocp/w9/reports/upgrade_check_report.json")
                });
            ...
            // Use canonical conformance runner pipeline (same runner as `ocp test --conformance`).
            ...
            let report_json = json!({
                "schema": "ocp.upgrade_check.v1",
                "project_dir": project_dir,
                "target_runtime": target_runtime,
                "runner_contract": "ocp test --conformance",
        ...
        "lts" => {
            let Some(scope) = args.get(1).map(String::as_str) else {
                eprintln!(
                    "usage: ocp lts <check|report> ...\n  check: ocp lts check <project_dir> [--manifest <file>] [--target-runtime <id>] [--out <file>] [--runtime deterministic|throughput] [--engine interpreter|bytecode|dual] [--universe <id>] [--json]\n  report: ocp lts report <report_file> [--json]"
                );
                return 2;
            };
            match scope {
                "check" => {
                    let Some(project_dir) = args.get(2) else {
                        eprintln!(
                            "usage: ocp lts check <project_dir> [--manifest <file>] [--target-runtime <id>] [--out <file>] [--runtime deterministic|throughput] [--engine interpreter|bytecode|dual] [--universe <id>] [--json]"
                        );
                        return 2;
                    };
                    let json_mode = args.iter().any(|a| a == "--json");
                    ...
                    let out_path = parse_string_flag(args, "--out")
                        .map(PathBuf::from)
                        .unwrap_or_else(|| {
                            PathBuf::from("target/ocp/w15/reports/lts_check_report.json")
                        });
                    let target_runtime = parse_string_flag(args, "--target-runtime")
                        .unwrap_or_else(|| "current".to_string());
                    let runtime_mode_raw = parse_string_flag(args, "--runtime")
                ...
                _ => {
                    eprintln!("usage: ocp lts <check|report> ...");
                    2
                }
            }
        }
        "test" => {
            if args.iter().any(|a| a == "--conformance") {
                ...
                let out_path = parse_string_flag(args, "--out")
                    .map(PathBuf::from)
                    .unwrap_or_else(|| {
                        PathBuf::from("target/ocp/w9/reports/conformance_report.json")
                    });
                let runtime_mode_raw = parse_string_flag(args, "--runtime")
                    .unwrap_or_else(|| "deterministic".to_string());
                let runtime_mode = match runtime_mode_raw.as_str() {
                ...
                let sign_key = parse_string_flag(args, "--sign-key")
                    .map(PathBuf::from)
                    .or_else(|| std::env::var("OCP_SIGN_KEY_PATH").ok().map(PathBuf::from));

                if locked {
                    if sign_key.is_none() {
                        eprintln!(
                            "W9-CONFORMANCE-SIGN-REQUIRED: locked conformance requires --sign-key or OCP_SIGN_KEY_PATH"
                        );
                        return 12;
                    }
                ...
                let Some(path) = args.get(1) else {
                    eprintln!(
                        "usage: ocp test <project_dir> [--locked] [--universe <id>] [--domain <id>] [--shadow <id> --shadow-policy forbid_commit|shadow_commit_log] [--golden <dir>] [--clean]"
                    );
                    return 2;
                };
                let locked = args.iter().any(|a| a == "--locked");
        ...
        "build" => {
            let Some(path) = args.get(1) else {
                eprintln!(
                    "usage: ocp build <project_dir> [--locked] [--source-only] [--universe <id>] [--attest] [--attest-key <keyid>]"
                );
                return 2;
            };
            let locked = args.iter().any(|a| a == "--locked");
            ...
            if source_only {
                ...
                match build_ocppkg_with_lock(Path::new(path), locked) {
                    Ok(summary) => {
                        if attest {
                            match build_attestation_v15(Path::new(path), attest_key.as_deref()) {
                                Ok(attest_summary) => {
                                    println!(
                                        "build ok (.ocppkg={}, files_bundled={}, payload_hash_blake3={}, attestation={}, sig={}, manifest_hash={})",
                            ...
                            println!(
                                "build ok (.ocppkg={}, files_bundled={}, payload_hash_blake3={})",
                                summary.artifact_path.display(),
                                summary.files_bundled,
                                summary.payload_hash_blake3
                            );
        ...
        "publish" => {
            let Some(artifact) = args.get(1) else {
                eprintln!("usage: ocp publish <artifact.ocppkg> [--registry <dir>]");
                return 2;
            };
            let registry =
        ...
        "fetch" => {
            let Some(artifact_or_package) = args.get(1) else {
                eprintln!("usage: ocp fetch <artifact|package> [--registry <dir>] [--out <dir>]");
                return 2;
            };
            let registry =
                parse_string_flag(args, "--registry").unwrap_or_else(|| "registry".to_string());
        ...
        "verify-supply" => {
            let Some(artifact) = args.get(1) else {
                eprintln!("usage: ocp verify-supply <artifact.ocppkg>");
                return 2;
            };
        ...
        "deps" => {
            let Some(scope) = args.get(1).map(String::as_str) else {
                eprintln!("usage: ocp deps <resolve|update|verify> ...");
                return 2;
            };
            match scope {
                "resolve" => {
                    let Some(path) = args.get(2) else {
                        eprintln!("usage: ocp deps resolve <project_dir> [--write-legacy-lock]");
                        return 2;
                    };
                    ...
                    match resolve_deps_v3(Path::new(path), write_legacy) {
                        Ok(summary) => {
                            println!(
                                "deps resolve ok (deps_resolved={}, lock_hash={}, lock_v3={}, ocp_lock={}, legacy_v2={})",
                                summary.deps_resolved,
                                summary.lock_hash,
                                summary.lock_v3_path.display(),
                                summary.ocp_lock_path.display(),
                ...
                "update" => {
                    let Some(path) = args.get(2) else {
                        eprintln!(
                            "usage: ocp deps update <project_dir> [pkg] [--write-legacy-lock]"
                        );
                        return 2;
                    };
                    let target = args
                    ...
                    match resolve_deps_v3(Path::new(path), write_legacy) {
                        Ok(summary) => {
                            println!(
                                "deps update ok (target={}, deps_resolved={}, lock_hash={}, lock_v3={}, ocp_lock={}, legacy_v2={})",
                                target,
                                summary.deps_resolved,
                                summary.lock_hash,
                                summary.lock_v3_path.display(),
                ...
                "verify" => {
                    let Some(path) = args.get(2) else {
                        eprintln!("usage: ocp deps verify <project_dir> [--no-lane-policy]");
                        return 2;
                    };
                    let root = Path::new(path);
        ...
        "pack" => {
            let Some(scope) = args.get(1).map(String::as_str) else {
                eprintln!("usage: ocp pack <build|sign|publish|verify> ...");
                return 2;
            };
            match scope {
                "build" => {
                    let Some(path) = args.get(2) else {
                        eprintln!("usage: ocp pack build <project_dir> [--locked]");
                        return 2;
                    };
                    let locked = args.iter().any(|a| a == "--locked");
                    match build_ocppkg_with_lock(Path::new(path), locked) {
                ...
                "sign" => {
                    let Some(artifact) = args.get(2) else {
                        eprintln!("usage: ocp pack sign <artifact.ocppkg>");
                        return 2;
                    };
                    match sign_ocppkg(Path::new(artifact)) {
                        Ok(summary) => {
                            println!(
                ...
                "publish" => {
                    let Some(artifact) = args.get(2) else {
                        eprintln!("usage: ocp pack publish <artifact.ocppkg> [--registry <dir>]");
                        return 2;
                    };
                ...
                "verify" => {
                    let Some(artifact) = args.get(2) else {
                        eprintln!("usage: ocp pack verify <artifact.ocppkg>");
                        return 2;
                    };
                    match verify_supply_artifact(Path::new(artifact)) {
                        Ok(summary) => {
        ...
        "lock" => {
            let Some(sub) = args.get(1).map(String::as_str) else {
                eprintln!("usage: ocp lock <sync|sign|verify> ...");
                return 2;
            };
            match sub {
                "sync" => {
                    let Some(path) = args.get(2) else {
                        eprintln!("usage: ocp lock sync <project_dir> [--write-legacy-lock]");
                        return 2;
                    };
                    let write_legacy = args.iter().any(|a| a == "--write-legacy-lock");
                    match resolve_deps_v3(Path::new(path), write_legacy) {
                        Ok(summary) => {
                            println!(
                                "lock sync ok (deps_resolved={}, lock_hash={}, lock_v3={}, ocp_lock={}, legacy_v2={})",
                                summary.deps_resolved,
                                summary.lock_hash,
                                summary.lock_v3_path.display(),
                                summary.ocp_lock_path.display(),
                ...
                "sign" => {
                    let Some(path) = args.get(2) else {
                        eprintln!("usage: ocp lock sign <project_dir> --key <keyid> [--lock deps.lock.v3]");
                        return 2;
                    };
                ...
                "verify" => {
                    let Some(path) = args.get(2) else {
                        eprintln!("usage: ocp lock verify <project_dir> [--lock deps.lock.v3]");
                        return 2;
                    };
                    if let Some(lock_value) = parse_string_flag(args, "--lock") {
                        if lock_value.replace('\\', "/") != "deps.lock.v3" {
        ...
        "perm" => {
            let Some(sub_raw) = args.get(1).map(String::as_str) else {
                eprintln!("usage: ocp perm <snapshot|diff|approve|review|doctor|fix> ...");
                return 2;
            };
            ...
            match sub {
                "doctor" => {
                    let Some(path) = args.get(2) else {
                        eprintln!("usage: ocp perm doctor <project_dir> [--out <report.json>]");
                        return 2;
                    };
                ...
                "fix" => {
                    let Some(mode) = args.get(2).map(String::as_str) else {
                        eprintln!("usage: ocp perm fix <--plan|--apply> <project_dir> [options]");
                        return 2;
                    };
                    match mode {
                        "--plan" | "plan" => {
                            let Some(path) = args.get(3) else {
                                eprintln!(
                                    "usage: ocp perm fix --plan <project_dir> [--out <permission_fix_plan.json>]"
                                );
                                return 2;
                            };
                            let out_path = parse_string_flag(args, "--out").map(PathBuf::from);
                        ...
                        "--apply" | "apply" => {
                            let Some(path) = args.get(3) else {
                                eprintln!(
                                    "usage: ocp perm fix --apply <project_dir> [--plan-file <permission_fix_plan.json>] [--patch <permission_fix.patch.toml>] [--out <permission_fix_safety_report.json>] [--approval <permissions.approval.toml>] --ack-risk --justification <text> --by <id> --date <YYYY-MM-DD>"
                                );
                                return 2;
                            };
                            let Some(approved_by) = parse_string_flag(args, "--by") else {
                                eprintln!(
                                    "usage: ocp perm fix --apply <project_dir> ... --by <id> --date <YYYY-MM-DD>"
                                );
                                return 2;
                            };
                            ...
                            let Some(justification) = parse_string_flag(args, "--justification")
                                ...
                                eprintln!(
                                    "usage: ocp perm fix --apply <project_dir> ... --justification <text> --ack-risk"
                                );
                                return 2;
                            };
                            let plan_file =
                        ...
                        _ => {
                            eprintln!(
                                "usage: ocp perm fix <--plan|--apply> <project_dir> [options]"
                            );
                            2
                        }
                    }
                ...
                "snapshot" => {
                    let Some(path) = args.get(2) else {
                        eprintln!("usage: ocp perm snapshot <project_dir> [--out-dir <dir>]");
                        return 2;
                    };
                ...
                "diff" => {
                    let Some(old_path) = args.get(2) else {
                        eprintln!("usage: ocp perm diff <old> <new> [--out <report.json>] [--approval <permissions.approval.toml>]");
                        return 2;
                    };
                ...
                "approve" => {
                    let Some(diff_report) = args.get(2) else {
                        eprintln!("usage: ocp perm approve <diff_report.json> [--approval <permissions.approval.toml>] --by <id> --date <YYYY-MM-DD> [--note <text>]");
                        return 2;
                    };
                    let Some(approved_by) = parse_string_flag(args, "--by") else {
                        eprintln!("usage: ocp perm approve <diff_report.json> [--approval <permissions.approval.toml>] --by <id> --date <YYYY-MM-DD> [--note <text>]");
        ...
        "policy" => {
            ...
            {
                eprintln!("usage: ocp policy lock sync <project_dir>");
                return 2;
            }
            let Some(path) = args.get(3) else {
                eprintln!("usage: ocp policy lock sync <project_dir>");
        ...
        "cosmos" => {
            let Some(scope) = args.get(1).map(String::as_str) else {
                eprintln!("usage: ocp cosmos <init|lock> ...");
                return 2;
            };
            match scope {
                "init" => {
                    let Some(path) = args.get(2) else {
                        eprintln!("usage: ocp cosmos init <project_dir> [--preset default|ci]");
                        return 2;
                    };
                    let preset = parse_string_flag(args, "--preset")
                ...
                "lock" => {
                    if args.get(2).map(String::as_str) != Some("sync") {
                        eprintln!("usage: ocp cosmos lock sync <project_dir> [--locked] [--signer-id <id>] [--sign-key <file>] [--trust-store <file>]");
                        return 2;
                    }
                    let Some(path) = args.get(3) else {
                        eprintln!("usage: ocp cosmos lock sync <project_dir> [--locked] [--signer-id <id>] [--sign-key <file>] [--trust-store <file>]");
        ...
        "organ" => {
            let Some(scope) = args.get(1).map(String::as_str) else {
                eprintln!("usage: ocp organ <lock|verify|install> ...");
                return 2;
            };
            let json_mode = args.iter().any(|a| a == "--json");
            match scope {
                "lock" => {
                    if args.get(2).map(String::as_str) != Some("sync") {
                        eprintln!("usage: ocp organ lock sync <project_dir> [--registry <index.toml>] [--json]");
                        return 2;
                    }
                    let Some(path) = args.get(3) else {
                        eprintln!("usage: ocp organ lock sync <project_dir> [--registry <index.toml>] [--json]");
                ...
                "verify" => {
                    let Some(path) = args.get(2) else {
                        eprintln!(
                            "usage: ocp organ verify <project_dir> [--locked|--unlocked] [--json]"
                        );
                        return 2;
                    };
                    let locked = args.iter().any(|a| a == "--locked");
                ...
                "install" => {
                    let Some(name) = args.get(2) else {
                        eprintln!("usage: ocp organ install <name> <version> [--project <dir>] [--registry <index.toml>] [--locked] [--json]");
                        return 2;
                    };
        ...
        "kit" => {
            let Some(scope) = args.get(1).map(String::as_str) else {
                eprintln!("usage: ocp kit <list|doctor> ...");
                return 2;
            };
            let json_mode = args.iter().any(|a| a == "--json");
            match scope {
                ...
                "doctor" => {
                    let Some(path) = args.get(2) else {
                        eprintln!("usage: ocp kit doctor <project_dir> [--locked|--json]");
                        return 2;
                    };
                    let locked = args.iter().any(|a| a == "--locked");
        ...
        "plugin" => {
            let Some(scope) = args.get(1).map(String::as_str) else {
                eprintln!("usage: ocp plugin <lock|verify> ...");
                return 2;
            };
            match scope {
                "lock" => {
                    if args.get(2).map(String::as_str) != Some("sync") {
                        eprintln!("usage: ocp plugin lock sync <project_dir>");
                        return 2;
                    }
                    let Some(path) = args.get(3) else {
                        eprintln!("usage: ocp plugin lock sync <project_dir>");
                ...
                "verify" => {
                    let Some(path) = args.get(2) else {
                        eprintln!("usage: ocp plugin verify <project_dir>");
                        return 2;
                    };
        ...
        "compose" => {
            let Some(path) = args.get(1) else {
                eprintln!("usage: ocp compose <project_dir> --phenotype <file> [--registry <dir>] [--locked] [--universe <id>]");
                return 2;
            };
        ...
        "doc" => {
            let Some(topic) = args.get(1) else {
                eprintln!("usage: ocp doc packs [--json]");
                return 2;
            };
            match topic.as_str() {
                "packs" => {
        ...
        "verify" => {
            if args.get(1).map(String::as_str) == Some("--contract-sign") {
                let Some(contract_path) = args.get(2) else {
                    eprintln!("usage: ocp verify --contract-sign <contract.json> [--signer-id <id>] [--trust-epoch <u32>]");
                    return 2;
                };
                let signer_id = parse_string_flag(args, "--signer-id")
                ...
                let Some(contract_path) = args.get(2) else {
                    eprintln!("usage: ocp verify --contract <contract.json>");
                    return 2;
                };
                ...
                let Some(sig_or_contract) = args.get(2) else {
                    eprintln!(
                        "usage: ocp verify --contract-signature <contract.json.sig|contract.json>"
                    );
                    return 2;
                };
                ...
                let Some(artifact_dir) = args.get(2) else {
                    eprintln!("usage: ocp verify --attest <artifact_dir>");
                    return 2;
                };
                ...
                let Some(artifact_dir) = args.get(2) else {
                    eprintln!("usage: ocp verify --repro <artifact_dir>");
                    return 2;
                };
                ...
                let Some(path) = args.get(1) else {
                    eprintln!("usage: ocp verify <project_dir> --phenotype <file> [--registry <dir>] [--locked] [--universe <id>]");
                    eprintln!("       ocp verify --contract-sign <contract.json> [--signer-id <id>] [--trust-epoch <u32>]");
                    eprintln!("       ocp verify --contract <contract.json>");
                    eprintln!(
                        "       ocp verify --contract-signature <contract.json.sig|contract.json>"
                    );
                    eprintln!("       ocp verify --attest <artifact_dir>");
                    eprintln!("       ocp verify --repro <artifact_dir>");
                    return 2;
                };
                let Some(phenotype) = parse_string_flag(args, "--phenotype") else {
                    eprintln!("usage: ocp verify <project_dir> --phenotype <file> [--registry <dir>] [--locked] [--universe <id>]");
...
fn print_version() {
    println!("{} {}", PRIMARY_CLI_COMMAND, OCP_PUBLIC_VERSION);
}

fn render_doc_packs_text_v09() -> String {
    let registry = CapabilityRegistry::v1_baseline();
    let mut out = String::new();
    out.push_str("# OCP capability docs (v0.9)\n\n");
...
where
    F: FnOnce() -> T,
{
    let prev = std::env::var("OCP_VIEW_ID").ok();
    match view_id {
        Some(value) => std::env::set_var("OCP_VIEW_ID", value),
        None => std::env::remove_var("OCP_VIEW_ID"),
    }
    let out = run();
    match prev {
        Some(value) => std::env::set_var("OCP_VIEW_ID", value),
...
fn detect_upgrade_check_features_v14(project_root: &Path) -> UpgradeCheckFeaturesV14 {
    ...
    let manifest_path = manifest_path_for_v071(project_root);
    ...
    let mut ocp_files = Vec::<PathBuf>::new();
    collect_ocp_sources_v14(&project_root.join("src"), &mut ocp_files);
    if ocp_files.is_empty() {
        collect_ocp_sources_v14(project_root, &mut ocp_files);
    }

    for path in ocp_files {
...
fn collect_ocp_sources_v14(root: &Path, out: &mut Vec<PathBuf>) {
    if !root.exists() {
        return;
    }
    ...
    for entry in entries.flatten() {
        let path = entry.path();
        if path.is_dir() {
            collect_ocp_sources_v14(&path, out);
            continue;
        }
        if path
            .extension()
...
fn run_lts_check_report_v15(
    ...
    let permission_gate = if lane == "locked_v071" {
        ...
        if !baseline.exists() {
            let msg = format!(
                "X-PERMISSION-REVIEW-REQUIRED: missing baseline snapshot `{}`; run `ocp perm snapshot {}` first",
                baseline.display(),
...
fn run_budget_command(args: &[String]) -> i32 {
    let Some(subcmd) = args.get(1).map(String::as_str) else {
        eprintln!("usage: ocp budget <analyze|doctor> <artifact_dir|audit.jsonl> [--json]");
        return 2;
    };
    let Some(path) = args.get(2) else {
        eprintln!("usage: ocp budget <analyze|doctor> <artifact_dir|audit.jsonl> [--json]");
...
fn resolve_artifact_dir_for_cassette_arg_v17(path: &Path) -> Result<PathBuf, SdkError> {
    ...
    let artifacts_root = path.join(".ocp_artifacts");
    if artifacts_root.is_dir() {
        return latest_artifact_dir_v17(&artifacts_root);
    }
    Err(SdkError::MissingProject(format!(
        "V-CASSETTE-PATH: `{}` is neither an artifact dir nor a project dir with .ocp_artifacts",
        path.display()
    )))
}
...
fn run_cassette_stats_command_v17(args: &[String]) -> i32 {
    let Some(path) = args.get(2) else {
        eprintln!("usage: ocp cassette stats <artifact_dir> [--json]");
        return 2;
    };
    let json_mode = has_flag(args, "--json");
...
fn run_cassette_prune_command_v17(args: &[String]) -> i32 {
    let Some(path) = args.get(2) else {
        eprintln!(
            "usage: ocp cassette prune <artifact_dir> --plan|--apply [--ttl-days <u32>] [--json]"
        );
        return 2;
    };
...
fn run_cassette_gc_command_v17(args: &[String]) -> i32 {
    let Some(path) = args.get(2) else {
        eprintln!("usage: ocp cassette gc <artifact_dir> [--json]");
        return 2;
    };
    let json_mode = has_flag(args, "--json");
    let artifact_dir = match resolve_artifact_dir_for_cassette_arg_v17(Path::new(path)) {
...
fn run_cassette_upgrade_command_v17(args: &[String]) -> i32 {
    let Some(path) = args.get(2) else {
        eprintln!("usage: ocp cassette upgrade <artifact_dir> --plan|--apply [--max-entries <u32>] [--max-block-bytes <u32>] [--json]");
        return 2;
    };
    let plan_mode = has_flag(args, "--plan");
    let apply_mode = has_flag(args, "--apply");
...
fn infer_project_root_from_artifact_dir_v17(artifact_dir: &Path) -> PathBuf {
    ...
    if let Some(artifacts_root) = parent {
        if artifacts_root
            .file_name()
            .map(|v| v.to_string_lossy() == ".ocp_artifacts")
            .unwrap_or(false)
        {
...
fn verify_dependency_override_guardrails_v10(root: &Path) -> Result<(), SdkError> {
    ...
    if std::env::var("OCP_ALLOW_OVERRIDES").unwrap_or_default() != "1" {
        return Err(SdkError::PermissionDenied(
            "V-PERMISSION-OVERRIDE-ENV-REQUIRED: set OCP_ALLOW_OVERRIDES=1 to allow permission_overrides in this run.".to_string(),
        ));
    }
    Ok(())
}
...
fn apply_tool_cli_template_v072(root: &Path) -> Result<(), SdkError> {
    ...
    fs::write(manifest_path_for_v071(root), manifest)?;
    ...
    fs::write(root.join("src").join("main.ocp"), source)?;
...
fn apply_tool_http_template_v08(root: &Path) -> Result<(), SdkError> {
    ...
    let readme = concat!(
        ...
        "Run record:\n",
        "- `OCP_QUARANTINE=1 ocp run .`\n\n",
        "Replay:\n",
        "- `OCP_QUARANTINE=1 ocp replay ./.ocp_artifacts/<run_id>`\n"
    );
    fs::write(root.join("README.md"), readme)?;
    Ok(())
}
...
fn apply_mini_game_template_v073(root: &Path) -> Result<(), SdkError> {
    ...
    let readme = concat!(
        "# mini-game template\n\n",
        "- Run: `ocp run .`\n",
        "- Replay: `ocp replay ./.ocp_artifacts/<run_id>`\n"
    );
    fs::write(root.join("README.md"), readme)?;
    Ok(())
}
...
fn apply_dep_permission_template_v10(root: &Path) -> Result<(), SdkError> {
    ...
    fs::write(dep_dir.join("package.ocpp"), dep_manifest)?;
    fs::write(
        dep_dir.join("src").join("widget.ocp"),
    ...
    let readme = concat!(
        ...
        "  - `ocp deps verify .`\n",
        "  - `ocp pack build .`\n",
        "  - `ocp pack sign ./.ocppkg/<artifact>.ocppkg`\n"
    );
    fs::write(root.join("README.md"), readme)?;
    Ok(())
}
...
fn manifest_path_for_v071(root: &Path) -> PathBuf {
    let upper_new = root.join("Ocp.toml");
    if upper_new.exists() {
        return upper_new;
    }
    let lower_new = root.join("ocp.toml");
    if lower_new.exists() {
        return lower_new;
    }
    let lower = root.join("ocp.toml");
    if lower.exists() {
        return lower;
    }
    let upper_legacy = root.join("Ocp.toml");
    if upper_legacy.exists() {
        return upper_legacy;
    }
    root.join("Ocp.toml")
}
...
fn read_lane_and_entry_for_v071(root: &Path) -> (String, String) {
    ...
    let mut entry = "src/main.ocp".to_string();
...
fn quarantine_env_enabled_v073() -> bool {
    matches!(std::env::var("OCP_QUARANTINE"), Ok(v) if v.trim() == "1")
}
...
fn enforce_quarantine_lane_gate_v073(lane: &str) -> Result<(), SdkError> {
    ...
    if mode == LaneModeV071::Quarantine && !quarantine_env_enabled_v073() {
        return Err(SdkError::MissingProject(
            "V-QUARANTINE-DISABLED: lane `quarantine` requires env `OCP_QUARANTINE=1`".to_string(),
        ));
    }
    Ok(())
}
...
const CASSETTE_SIGN_SECRET_REL_PATH_V15: &str = ".ocp_signing/cassette_sign.key.v15";
...
const ENV_PROJECT_LANE_V08: &str = "OCP_PROJECT_LANE";
const ENV_QUARANTINE_MODE_V08: &str = "OCP_QUARANTINE_MODE";
const ENV_WALLCLOCK_RECORD_PATH_V08: &str = "OCP_V08_WALLCLOCK_RECORD_PATH";
const ENV_PROC_RECORD_PATH_V08: &str = "OCP_V08_PROC_RECORD_PATH";
const ENV_HTTP_RECORD_PATH_V08: &str = "OCP_V08_HTTP_RECORD_PATH";
const ENV_CASSETTE_JSONL_PATH_V08: &str = "OCP_V08_CASSETTE_JSONL_PATH";
const ENV_CASSETTE_INDEX_PATH_V08: &str = "OCP_V08_CASSETTE_INDEX_PATH";
const ENV_STD_FS_ROOT_V08: &str = "OCP_STD_FS_ROOT";
const ENV_STD_FS_ALLOW_READ_V08: &str = "OCP_STD_FS_ALLOW_READ";
const ENV_STD_FS_ALLOW_WRITE_V08: &str = "OCP_STD_FS_ALLOW_WRITE";
const ENV_STD_FS_ALLOW_REMOVE_V08: &str = "OCP_STD_FS_ALLOW_REMOVE";
const ENV_STD_FS_ALLOW_RENAME_V08: &str = "OCP_STD_FS_ALLOW_RENAME";
const ENV_STD_FS_ALLOW_LIST_V08: &str = "OCP_STD_FS_ALLOW_LIST";
const ENV_STD_FS_MAX_READ_BYTES_V08: &str = "OCP_STD_FS_MAX_READ_BYTES";
const ENV_STD_FS_MAX_WRITE_BYTES_V08: &str = "OCP_STD_FS_MAX_WRITE_BYTES";
const ENV_STD_FS_MAX_LIST_ENTRIES_V08: &str = "OCP_STD_FS_MAX_LIST_ENTRIES";
const ENV_STD_PROC_ALLOW_BINS_V08: &str = "OCP_STD_PROC_ALLOW_BINS";
const ENV_STD_PROC_TIMEOUT_MS_V08: &str = "OCP_STD_PROC_TIMEOUT_MS";
const ENV_STD_PROC_MAX_STDOUT_BYTES_V08: &str = "OCP_STD_PROC_MAX_STDOUT_BYTES";
const ENV_STD_PROC_MAX_STDERR_BYTES_V08: &str = "OCP_STD_PROC_MAX_STDERR_BYTES";
const ENV_STD_NET_ALLOW_HOSTS_V08: &str = "OCP_STD_NET_ALLOW_HOSTS";
const ENV_STD_NET_ALLOW_METHODS_V08: &str = "OCP_STD_NET_ALLOW_METHODS";
const ENV_STD_NET_TIMEOUT_MS_V08: &str = "OCP_STD_NET_TIMEOUT_MS";
const ENV_STD_NET_MAX_BODY_BYTES_V08: &str = "OCP_STD_NET_MAX_BODY_BYTES";
...
fn checkpoint_seed_state_digest_v11() -> String {
    sha256_hex(b"ocp.v11.checkpoint.state.seed")
}

fn checkpoint_seed_env_digest_v11() -> String {
    sha256_hex(b"ocp.v11.checkpoint.env.seed")
}
...
fn collect_cache_stats_report_v13(project_root: &Path) -> Result<CacheStatsReportV13, SdkError> {
    ...
    let local_compile_root = project_root.join(".ocp_cache").join("compile");
...
fn clean_cache_state_v13(project_root: &Path) -> Result<CacheCleanSummaryV13, SdkError> {
    ...
    let artifacts_dir = project_root.join(".ocp_artifacts");
    if artifacts_dir.exists() {
        fs::remove_dir_all(&artifacts_dir)?;
        summary.removed_artifacts = true;
    }
    let local_cache_dir = project_root.join(".ocp_cache");
    if local_cache_dir.exists() {
        fs::remove_dir_all(&local_cache_dir)?;
        summary.removed_local_cache = true;
    }
    let target_cache_dir = project_root.join("target").join("ocp").join("cache");
...
fn run_cache_benchmark_v13(
    ...
    let cold = with_runtime_env_v08(vec![("OCP_CACHE_DISABLE", Some("0".to_string()))], || {
        run_project_with_trace_engine_and_lock(project_root, run_engine, locked)
    })?;
    let warm = with_runtime_env_v08(vec![("OCP_CACHE_DISABLE", Some("0".to_string()))], || {
        run_project_with_trace_engine_and_lock(project_root, run_engine, locked)
    })?;
    let no_cache =
    ...
    let report_dir = project_root.join("target").join("ocp").join("v13");
...
fn emit_v071_artifacts_for_project_run(
    ...
    let artifact_dir = project_root.join(".ocp_artifacts").join(&run_id);
...
fn emit_v072_io_replay_metadata(project_root: &Path, artifact_dir: &Path) -> Result<(), SdkError> {
    ...
    let kv_source = project_root.join(".ocp_state").join("kv.json");
...
fn phase_to_str(phase: ocp_runtime_core::DiagPhase) -> &'static str {
    match phase {
        ocp_runtime_core::DiagPhase::Parse => "parse",
        ocp_runtime_core::DiagPhase::Typecheck => "typecheck",
        ocp_runtime_core::DiagPhase::Exec => "exec",
        ocp_runtime_core::DiagPhase::Runtime => "runtime",
    }
}
...
fn print_help() {
    eprintln!("{} <command> [args]", PRIMARY_CLI_COMMAND);
    eprintln!("commands:");
    ...
    eprintln!("  publish <artifact.ocppkg> [--registry <dir>]");
    eprintln!("  fetch <artifact|package> [--registry <dir>] [--out <dir>]");
    eprintln!("  verify-supply <artifact.ocppkg>");
    ...
    eprintln!("  pack  sign <artifact.ocppkg>");
    eprintln!("  pack  publish <artifact.ocppkg> [--registry <dir>]");
    eprintln!("  pack  verify <artifact.ocppkg>");
...
mod tests {
    ...
    fn temp_project_dir(tag: &str) -> PathBuf {
        let stamp = SystemTime::now()
            ...
            .expect("clock drift")
            .as_millis();
        std::env::temp_dir().join(format!("ocp_cli_w1_{tag}_{stamp}"))
    }
...
"#;
    ...
    fn assert_v071_artifacts_written(root: &Path) {
        ...
        assert!(artifacts_root.exists(), "missing .ocp_artifacts directory");
        assert!(
            artifacts_root.is_dir(),
            ".ocp_artifacts must be a directory"
        );

        let mut found_bundle = false;
        let entries = fs::read_dir(&artifacts_root).expect("read .ocp_artifacts");
        for entry in entries {
    ...
    fn v7_a_cli_run_emits_artifacts_on_fail() {
        ...
        prepare_runtime_project(&root);
        fs::write(root.join("src").join("main.ocp"), "let = 1;\n").expect("write invalid source");
        let args = vec!["run".to_string(), root.to_string_lossy().to_string()];
    ...
    fn prepare_v5_w1_universe_project(root: &Path) {
        prepare_runtime_project(root);
        let mut manifest = fs::read_to_string(manifest_path_for_v071(root)).expect("read manifest");
        manifest.push_str("\n[policy]\nbudget_profile = \"ci_default\"\n");
        fs::write(manifest_path_for_v071(root), manifest).expect("write manifest with policy");
    }
...
"#;
        fs::write(root.join("tests").join("smoke.ocp"), smoke).expect("write smoke");
        sync_deps_lock_v1(root).expect("sync lock");
    }
    ...
    fn w4_cli_build_publish_fetch_verify_and_run_artifact_pass() {
        ...
        let artifact = root.join(".ocppkg").join("w4_cli_pkg-0.1.0.ocppkg");
        assert!(artifact.exists(), "artifact .ocppkg not found");
        ...
        let fetched = out.join("w4_cli_pkg-0.1.0.ocppkg");
    ...
    fn w9_cli_conformance_requires_sign_key_when_locked() {
        std::env::remove_var("OCP_SIGN_KEY_PATH");
        let args = vec![
            "test".to_string(),
            "--conformance".to_string(),
            "--manifest".to_string(),
            "projects/ocp/conformance/conformance.v1.toml".to_string(),
            "--locked".to_string(),
            "--signer-id".to_string(),
            "dev-root-1".to_string(),
        ];
