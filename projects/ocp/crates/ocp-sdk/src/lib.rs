...
pub fn project_layout(root: &Path) -> ProjectLayout {
    ...
    let manifest = if manifest_upper_new.exists() {
        manifest_upper_new
    } else if manifest_lower_new.exists() {
        manifest_lower_new
    } else {
        root.join("Ocp.toml")
    };
    ...
    ProjectLayout {
        ...
        bundle_dir: root.join(".ocpbundle"),
    }
}
...
fn std_fs_action_from_key(key: &str) -> Option<&'static str> {
    match key {
        "std.fs.read" | "std.fs.read_text" | "std.fs.stat" => Some("read"),
        "std.fs.list" | "std.fs.list_dir" => Some("list"),
        "std.fs.write" | "std.fs.write_text" | "std.fs.mkdir" => Some("write"),
...
fn build_permission_hint_for_std_fs(action: &str) -> String {
    match action {
        "read" => {
            "Hint: add `[permissions.std_fs]` with `read = [\"./data/**\"]` in Ocp.toml."
                .to_string()
        }
        "list" => {
            "Hint: add `[permissions.std_fs]` with `list = [\"./data/**\"]` in Ocp.toml."
                .to_string()
        }
        "write" => {
            "Hint: add `[permissions.std_fs]` with `write = [\"./out/**\"]` in Ocp.toml."
                .to_string()
        }
        "remove" => {
            "Hint: add `[permissions.std_fs]` with `remove = [\"./out/**\"]` in Ocp.toml."
                .to_string()
        }
        "rename" => {
            "Hint: add `[permissions.std_fs]` with `rename = [\"./out/**\"]` in Ocp.toml."
                .to_string()
        }
...
fn verify_pack_permissions_for_key(
    ...
    if let Some(action) = engine_ui_action_from_key(key) {
        let Some(cfg) = permissions.std_ui.as_ref() else {
            return Err(SdkError::PermissionDenied(format!(
                "V-PERMISSION-DENIED: key `{}` is denied for file `{}`{}; reason=`RC-UI-DISABLED`; missing section `[permissions.std_ui]` required by engine.ui. Hint: add `[permissions.std_ui]` with `enabled = true` in Ocp.toml.",
                key,
                file_path.display(),
                module_path
    ...
    if let Some(action) = engine_game_action_from_key(key) {
        let Some(cfg) = permissions.std_game.as_ref() else {
            return Err(SdkError::PermissionDenied(format!(
                "V-PERMISSION-DENIED: key `{}` is denied for file `{}`{}; reason=`RC-GAME-DISABLED`; missing section `[permissions.std_game]` required by engine.game. Hint: add `[permissions.std_game]` with `enabled = true` in Ocp.toml.",
                key,
                file_path.display(),
                module_path
                    .map(|m| format!(" (module `{m}`)"))
    ...
    if let Some(action) = engine_shadow_action_from_key(key) {
        let Some(cfg) = permissions.std_shadow.as_ref() else {
            return Err(SdkError::PermissionDenied(format!(
                "V-PERMISSION-DENIED: key `{}` is denied for file `{}`{}; reason=`RC-SHADOW-DISABLED`; missing section `[permissions.std_shadow]` required by engine.shadow. Hint: add `[permissions.std_shadow]` with `enabled = true` in Ocp.toml.",
                key,
                file_path.display(),
                module_path
                    .map(|m| format!(" (module `{m}`)"))
    ...
    if key.starts_with("std.kv.") {
        let Some(cfg) = permissions.std_kv.as_ref() else {
            return Err(SdkError::PermissionDenied(format!(
                "V-PERMISSION-DENIED: key `{}` is denied for file `{}`{}; reason=`RC-KV-PERMISSION-DENIED`; missing section `[permissions.std_kv]`. Hint: add `[permissions.std_kv]` with `enabled = true` in Ocp.toml.",
                key,
                file_path.display(),
                module_path
                    .map(|m| format!(" (module `{m}`)"))
    ...
    if key.starts_with("std.time.") {
        let Some(cfg) = permissions.std_time.as_ref() else {
            return Err(SdkError::PermissionDenied(format!(
                "V-PERMISSION-DENIED: key `{}` is denied for file `{}`{}; reason=`RC-TIME-DISABLED`; missing section `[permissions.std_time]`. Hint: add `[permissions.std_time]` with `enabled = true` in Ocp.toml.",
                key,
                file_path.display(),
                module_path
                    .map(|m| format!(" (module `{m}`)"))
    ...
    if let Some(action) = std_net_http_action_from_key(key) {
        let Some(cfg) = permissions.std_net_http.as_ref() else {
            return Err(SdkError::PermissionDenied(format!(
                "V-PERMISSION-DENIED: key `{}` is denied for file `{}`{}; reason=`RC-NET-HOST-DENIED`; missing section `[permissions.std_net_http]`. Hint: add `[permissions.std_net_http]` with `enabled = true`, non-empty `allow_hosts`, and non-empty `allow_methods` in Ocp.toml.",
                key,
                file_path.display(),
                module_path
                    .map(|m| format!(" (module `{m}`)"))
    ...
    if let Some(action) = std_proc_action_from_key(key) {
        let Some(cfg) = permissions.std_proc.as_ref() else {
            return Err(SdkError::PermissionDenied(format!(
                "V-PERMISSION-DENIED: key `{}` is denied for file `{}`{}; reason=`RC-PROC-BIN-DENIED`; missing section `[permissions.std_proc]`. Hint: add `[permissions.std_proc]` with `enabled = true` and non-empty `allow_bins` in Ocp.toml.",
                key,
                file_path.display(),
                module_path
                    .map(|m| format!(" (module `{m}`)"))
    ...
    if let Some(action) = std_game_action_from_key(key) {
        let Some(cfg) = permissions.std_game.as_ref() else {
            return Err(SdkError::PermissionDenied(format!(
                "V-PERMISSION-DENIED: key `{}` is denied for file `{}`{}; reason=`RC-GAME-DISABLED`; missing section `[permissions.std_game]`. Hint: add `[permissions.std_game]` with `enabled = true` in Ocp.toml.",
                key,
                file_path.display(),
                module_path
                    .map(|m| format!(" (module `{m}`)"))
    ...
    if let Some(action) = std_shadow_action_from_key(key) {
        let Some(cfg) = permissions.std_shadow.as_ref() else {
            return Err(SdkError::PermissionDenied(format!(
                "V-PERMISSION-DENIED: key `{}` is denied for file `{}`{}; reason=`RC-SHADOW-DISABLED`; missing section `[permissions.std_shadow]`. Hint: add `[permissions.std_shadow]` with `enabled = true` in Ocp.toml.",
                key,
                file_path.display(),
                module_path
                    .map(|m| format!(" (module `{m}`)"))
    ...
    if let Some(action) = std_ui_action_from_key(key) {
        let Some(cfg) = permissions.std_ui.as_ref() else {
            return Err(SdkError::PermissionDenied(format!(
                "V-PERMISSION-DENIED: key `{}` is denied for file `{}`{}; reason=`RC-UI-DISABLED`; missing section `[permissions.std_ui]`. Hint: add `[permissions.std_ui]` with `enabled = true` in Ocp.toml.",
                key,
                file_path.display(),
                module_path
                    .map(|m| format!(" (module `{m}`)"))
...
fn load_permissions_for_layout(
    ...
    if locked && permissions.package.is_none() {
        return Err(SdkError::PermissionMissing(
            "V-PERMISSIONS-MISSING: `--locked` requires [permissions.package] in Ocp.toml"
                .to_string(),
        ));
    }
...
pub fn init_project(root: &Path) -> Result<ProjectLayout, SdkError> {
    ...
    let smoke = layout.tests_dir.join("smoke.ocp");
    if !smoke.exists() {
...
fn verify_project_exists(layout: &ProjectLayout) -> Result<(), SdkError> {
    ...
    warn_legacy_ocp_sources(layout);
    Ok(())
}

fn warn_legacy_ocp_sources(layout: &ProjectLayout) {
    let Ok(legacy_files) = gather_legacy_ocp_files(layout) else {
        return;
    };
...
fn parse_manifest_dependencies(manifest: &str) -> Result<Vec<ManifestDep>, SdkError> {
    ...
    for raw in manifest.lines() {
        ...
        let Some((k, v)) = line.split_once('=') else {
            return Err(SdkError::MissingProject(
                "invalid [dependencies] entry in Ocp.toml".to_string(),
            ));
        };
        let alias = k.trim();
        if alias.is_empty() {
...
fn load_dependency_package_index_v10(
    ...
    let package_manifest_path = dep_root.join("package.ocpp");
...
fn verify_lock_consistency(layout: &ProjectLayout) -> Result<Vec<LockDep>, SdkError> {
    ...
    if !layout.deps_lock.exists() {
        return Err(SdkError::LockMismatch(format!(
            "missing deps.lock: {} (run `ocp lock sync <project_dir>`)",
    ...
    if current != expected {
        return Err(SdkError::LockMismatch(
            "deps.lock mismatch with Ocp.toml dependencies (run `ocp lock sync <project_dir>`)"
                .to_string(),
        ));
    }
...
fn ocp_lock_export_path(layout: &ProjectLayout) -> PathBuf {
    layout.root.join("ocp.lock")
}
...
fn encode_ocp_lock_export_v3(lock_hash: &str, lock_deps: &[LockDepV3]) -> String {
    let mut out = String::from("version=1\nsource=deps.lock.v3\n");
...
fn verify_lock_v3_consistency(layout: &ProjectLayout) -> Result<Vec<LockDepV3>, SdkError> {
    ...
    if current != expected {
        return Err(SdkError::LockMismatch(
            "deps.lock.v3 mismatch with Ocp.toml dependencies (run `ocp deps resolve <project_dir>`)"
                .to_string(),
        ));
    }
...
fn write_lock_v3_and_export(layout: &ProjectLayout) -> Result<(usize, String), SdkError> {
    ...
    fs::write(
        ocp_lock_export_path(layout),
        encode_ocp_lock_export_v3(&lock_hash, &deps_v3),
    )?;
...
pub fn resolve_deps_v3(
    ...
    Ok(DepResolveSummaryV3 {
        ...
        ocp_lock_path: ocp_lock_export_path(&layout),
...
fn collect_legacy_ocp_files(base: &Path, out: &mut Vec<PathBuf>) -> Result<(), SdkError> {
    if !base.exists() {
        return Ok(());
    }
    for entry in fs::read_dir(base)? {
...
fn gather_legacy_ocp_files(layout: &ProjectLayout) -> Result<Vec<PathBuf>, SdkError> {
    let mut files = Vec::new();
    collect_legacy_ocp_files(&layout.root.join("src"), &mut files)?;
    collect_legacy_ocp_files(&layout.tests_dir, &mut files)?;
    files.sort();
    Ok(files)
}

fn gather_project_ocp_files(layout: &ProjectLayout) -> Result<Vec<PathBuf>, SdkError> {
    let mut files = Vec::new();
...
fn enforce_locked_plugin_contract(layout: &ProjectLayout, locked: bool) -> Result<(), SdkError> {
    ...
    std::env::set_var("OCP_PLUGIN_ROOT", layout.root.to_string_lossy().to_string());
    Ok(())
}
...
pub fn run_reactor_service_with_lock(
    ...
    if options.socket_listen.is_some() && !runtime_budget.std_net_enabled {
        return Err(SdkError::MissingProject(
            "socket runtime requires `[runtime] std_net = true` in Ocp.toml".to_string(),
        ));
    }
...
fn parse_package_name_version(manifest: &str) -> Result<(String, String), SdkError> {
    ...
    let Some(name) = name else {
        return Err(SdkError::MissingProject(
            "Ocp.toml missing [package].name".to_string(),
        ));
    };
    let Some(version) = version else {
        return Err(SdkError::MissingProject(
            "Ocp.toml missing [package].version".to_string(),
        ));
    };
...
fn parse_package_manifest_v10(raw: &str) -> Result<PackageManifestV10, SdkError> {
    ...
    let Some(name) = name else {
        return Err(SdkError::MissingProject(
            "package.ocpp missing [package].name".to_string(),
        ));
    };
    let Some(version) = version else {
        return Err(SdkError::MissingProject(
...
fn load_package_manifest_v10(layout: &ProjectLayout) -> Result<PackageManifestV10, SdkError> {
    let package_ocpp = layout.root.join("package.ocpp");
    if package_ocpp.exists() {
        let raw = fs::read_to_string(&package_ocpp)?;
...
fn parse_ocppkg(path: &Path) -> Result<ParsedOcpPkg, SdkError> {
    let raw = fs::read_to_string(path)?;
    let mut lines = raw.lines();
    if lines.next() != Some("OCPPKGv1") {
        return Err(SdkError::SupplyInvalid("invalid .ocppkg magic".to_string()));
    }
    ...
    for line in payload.lines() {
        if let Some(v) = line.strip_prefix("name=") {
            ...
            let Some((rel, content_b64)) = v.split_once('|') else {
                return Err(SdkError::SupplyInvalid(
                    "invalid file line in .ocppkg".to_string(),
                ));
            };
            ...
            if parts.len() != 5 {
                return Err(SdkError::SupplyInvalid(
                    "invalid dep line in .ocppkg".to_string(),
                ));
            }
            deps.push(LockDepV2 {
                name: parts[0].to_string(),
...
pub fn sign_ocppkg(path: &Path) -> Result<SignOcpPkgSummary, SdkError> {
    let parsed = parse_ocppkg(path)?;
    ...
    let mut ocppkg = String::new();
    ocppkg.push_str("OCPPKGv1\n");
    ocppkg.push_str("payload_hash_blake3=");
    ocppkg.push_str(&parsed.payload_hash_blake3);
    ocppkg.push('\n');
    if let Some(content_hash_sha256) = parsed.content_hash_sha256.as_ref() {
        ocppkg.push_str("content_hash_sha256=");
        ocppkg.push_str(content_hash_sha256);
        ocppkg.push('\n');
    }
    ocppkg.push_str("signature_ed25519=");
    ocppkg.push_str(&signature_ed25519_b64);
    ocppkg.push('\n');
    ocppkg.push_str("signer_pub_ed25519=");
    ocppkg.push_str(&signer_pub_ed25519_b64);
    ocppkg.push('\n');
    ocppkg.push_str(&parsed.payload);
    fs::write(path, ocppkg)?;

    Ok(SignOcpPkgSummary {
...
pub fn build_ocppkg(root: &Path) -> Result<BuildOcpPkgSummary, SdkError> {
    build_ocppkg_with_lock(root, false)
}

pub fn build_ocppkg_with_lock(root: &Path, locked: bool) -> Result<BuildOcpPkgSummary, SdkError> {
    ...
    ocppkg.push_str(&signer_pub_ed25519_b64);
    ocppkg.push('\n');
    ocppkg.push_str(&payload);

    let artifact_dir = layout.root.join(".ocppkg");
    fs::create_dir_all(&artifact_dir)?;
    let artifact_path = artifact_dir.join(format!("{package_name}-{version}.ocppkg"));
    fs::write(&artifact_path, ocppkg)?;

    Ok(BuildOcpPkgSummary {
        artifact_path,
...
pub fn publish_artifact(
    ...
    let parsed = parse_ocppkg(artifact_path)?;
...
pub fn fetch_artifact(
    ...
    let target = if artifact_or_package.ends_with(".ocppkg") {
        entries
            .into_iter()
...
fn result_kind_label(kind: ocp_runtime_core::ResultKind) -> String {
    match kind {
        ocp_runtime_core::ResultKind::Ok => "ok".to_string(),
        ocp_runtime_core::ResultKind::Degraded => "degraded".to_string(),
        ocp_runtime_core::ResultKind::Insufficient => "insufficient".to_string(),
        ocp_runtime_core::ResultKind::Deferred => "deferred".to_string(),
    }
}
