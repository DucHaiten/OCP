...
pub fn default_conformance_manifest_path(workspace_root: &Path) -> PathBuf {
    let selector = std::env::var("OCP_CONFORMANCE_DEFAULT").ok();
...
pub fn default_conformance_manifest_path_with_selector(
    ...
    let v1 = workspace_root.join("projects/ocp/conformance/conformance.v1.toml");
    let v5 = workspace_root.join("projects/ocp/conformance/conformance.v5.toml");
    match selector
        .unwrap_or_default()
        .trim()
        .to_ascii_lowercase()
...
pub fn parse_conformance_manifest_v1(path: &Path) -> Result<ConformanceManifestV1, SdkError> {
    ...
    for raw_line in raw.lines() {
        ...
        if !in_scenario {
            if key == "version" && strip_quotes(value) == "1" {
                schema = Some("ocp.conformance.v1".to_string());
            } else if key == "schema" {
    ...
    let schema = schema.ok_or_else(|| {
        SdkError::MissingProject(
            "invalid conformance manifest: missing `version = 1` or `schema = \"ocp.conformance.manifest.v5\"`"
                .to_string(),
        )
    })?;
    if schema != "ocp.conformance.v1" && schema != "ocp.conformance.manifest.v5" {
        return Err(SdkError::MissingProject(format!(
...
fn resolve_organ_registry_index_path(
    ...
    workspace_root.join("projects/ocp/registry/organs/index.toml")
}
...
fn run_replay_artifact_step(
    ...
    let artifact_dir = app_root.join(".ocp_artifacts").join("conformance.replay");
...
fn assert_supplychain_outputs_v14(
    ...
    let output_dir = app_root.join(".ocp_artifacts").join("conformance.outputs");
...
fn render_deps_output_v14(app_root: &Path) -> Result<String, SdkError> {
    ...
    let mut out = String::new();
    out.push_str("{\n");
    out.push_str("  \"schema\":\"ocp.conformance.deps.v14\",\n");
...
fn render_permissions_effective_output_v14(app_root: &Path) -> Result<String, SdkError> {
    ...
    let mut out = String::new();
    out.push_str("{\n");
    out.push_str("  \"schema\":\"ocp.conformance.permissions_effective.v14\",\n");
    out.push_str("  \"packages\":[");
...
fn copy_runtime_registry(runtime_root: &Path, workspace_root: &Path) -> Result<PathBuf, SdkError> {
    let src = workspace_root.join("projects/ocp/registry");
