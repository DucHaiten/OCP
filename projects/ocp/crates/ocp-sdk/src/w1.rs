...
pub fn sync_cosmos_lock_v1(
    ...
    if !cosmos_path.exists() {
        return Err(SdkError::LockMismatch(format!(
            "V-COSMOS-MISSING: missing cosmos config `{}` (run `ocp cosmos init <project_dir> --preset ci`)",
            cosmos_path.display()
        )));
    }
...
fn verify_policy_lock_matches_manifest_v1(root: &Path) -> Result<(), SdkError> {
    ...
    if !policy_lock_path.exists() {
        return Err(SdkError::LockMismatch(format!(
            "V-POLICY-LOCK-REQUIRED: missing `{}` (run `ocp policy lock sync <project_dir>`)",
...
fn verify_cosmos_lock_matches_config_v1(
    ...
    if !lock_path.exists() {
        return Err(SdkError::LockMismatch(format!(
            "V-COSMOS-LOCK-REQUIRED: missing `{}` (run `ocp cosmos lock sync <project_dir> --locked ...`)",
            lock_path.display()
        )));
    }
