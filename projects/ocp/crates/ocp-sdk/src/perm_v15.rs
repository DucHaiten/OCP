    ...
    if let Some(approval_file) = approval_path {
        ...
        if introduces_new_permissions && !approved {
            return Err(SdkError::PermissionDenied(format!(
                "X-PERMISSION-APPROVAL-REQUIRED: permission diff `{}` is not approved in `{}`; reason=`RC-PERMISSION-UNAPPROVED`. Run `ocp perm approve {}` first.",
...
pub(crate) fn enforce_permission_review_policy_v15(root: &Path) -> Result<(), SdkError> {
    ...
    if !approved {
        return Err(SdkError::PermissionDenied(format!(
            "X-PERMISSION-APPROVAL-REQUIRED: permission diff `{}` is not approved in `{}`; reason=`RC-PERMISSION-UNAPPROVED`. Run `ocp perm snapshot`, `ocp perm diff`, and `ocp perm approve` first.",
            diff_hash,
            approval.display()
        )));
    }
...
fn default_w17_dx_root_v17(root: &Path) -> PathBuf {
    root.join("target").join("ocp").join("w17").join("dx")
}

fn wildcard_risk_hint_v17() -> String {
    "Run `ocp perm fix --plan <project_dir>` and replace wildcard with explicit scoped allowlist."
        .to_string()
}
...
pub fn apply_permission_fix_plan_v17(
    ...
    patch.push_str("# OCP v0.17 permission fix patch (advisory-only)\n");
    patch.push_str("# This file does not auto-apply permissions.\n");
    patch.push_str("# Review and patch Ocp.toml manually.\n");
