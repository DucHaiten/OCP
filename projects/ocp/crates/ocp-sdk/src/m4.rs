...
pub fn load_component_catalog(registry_root: &Path) -> Result<Vec<ComponentSpecV1>, SdkError> {
    ...
    for entry in fs::read_dir(&components_dir)? {
        ...
        let template_rel = kv
            .get("template")
            .cloned()
            .unwrap_or_else(|| format!("templates/{stem}.ocp"));
...
pub fn compose_phenotype(
    ...
    let module_path = generated_dir.join("mod.ocp");
    ...
    for spec in &selected {
        let file_name = format!("{}.ocp", sanitize_name(&spec.name));
...
pub fn verify_assembly(
    ...
    let generated_files = collect_generated_source_files(&generated_dir)?;
...
fn write_compose_cache_marker(
    ...
    let cache_dir = project_root.join("target").join("ocp").join("composer");
...
fn collect_generated_source_files(generated_dir: &Path) -> Result<Vec<PathBuf>, SdkError> {
    ...
    for entry in fs::read_dir(generated_dir)? {
        ...
        if ext.eq_ignore_ascii_case("ocp") || ext.eq_ignore_ascii_case("ocp") {
            files.push(path);
        }
    }
    files.sort();
