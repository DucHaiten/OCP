...
fn w9_manifest_has_10_scenarios_in_order() {
    let root = repo_root();
    let manifest_path = root.join("projects/ocp/conformance/conformance.v1.toml");
    let manifest = parse_conformance_manifest_v1(&manifest_path).expect("parse conformance");

    assert_eq!(manifest.schema, "ocp.conformance.v1");
...
fn w9_conformance_runner_locked_dual_pass() {
    ...
    assert_eq!(report.schema, "ocp.conformance.v1");
...
fn w9_default_manifest_selector_supports_v1_v5() {
    ...
    assert!(v1.ends_with("projects/ocp/conformance/conformance.v1.toml"));
    assert!(v5.ends_with("projects/ocp/conformance/conformance.v5.toml"));
}

#[test]
fn w9_v5_manifest_expected_fail_contract_passes() {
    let root = repo_root();
    let manifest_path = root.join("projects/ocp/conformance/conformance.v5.toml");
    ...
    assert_eq!(report.schema, "ocp.conformance.manifest.v5");
    assert_eq!(report.scenarios_failed, 0);
