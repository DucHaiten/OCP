...
use ocp_sdk::{compose_phenotype, init_project, verify_assembly};
...
fn temp_project_dir(tag: &str) -> PathBuf {
    let stamp = SystemTime::now()
        ...
        .expect("clock drift")
        .as_millis();
    std::env::temp_dir().join(format!("ocp_m4_{tag}_{stamp}"))
}
...
fn m4_verify_fails_when_generated_mutated() {
    ...
    let generated_mod = root.join("src").join("generated").join("mod.ocp");
    fs::write(
        generated_mod,
