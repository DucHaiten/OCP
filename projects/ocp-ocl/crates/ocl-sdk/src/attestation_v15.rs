...
fn build_manifest_value_v15(
    ...
    let cache_mode = match std::env::var("OCL_DISABLE_CACHE") {
        Ok(value)
            if value.trim() == "1"
    ...
    runtime.insert(
        ...
        JsonValue::String(
            std::env::var("OCL_GIT_COMMIT").unwrap_or_else(|_| "unknown".to_string()),
        ),
    );
...
fn attestation_artifact_dir_v15(root: &Path) -> PathBuf {
    root.join("target").join("ocl").join("attestation")
}
