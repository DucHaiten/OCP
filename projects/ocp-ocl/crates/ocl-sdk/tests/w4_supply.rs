...
use ocl_sdk::{
    build_oclpkg_with_lock, fetch_artifact, init_project, publish_artifact, run_artifact,
...
fn temp_project_dir(tag: &str) -> PathBuf {
    let stamp = SystemTime::now()
        ...
        .expect("clock drift")
        .as_millis();
    std::env::temp_dir().join(format!("ocl_w4_{tag}_{stamp}"))
}
...
fn w4_build_verify_publish_fetch_run_artifact_pass() {
    ...
    let build = build_oclpkg_with_lock(&root, true).expect("build oclpkg");
...
fn w4_locked_fails_when_dep_signature_missing() {
    ...
    let err = build_oclpkg_with_lock(&root, true).expect_err("locked build should fail");
