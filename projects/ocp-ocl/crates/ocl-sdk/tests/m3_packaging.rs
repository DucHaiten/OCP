...
use ocl_sdk::{build_project_with_lock, check_project_with_lock, init_project, sync_deps_lock_v1};
...
fn temp_project_dir(tag: &str) -> PathBuf {
    let stamp = SystemTime::now()
        ...
        .expect("clock drift")
        .as_millis();
    std::env::temp_dir().join(format!("ocl_m3_{tag}_{stamp}"))
}
...
fn m3_build_locked_manifest_contains_dep_entries() {
    ...
    let manifest = fs::read_to_string(root.join(".oclbundle").join("manifest.txt"))
