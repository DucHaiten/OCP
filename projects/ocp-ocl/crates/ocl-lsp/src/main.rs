...
const SERVER_NAME: &str = "ocl-lsp";
const SERVER_VERSION: &str = "1.0.0";
const FORMATTING_CONTRACT_JSON: &str = r#"{
  "engine": "ocl_fmt_shared",
  "options": {
    "indent_size": 2,
    "line_width": 100
  }
...
fn rename_result(state: &LspState, params: &JsonValue) -> JsonValue {
    ...
    let preview = lsp_rename_preview_v19(&source, file_id_from_uri(&uri), &symbol, new_name)
        .unwrap_or_else(|_| ocl_sdk::EditorRenamePreviewV19 {
            edits: Vec::new(),
            replaced_count: 0,
        });
    let edits = preview
...
fn code_action_result() -> JsonValue {
    JsonValue::Array(vec![
        code_action(
            "Permission Fix Plan",
            "quickfix",
            "ocpOcl.codeAction.permissionFixPlan",
            "ocpOcl.openFixPlan",
        ),
        code_action(
            "Open Diff Report",
            "quickfix",
            "ocpOcl.codeAction.openDiff",
        ...
        code_action(
            "Open Budget Analyze",
            "quickfix",
            "ocpOcl.codeAction.openBudgetAnalyze",
            "ocpOcl.checkWorkspace",
        ),
        code_action(
            "Open Cassette Report",
            "quickfix",
            "ocpOcl.codeAction.openCassetteReport",
            "ocpOcl.openDebugTrace",
        ),
        code_action(
            "Open Contract Mismatch",
            "quickfix",
            "ocpOcl.codeAction.openContractMismatch",
            "ocpOcl.openDoctorReport",
        ),
    ])
}
