...
const SERVER_NAME: &str = "ocl-dap";
const SERVER_VERSION: &str = "1.0.0";
const DEBUG_CONTRACT_JSON: &str = r#"{
  "debug_mode": "dap_replay_backed",
  "trace_acquisition": "generate_on_launch",
  "trace_artifact_path_scheme": ".ocl_artifacts/editor_dbg/<workspace_hash>/<run_id>/",
...
fn current_stack_frames(state: &DapState) -> Vec<JsonValue> {
    ...
    vec![json!({
        ...
        "source": {
            "name": Path::new(path).file_name().map(|name| name.to_string_lossy().to_string()).unwrap_or_else(|| "main.ocl".to_string()),
            "path": path
        },
        "line": event.line,
        "column": event.column
...
fn write_trace_artifacts(
    ...
    summary: &ocl_sdk::EditorDapLaunchSummaryV19,
