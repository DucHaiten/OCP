...
fn temp_project_dir(tag: &str) -> PathBuf {
    let stamp = SystemTime::now()
        ...
        .expect("clock drift")
        .as_millis();
    std::env::temp_dir().join(format!("ocl_cli_shadow_preview_v12_{tag}_{stamp}"))
}
...
fn run_template_program(root: &Path) -> (i64, i64) {
    let src =
        fs::read_to_string(root.join("src").join("main.ocl")).expect("read template main.ocl");
...
fn cli_shadow_preview_template_v12_run_replay_and_kpi_pass() {
    ...
    let init = run_ocl_cli(&["init", &root_str, "--template", "shadow-preview"]);
    assert!(
        init.status.success(),
        "init failed:\nstdout={}\nstderr={}",
    ...
    let main_src =
        fs::read_to_string(root.join("src").join("main.ocl")).expect("read generated main.ocl");
    assert!(
    ...
    let _memo_off = EnvVarGuard::set("OCL_STD_SHADOW_MEMOIZE_DETERMINISTIC_OBSERVE", "0");
    let _reuse_off = EnvVarGuard::set("OCL_STD_SHADOW_CHECKPOINT_REUSE", "0");
    let (baseline_steps_executed, baseline_steps_charged) = run_template_program(&root);
    drop(_reuse_off);
    drop(_memo_off);

    let _memo_on = EnvVarGuard::set("OCL_STD_SHADOW_MEMOIZE_DETERMINISTIC_OBSERVE", "1");
    let _reuse_on = EnvVarGuard::set("OCL_STD_SHADOW_CHECKPOINT_REUSE", "1");
