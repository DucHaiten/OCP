...
fn temp_project_dir(tag: &str) -> PathBuf {
    let stamp = SystemTime::now()
        ...
        .expect("clock drift")
        .as_millis();
    std::env::temp_dir().join(format!("ocp_v15_attestation_{tag}_{stamp}"))
}
...
fn v15_build_attest_and_verify_repro_pass() {
    ...
    let verify_attest = run_ocp_cli(&["verify", "--attest", &artifact_dir_s], &envs);
    assert_ok(&verify_attest, "verify --attest");

    let verify_repro = run_ocp_cli(&["verify", "--repro", &artifact_dir_s], &envs);
