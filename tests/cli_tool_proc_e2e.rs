...
fn temp_project_dir(tag: &str) -> PathBuf {
    let stamp = SystemTime::now()
        ...
        .expect("clock drift")
        .as_millis();
    std::env::temp_dir().join(format!("ocp_cli_tool_proc_e2e_{tag}_{stamp}"))
}
...
#[test]
fn cli_tool_proc_template_run_record_replay_pass() {
    let root = temp_project_dir("record_replay");
    let root_str = root.to_string_lossy().to_string();

    let init = run_ocp_cli(&["init", &root_str, "--template", "tool-proc"], None);
    assert!(
        init.status.success(),
        "init failed:\nstdout={}\nstderr={}",
    ...
    assert!(
        root.join("out").join("proc.txt").exists(),
        "template should materialize out/proc.txt"
    );
    ...
    assert!(
        ...
        "cassette must include proc entry"
    );
    assert!(
        cassette.contains("\"call_id\":"),
        "cassette proc entry must include call_id"
    );
