...
fn v18_pack_build_sign_verify_enforces_signature_chain() {
    ...
    let init = common::run_ocl_cli(&["init", &root_s, "--template", "dep-permission"], &envs);
    let _ = common::assert_ok(&init, "init dep-permission");

    let resolve = common::run_ocl_cli(&["deps", "resolve", &root_s], &envs);
    let _ = common::assert_ok(&resolve, "deps resolve");

    let build = common::run_ocl_cli(&["pack", "build", &root_s], &envs);
    let _ = common::assert_ok(&build, "pack build");
    let artifact = common::first_oclpkg(&root);
    ...
    let verify_fail = common::run_ocl_cli(&["pack", "verify", &artifact_s], &envs);
    let verify_fail_stderr = common::assert_fail(&verify_fail, "pack verify tampered");

    let sign = common::run_ocl_cli(&["pack", "sign", &artifact_s], &envs);
    let _ = common::assert_ok(&sign, "pack sign");

    let verify_ok = common::run_ocl_cli(&["pack", "verify", &artifact_s], &envs);
