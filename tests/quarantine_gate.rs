...
#[test]
fn quarantine_gate_requires_env_and_writes_replay_metadata() {
    ...
    assert!(
        replay_toml.contains("mode = \"record\""),
        "replay.toml must include record mode"
    );
    assert!(
        replay_toml.contains("cassette_hash = \""),
        "replay.toml must include cassette_hash"
    );
    assert!(
        replay_toml.contains("hasher_version = \"sha256-v1\""),
        "replay.toml must include hasher_version"
    );
    assert!(
        run_dir.join("cassette").join("cassette.jsonl").exists(),
        "missing cassette/cassette.jsonl"
    );
    assert!(
        run_dir
            .join("cassette")
            ...
            .exists(),
        "missing cassette/cassette_index.json"
    );
    assert!(
        run_dir.join("cassette").join("cassette_meta.toml").exists(),
        "missing cassette/cassette_meta.toml"
    );
    assert!(
        run_dir.join("cassette").join("cassette_hash.txt").exists(),
