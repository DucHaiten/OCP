use ocp_sdk::{dap_trace_path_v19, debug_contract_profile_v19};
use serde_json::json;
...
fn v19_dap_trace_acquisition_scheme_is_deterministic() {
    ...
    assert!(
        a.starts_with(".ocp_artifacts/editor_dbg/workspace_a/run_1"),
        "trace path must follow contract scheme"
    );

    let report = json!({
        "schema": "ocp.w19.debug.dap_trace_acquisition_report.v1",
        "status": "PASS",
