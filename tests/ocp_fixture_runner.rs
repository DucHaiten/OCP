...
use ocp::ocp::{run_fixture_file, run_fixture_source, ExecConfig, FixtureRunnerError};
...
fn fixture_runner_runs_from_file() {
    ...
    p.push("tests/fixtures/v1/exec_ok/determinism_basic.ocp");
...
fn fixture_runner_io_error_is_reported() {
    let mut p = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    p.push("tests/fixtures/v1/exec_ok/does_not_exist.ocp");
