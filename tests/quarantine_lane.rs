...
fn temp_project_dir(tag: &str) -> PathBuf {
    let stamp = SystemTime::now()
        ...
        .expect("clock drift")
        .as_millis();
    std::env::temp_dir().join(format!("ocl_cli_quarantine_lane_{tag}_{stamp}"))
}
...
fn locked_lane_works_without_quarantine_env() {
    ...
    let run = run_ocl_cli(&["run", &root_s], None);
    assert!(
        run.status.success(),
    ...
    let run_dir = latest_artifact_dir(&root);
    let replay = run_ocl_cli(&["replay", &run_dir.to_string_lossy()], None);
    assert!(
