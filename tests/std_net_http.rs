...
fn temp_project_dir(tag: &str) -> PathBuf {
    let stamp = SystemTime::now()
        ...
        .expect("clock drift")
        .as_millis();
    std::env::temp_dir().join(format!("ocl_cli_std_net_http_{tag}_{stamp}"))
}
...
fn std_net_http_denies_host_outside_allowlist() {
    ...
    let _hosts = EnvVarGuard::set("OCL_STD_NET_ALLOW_HOSTS", "allowed.example");
    let _methods = EnvVarGuard::set("OCL_STD_NET_ALLOW_METHODS", "GET");
    let _timeout = EnvVarGuard::set("OCL_STD_NET_TIMEOUT_MS", "3000");
    let _max_body = EnvVarGuard::set("OCL_STD_NET_MAX_BODY_BYTES", "1024");
    let _record = EnvVarGuard::set(
        "OCL_V08_HTTP_RECORD_PATH",
        std::env::temp_dir()
            .join("ocl_std_net_http_record_host.tmp")
            .to_string_lossy()
            .as_ref(),
    );
...
fn std_net_http_denies_method_outside_allowlist() {
    ...
    let _lane = EnvVarGuard::set("OCL_PROJECT_LANE", "quarantine");
    let _mode = EnvVarGuard::set("OCL_QUARANTINE_MODE", "record");
    let _hosts = EnvVarGuard::set("OCL_STD_NET_ALLOW_HOSTS", "127.0.0.1");
    let _methods = EnvVarGuard::set("OCL_STD_NET_ALLOW_METHODS", "GET");
    let _timeout = EnvVarGuard::set("OCL_STD_NET_TIMEOUT_MS", "3000");
    let _max_body = EnvVarGuard::set("OCL_STD_NET_MAX_BODY_BYTES", "1024");
    let _record = EnvVarGuard::set(
        "OCL_V08_HTTP_RECORD_PATH",
        std::env::temp_dir()
            .join("ocl_std_net_http_record_method.tmp")
            .to_string_lossy()
            .as_ref(),
    );
