# OCP v0.18 — v1.0 Shipproof Lock + Migration + Release Packaging (Final Pre-Release Hard Freeze)
...
3) **Operability chuẩn**: `ocp perm doctor`/`ocp perm fix --plan|--apply`/`ocp budget analyze`/`ocp cassette ...` thành workflow chuẩn, không còn “kẹt”.
...
- Canonical commands:
  - `ocp perm doctor`
  - `ocp perm fix --plan/--apply`
  - `ocp budget analyze`
  - `ocp cassette ...`
- Alias compatibility (additive, bắt buộc trong v0.18, không được có semantics riêng):
  - `ocp doctor` -> `ocp perm doctor`
  - `ocp fix` -> `ocp perm fix`
- `ocp perm fix --plan/--apply` chỉ tạo patch + records, không bypass strict lane.
- `ocp budget analyze` xuất budget path machine-readable theo semantics edge model.
...
- Source-of-truth tổng hợp RL-1..RL-17:
  - `target/ocp/w18/rc/rl_matrix_report.json`
- Schema tối thiểu:
...
- `target/ocp/w18/contracts/sot_alias_report.json`
- `target/ocp/w18/meta/run_manifest.json`
...
- Alias compatibility bắt buộc (additive, không thêm semantics mới):
  - `ocp doctor` -> `ocp perm doctor`
  - `ocp fix` -> `ocp perm fix`
- `ocp perm doctor` sinh report: permissions, budget, cassette hygiene, pack trust, platform profile, reproducible env checks.
- `ocp perm fix --plan/--apply` chỉ tạo patch + records, không bypass strict lane.
- `ocp budget analyze` bắt buộc budget path machine-readable theo edge model.
...
**Scope**
- `ocp pack build/sign/verify` theo pack ABI.
...
**Artifacts**
- `target/ocp/w18/rc/release_artifact_manifest.json`
- `target/ocp/w18/rc/release_artifact_manifest.sig`
- `target/ocp/w18/rc/golden_journey_tool_cli_report.json`
- `target/ocp/w18/rc/golden_journey_connector_report.json`
- `target/ocp/w18/rc/rl_matrix_report.json`
- `target/ocp/w18/rc/release_readiness_report.json`
...
- RL-1..RL-17 đều `PASS` và được tổng hợp vào:
  - `target/ocp/w18/rc/rl_matrix_report.json`
  - `target/ocp/w18/rc/release_readiness_report.json`
- `target/ocp/w18/rc/release_artifact_manifest.sig` verify pass.
...
- Scope:
  - thêm API `ocp-sdk` cho verify/sign v18 contract set.
  - chốt canonical SoT files ở `contracts/*` theo danh sách Gate 18-A.
  - thêm tests v18 cho completeness/signature/alias/platform/serializer và xuất artifacts `target/ocp/w18/contracts/*`.
- Expected tests:
  - `cargo test --test v18_contract_inventory_complete`
  - `cargo test --test v18_sot_signature_verify`
  - `cargo test --test v18_sot_alias_contract`
...
- Implemented:
  ...
  - thêm test suite Gate 18-A (`v18_*`) + sinh artifacts tại `target/ocp/w18/contracts/*`.
...
- Implemented:
  ...
  - các test sinh đầy đủ artifacts dưới `target/ocp/w18/migration/*` với schema lock đã chốt.
...
- Scope:
  - thêm alias bắt buộc `ocp doctor` -> `ocp perm doctor` và `ocp fix` -> `ocp perm fix`.
  - thêm test v18 cho doctor/fix/budget/rubberstamp/alias.
  - sinh artifacts `target/ocp/w18/ops/*` đúng schema lock của Gate 18-C.
- Expected tests:
  - `cargo test --test v18_doctor_all`
  - `cargo test --test v18_fix_plan_apply`
  - `cargo test --test v18_fix_strict_lane_negative`
...
- Implemented:
  - thêm alias CLI top-level:
    - `ocp doctor` forward sang `ocp perm doctor`
    - `ocp fix` forward sang `ocp perm fix`
