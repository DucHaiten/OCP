# OCL v0.9 — Verifiable Contracts: Schema + Typing for ctx/payload/effects (Policy + Correctness)
...
| CLI docs/examples (`ocl doc packs`, templates) | `C` | additive | `tests/cli_doc_packs.rs` | `9-G` |
...
- CLI can offer `ocl fix ctx` (optional) to convert trivial cases.
...
- Commands run:
  - `cargo test --test type_ctx_schema`
  - `cargo test --test ocl_parser --test ocl_typecheck --test ocl_exec`
  - `cargo test --test type_ctx_schema --test ocl_diag --test ocl_typecheck --test ocl_exec`
  - `cargo test`
  - `cargo fmt`
  - `cargo fmt -- --check`
  - `cargo test --test type_ctx_schema --test ocl_typecheck --test ocl_exec`
- Test results:
  - Targeted tests (must-pass for gate):
...
- Expected tests:
  - `cargo test --test type_effects`
  - `cargo test --test ocl_typecheck --test ocl_commit_policy --test ocl_exec`
  - `cargo test`
  - `cargo fmt -- --check`
- Exit criteria:
...
- Implemented:
  ...
  - Tests:
    ...
    - cập nhật `tests/ocl_stdlib.rs` để case `std.db.query_int` commit-forbidden được assert ở typecheck (khớp contract 9-D).
- Files changed:
  - `src/ocp_ocl/registry.rs`
  - `src/ocp_ocl/typecheck.rs`
  - `src/ocp_ocl/diag.rs`
...
- Commands run:
  - `cargo test --test type_effects`
  - `cargo test --test ocl_typecheck --test ocl_commit_policy --test ocl_exec`
  - `cargo test --test type_effects --test ocl_stdlib`
  - `cargo test`
  - `cargo fmt`
  - `cargo fmt -- --check`
- Test results:
...
- Design alignment:
  - `FULL`
- Notes/risks:
  - Có lệch tạm thời ở test net do dùng sai tên env var allowlist; đã sửa theo runtime contract (`OCL_STD_NET_ALLOW_HOSTS`, `OCL_STD_NET_ALLOW_METHODS`, `OCL_STD_NET_TIMEOUT_MS`, `OCL_STD_NET_MAX_BODY_BYTES`) và re-run targeted suites.
...
- Scope:
  ...
  - Nối policy từ `ocl-sdk` vào `ocl-runtime-core` check/run paths.
...
- Expected tests:
  ...
  - `cargo test --test ocl_manifest --test compat_ctx_string --test compat_ctx_extra_fields`
  - `cargo test`
  - `cargo clippy --all-targets -- -D warnings`
  - `cargo fmt --all -- --check`
- Exit criteria:
...
- Scope:
  - Triển khai CLI command `ocl doc packs` (text + `--json`) sinh docs từ `CapabilityRegistry::v1_baseline()`.
...
- Expected tests:
  ...
  - `cargo test`
- Exit criteria:
  - `ocl doc packs` chạy ổn định, output có đủ fields docs theo scope.
  - `ocl doc packs --json` parseable và chứa known keys quan trọng.
...
- Implemented:
  ...
  - CLI `doc packs`:
    - thêm command `ocl doc packs [--json]`.
...
- Design alignment:
  - `FULL`
- Notes/risks:
  - `ocl doc packs` hiện sinh docs từ baseline registry của runtime; nếu registry schema đổi ở các phiên bản sau thì output docs sẽ đổi tương ứng.
