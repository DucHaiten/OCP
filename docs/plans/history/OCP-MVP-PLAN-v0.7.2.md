# OCP v0.7.2 — Expansion Plan (Tool-grade Packs + Real File/Store + Deterministic IO Harness)
...
Mục tiêu v0.7.2: biến OCP thành “làm tool phổ thông được” bằng cách ship các **packs có IO thực dụng nhưng vẫn an toàn**:
...
|---|---|---|---|---|
| Registry/manifest mở rộng cho fs/kv/time | `S` | `additive` | `tests/ocp_manifest_fs_kv_time.rs` | `7.2-A` |
...
| CLI template `tool-cli` + `ocp test` IO harness | `C` | `additive` | `tests/cli_tool_cli_e2e.rs` | `7.2-F` |
...
- Script OCP mục tiêu: **≤ 250–400 LOC** (không tính packs).
- Project tạo bởi `ocp init tool-cli`: **≤ 6 file** (bao gồm 1–2 fixtures).

**KPI-2: Permission DX**
- Thiếu quyền read/write/kv => error có hint sửa `ocp.toml` trong **≤ 2 vòng**.

**KPI-3: Deterministic IO testing**
- `ocp test` chạy suite fixtures IO => signature ổn định giữa runs.
...
- Mở schema `ocp.toml` cho fs/kv/time permissions:
...
- storage file: `./.ocp_state/kv.json` (locked lane)
...
### 8.1 `ocp.toml` schema extensions
- `[permissions.std_fs] ...`
- `[permissions.std_kv] ...`
- `[permissions.std_time] ...`
...
- creates clean sandbox root:
  ...
  - runs `ocp run` with deterministic seed
  - captures artifacts
  - compares:
    - signature (required)
...
- artifacts additive bắt buộc:
  - `.ocp_artifacts/<run_id>/io/fixtures_manifest.json` (mỗi file gồm `path`, `sha256`)
  - `.ocp_artifacts/<run_id>/state/kv_start.json`
...
- optional debug artifact:
  - `.ocp_artifacts/<run_id>/state/kv_end.json`
...
### 10.1 New templates
- `ocp init tool-cli <dir>`
  - creates:
    - `ocp.toml` (std_fs/std_kv/std_time perms)
    - `main.ocp`
    - `README.md`
    - `fixtures/in/sample.json` (1 file)
    - `fixtures/expected/out.json` (1 file)

### 10.2 `ocp test` enhancements
...
Tests:
- `tests/ocp_manifest_fs_kv_time.rs`
Command:
- `cargo test --test ocp_manifest_fs_kv_time`
Artifacts:
- test report `ocp_manifest_fs_kv_time` pass
Pass/Fail:
...
Scope:
- `ocp init tool-cli`
- `ocp test` fixture harness for IO
...
- Implemented:
  ...
  - Thêm test suite gate:
    - `tests/ocp_manifest_fs_kv_time.rs` (5 tests).
- Files changed:
  - `projects/ocp/crates/ocp-sdk/src/lib.rs`
  - `tests/ocp_manifest_fs_kv_time.rs`
  - `OCP-MVP-PLAN-v0.7.2.md`
- Commands run:
  - `cargo test --test ocp_manifest_fs_kv_time`
  - `cargo test --test ocp_manifest`
  - `cargo test`
...
- Scope:
  ...
  - Storage file mặc định `./.ocp_state/kv.json`, atomic write (temp-file + rename), có thể override bằng env `OCP_STD_KV_PATH` cho test.
  - Bổ sung test targeted `tests/std_kv.rs`.
- Expected tests:
  - `cargo test --test std_kv`
  - `cargo test --test std_kv --test std_time --test ocp_stdlib`
- Exit criteria:
...
- Test results:
  ...
  - Kết luận gate:
    - `DONE` (targeted tests pass; quality gate đã PASS sau lần chốt lại).
- Notes/risks:
  - v0.7.2-C runtime hiện lấy kv caps từ runtime defaults/env (`OCP_STD_KV_MAX_KEYS`, `OCP_STD_KV_MAX_VALUE_BYTES`); wiring trực tiếp từ manifest `permissions.std_kv.max_*` vào runtime sẽ tiếp tục siết ở gate sau.
...
- Expected tests:
  - `cargo test --test std_fs_observe`
  - `cargo test --test std_fs_observe --test std_kv --test std_time --test ocp_stdlib`
- Exit criteria:
...
- Implemented:
  ...
  - Thêm fs helpers:
    - root/caps/allowlist từ env harness (`OCP_STD_FS_*`),
...
- Test results:
  ...
  - Kết luận gate:
    - `DONE` (targeted tests pass; quality gate đã PASS sau lần chốt lại).
- Notes/risks:
  - Gate D runtime sử dụng allowlist/cap từ deterministic harness env (`OCP_STD_FS_ALLOW_*`, `OCP_STD_FS_MAX_*`, `OCP_STD_FS_ROOT`) để enforce path-level policy tại runtime.
...
- Implemented:
  ...
  - Bổ sung helper:
    - `fs_max_write_bytes` (`OCP_STD_FS_MAX_WRITE_BYTES`, default `1_048_576`).
...
- Notes/risks:
  - Gate E hiện lấy fs caps/allowlists từ deterministic env harness (`OCP_STD_FS_*`) tại runtime để đảm bảo fail-closed.
...
- Gate/Step:
  - 7.2-F
- Why:
  - Hoàn tất mặt CLI cho v0.7.2 để người dùng có template `tool-cli`, chạy `ocp test` với golden compare, và có replay IO metadata machine-checkable.
- Scope:
  - Mở rộng `ocp init` với `--template tool-cli`.
  - Mở rộng `ocp test` với `--golden <dir>` và `--clean`.
...
- Expected tests:
  ...
  - `cargo test`
- Exit criteria:
  - `ocp init --template tool-cli` tạo đủ template files theo gate.
  - `ocp test --golden fixtures/expected --clean` pass/fail đúng theo golden content.
...
- Implemented:
  - Mở rộng CLI `init`:
    ...
    - thêm template `tool-cli` với scaffold:
      - `Ocp.toml` (permissions `std_fs/std_kv/std_time`),
      - `src/main.ocp`,
      - `README.md`,
  ...
  - Mở rộng CLI `test`:
    ...
    - thêm `--clean` để dọn `out/` và `.ocp_artifacts/` trước test run.
  ...
  - Bổ sung replay IO metadata:
    - ghi `.ocp_artifacts/<run_id>/io/fixtures_manifest.json` với `path` + `sha256`.
    - ghi `.ocp_artifacts/<run_id>/state/kv_start.json` (snapshot toàn bộ `kv.json`, fallback `{}`).
...
- Test results:
  ...
  - Regression tests (supporting only):
    - PASS: `cli_e2e` 2/2.
    - PASS: `ocp-cli` unit tests 33/33.
    - PASS: full workspace `cargo test`.
    - PASS: `cargo fmt -- --check`.
  - Kết luận gate:
    - `DONE` (targeted tests pass và quality gates pass).
- Notes/risks:
  - Fixture harness của `ocp test` ở gate này dùng deterministic transform cố định (`fixtures/in/sample.json` -> `out/out.json`) để khóa contract golden/replay cho template `tool-cli`.
