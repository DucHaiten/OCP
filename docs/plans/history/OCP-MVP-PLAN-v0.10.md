# OCP v0.10 — Packaging + Lockfile + Permission Manifests per Package (Supply-chain Controlled Capabilities)
...
- Mục tiêu phiên bản:
  - Đóng gói package OCP + lockfile + per-package permission manifests theo hướng supply-chain controlled capabilities.
...
#### In-scope bắt buộc
- Package format `package.ocpp` + canonical archive/content hash deterministic.
...
- CLI `ocp deps *` + `ocp pack *` đủ để build/verify flow v0.10.
...
- Native code pack distribution ngoài phạm vi OCP-only.
...
### 1.1 North Star
- Dự án OCP có thể khai báo dependencies (packages) và dùng ngay modules/engines/packs mà không copy code.
...
- `ocp build` + `ocp run` với lockfile => cùng dependency graph, cùng hashes.
...
- Mọi ví dụ `ocp.toml` trong v0.10 phải dùng literal canonical ở trên.
...
### 3.1 Package types
- **lib package**: cung cấp modules OCP thuần (pure hoặc capability-using).
- **engine package**: cung cấp engine modules (ui/game/shadow wrappers).
- **pack package**: cung cấp capability packs (thường đi kèm host runtime adapters; trong phạm vi OCP-only, pack package có thể là “bundle spec” + “enabled in runtime”, implement host adapters vẫn ở core distribution).
- **template package** (optional): cung cấp project scaffold.

### 3.2 Two-layer permission model
- **Package requested permissions**: `package.ocpp` declare needs.
- **Project granted permissions**: `ocp.toml` decide what is granted.
...
### 4.1 Package manifest file: `package.ocpp` (OCP Package)
Example:
...
- Text/binary detection deterministic:
  - extension allowlist text: `.ocp`, `.md`, `.toml`, `.json`, `.yaml`, `.yml`, `.txt`
...
### 5.1 Project manifest (`ocp.toml`) dependencies section
Example:
...
- Writer canonical (`ocp fmt`, `ocp deps resolve`, `ocp deps update`) chỉ ghi object form.
...
- `ocp.lock` chỉ là export/read-only view cho tooling, không phải SoT.
- Mã runtime/SDK/CLI phải resolve theo `deps.lock.v3`; không resolve từ `ocp.lock`.
...
### 6.3 `ocp.lock` export view
- `ocp.lock` được generate tự động từ `deps.lock.v3` để human review / external tools.
- `ocp.lock` có thể bị xóa/rebuild mà không thay đổi behavior resolver.
...
- Rollback path:
  - `ocp deps resolve --write-legacy-lock` + CI verify `deps.lock.v2` để quay về pipeline cũ.
...
### 6.6 Update workflow
- `ocp deps resolve`:
  - computes graph, writes `deps.lock.v3` (+ optional export `ocp.lock`)
- `ocp deps update <pkg>`:
...
OCP packages carry “requested permissions” as first-class metadata. This is the core differentiator:
...
- error must show:
  ...
  - hint snippet to adjust `ocp.toml` (or explain risk)
...
In OCP-only scope, pack implementations are inside runtime; packages can:
...
### 10.1 New commands
- `ocp deps resolve [--write-legacy-lock]`
- `ocp deps update [pkg]`
- `ocp deps verify`
- `ocp pack build` (build a package archive from local dir)
- `ocp pack sign` (sign package with local key)
- `ocp pack publish` (optional; if no registry, skip)
...
- Canonical hashing/archive builder + provenance runtime model:
  - source of truth: `projects/ocp/crates/ocp-runtime-core`
- CLI commands `ocp deps *`, `ocp pack *`:
...
- write/read `deps.lock.v3` (SoT) + export `ocp.lock` view
- pin hashes
...
- Gate/Step:
  - 10-A
- Why:
  - Khóa parser `package.ocpp` và canonical content hashing trước khi mở các gate resolver/lock, để tránh drift identity/hash ở các bước sau.
- Scope:
  - Thêm parser `package.ocpp` (fallback tương thích `Ocp.toml` cũ).
  - Thêm canonical hashing rule cho text/binary:
    - text: LF normalization,
    - binary: raw bytes.
  - Ghi `content_hash_sha256` vào artifact `.ocppkg`.
...
- Expected tests:
  ...
  - `cargo clippy --all-targets -- -D warnings`
- Exit criteria:
  - Parser `package.ocpp` hoạt động và override identity từ manifest cũ khi cùng tồn tại.
...
- Regression tests (supporting only):
  - `cargo test -p ocp-sdk --test w4_supply`: pass 2/2.
...
- Scope:
  - Thêm model `deps.lock.v3` (read/write), lock hash stable, export `ocp.lock` view.
  ...
  - Nâng CLI: `ocp deps resolve [--write-legacy-lock]` và `ocp lock sync` dùng resolver v3.
...
- Implemented:
  - SDK:
    - thêm lock graph v3 (`LockDepV3`) + parser/writer `deps.lock.v3` + export `ocp.lock`.
    - thêm API Gate B:
      - `resolve_deps_v3(root, write_legacy_lock_v2)`
      - `read_resolved_deps_v3(root)`
      - `verify_deps_lock_v3(root)`
    - parse dependency object form trong `Ocp.toml` (`name/version/source`) + resolve version deterministic cho wildcard (`0.3.* -> 0.3.0`).
    - verify lock cũ (`deps.lock`) được nâng để check mismatch lock v3 khi file v3 tồn tại.
  - CLI:
    - thêm command `ocp deps resolve <project_dir> [--write-legacy-lock]`.
    - `ocp lock sync` chuyển sang resolver v3, có support `--write-legacy-lock`.
...
- Notes/risks:
  - Sự cố tạm thời đã xử lý trong quá trình đóng gate:
    ...
    - auto-generate `deps.lock.v3/ocp.lock` từ `sync_deps_lock_v1/v2` gây lệch regression template count; đã revert để giảm blast radius.
  - Writer v3 hiện đi qua resolver command (`ocp deps resolve` / `ocp lock sync`) thay vì auto-sync toàn bộ flow legacy.
- Design alignment:
  - `FULL`
...
- Scope:
  - Parse `[requested_permissions]` trong `package.ocpp`.
...
- Scope:
  - Mở rộng `ocp deps *` với `update` và `verify`.
  - Mở rộng `ocp pack *` với `build/sign/publish/verify`.
...
- Implemented:
  ...
  - SDK:
    - thêm API `sign_ocppkg(...)` để ký lại artifact `.ocppkg` theo payload hash canonical.
  - Guardrails override:
    - `deps verify` chặn `permission_overrides` nếu thiếu `[security].allow_overrides=true`,
    - yêu cầu env `OCP_ALLOW_OVERRIDES=1` khi có override.
  - Template:
    - thêm `dep-permission` vào `ocp init --template ...` để scaffold dependency + requested_permissions + project permission baseline.
  - Bổ sung test targeted:
...
- Commands run:
  - `cargo test --test cli_deps_e2e --test cli_deps_override_guardrails`
  - `cargo clippy --all-targets -- -D warnings`
  - `rustfmt projects/ocp/crates/ocp-cli/src/main.rs projects/ocp/crates/ocp-sdk/src/lib.rs tests/cli_deps_e2e.rs tests/cli_deps_override_guardrails.rs`
  - `rustfmt --check projects/ocp/crates/ocp-cli/src/main.rs projects/ocp/crates/ocp-sdk/src/lib.rs tests/cli_deps_e2e.rs tests/cli_deps_override_guardrails.rs`
  - `cargo fmt -- --check`
  - `cargo test`
- Test results:
- Targeted tests (must-pass for gate):
