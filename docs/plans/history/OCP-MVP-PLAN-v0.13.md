# OCP v0.13 — Performance via IR + Reproducible Caching (Determinism-preserving)
...
- CLI `ocp cache stats|clean` + benchmark protocol machine-checkable.
...
### 1.1 North Star
- OCP chạy nhanh hơn đáng kể trong các template (tool-ui, mini-game, shadow-preview).
...
- Signature contract (LOCKED):
  ...
  - Cache telemetry tách riêng sang `./.ocp_artifacts/<run_id>/perf_cache.jsonl`.
...
- CLI commands `ocp cache stats|clean` + benchmark harness entry:
  - crate: `ocp-cli`
...
### 5.4 CLI controls
- `ocp build` default uses cache
- `ocp build --no-cache`
- `ocp cache clean`
- `ocp cache stats`
- CLI compatibility contract:
  - `ocp cache` là namespace additive, không đổi behavior lệnh cũ.
  - `ocp cache stats` chỉ đọc, không mutate state.
  - `ocp cache clean` yêu cầu `--yes` để tránh xóa nhầm và làm lệch benchmark.
...
- Allow-list v0.13:
  - `OCP_CACHE_DISABLE` (`0|1`)
  - `OCP_CACHE_PROFILE` (`default|strict`)
- Với toggles ngoài allow-list:
  - locked lane: force `no-cache` cho run hiện tại + emit audit warning + ghi marker vào artifacts.
- Legacy shadow toggles từ v0.12 (`OCP_STD_SHADOW_CHECKPOINT_REUSE`, `OCP_STD_SHADOW_MEMOIZE_DETERMINISTIC_OBSERVE`):
...
- Implemented:
  - Thêm `src/ocp/hir.rs`:
    ...
    - `ir_hash_sha256_v1`.
  - Mở public API qua `src/ocp/mod.rs`.
...
- Implemented:
  ...
  - Mở public API compile cache qua `src/ocp/mod.rs`.
...
- Implemented:
  ...
  - Cập nhật initializer `ExecConfig` trong `projects/ocp/crates/ocp-sdk/src/lib.rs` để tương thích field mới.
- Files changed:
  - `src/ocp/budget.rs`
  - `src/ocp/exec.rs`
  - `src/ocp/mod.rs`
...
- Scope:
  ...
  - Thêm targeted tests `tests/observe_cache.rs` và cập nhật `tests/ocp_registry.rs`.
- Expected tests:
  - `cargo test --test observe_cache --test ocp_registry`
- Exit criteria:
...
- Implemented:
  ...
  - Thêm targeted tests:
    ...
    - `tests/ocp_registry.rs`: kiểm tra std keys có metadata explicit + kỳ vọng determinism/cacheability mẫu.
- Files changed:
  - `src/ocp/registry.rs`
  - `src/ocp/exec.rs`
  - `src/ocp/mod.rs`
...
- Test results:
  - Targeted tests (must-pass for gate):
    - `Đạt` (`tests/observe_cache.rs`, `tests/ocp_registry.rs`).
  - Regression tests (supporting only):
    - `Đạt` (`cargo test` toàn repo).
    - `Đạt` (`cargo clippy --all-targets -- -D warnings`).
    - `Đạt` (`cargo fmt -- --check`).
...
- Scope:
  - Thêm CLI `ocp cache stats`, `ocp cache clean --yes`, `ocp cache bench`.
  - Emit/parse `perf_cache.jsonl` tách khỏi signature stream semantic.
  - Bổ sung report benchmark `target/ocp/v13/cache_benchmark.json`.
...
- Expected tests:
  ...
  - `cargo fmt -- --check`
- Exit criteria:
  - `ocp cache stats` trả counters machine-checkable (`--json`).
  - `ocp cache clean` bắt buộc `--yes`, không cho xóa nhầm.
  - `ocp cache bench` sinh report benchmark và xác nhận signature equivalence cache on/off.
...
- Gate/Step: 13-F
- Implemented:
  - Bổ sung CLI namespace `ocp cache`:
    - `stats`: tổng hợp compile/exec/observe counters từ `.ocp_artifacts/*/perf_cache.jsonl` và compile cache stores.
    - `clean`: yêu cầu `--yes`, dọn `.ocp_artifacts`, `.ocp_cache`, `target/ocp/cache`.
  ...
  - Mở rộng env `OCP_CACHE_DISABLE` để vô hiệu hóa cả observe cache và exec cache trong benchmark no-cache mode.
