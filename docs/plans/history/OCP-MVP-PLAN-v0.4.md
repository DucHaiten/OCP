...
Ngày tạo: 2026-02-25  
Mục tiêu: đưa OCP từ MVP ngôn ngữ thành platform hoàn chỉnh để xây ứng dụng đa dụng (compute + service), có deploy artifact chuẩn, supply-chain an toàn, tooling vận hành thực tế, và mở rộng được qua plugin ABI.
...
4. No naked IO: không syscall trực tiếp từ OCP.
...
1. Artifact chuẩn: `.ocppkg` single-file.
2. Mặc định `ocp build` xuất `.ocppkg`.
3. `--source-only` xuất `.ocpbundle` để compat.
...
### 1.13 Testing (LOCK)
1. `ocp test` là runner chính thức/source-of-truth.
...
### 7.1 Trace
1. `ocp trace run ...` sinh `trace.jsonl`.
...
1. `ocp fmt`
2. `ocp check --locked`
3. `ocp test --locked --seed ...`
4. nếu có phenotype: `ocp compose --locked` + `ocp verify --locked`
5. `ocp build --locked` -> `.ocppkg`
6. `ocp run <artifact>`

### 13.2 Deploy loop
1. `.ocppkg` là deploy unit duy nhất.
...
### W0 — Stabilize baseline (entry gate, OCP-only rebaseline)
1. Active surface chi con `projects/ocp/**`.
2. Legacy root runtime/test/docs/scripts duoc archive vao `/_archive/legacy-root-pre-v0.4/**` va mac dinh read-only.
3. Boundary guard dung merge-base semantics + root allowlist OCP-only.
...
3. Deploy: `.ocppkg` chạy offline, reproducible, có proofs.
...
10. Conformance: `ocp test --conformance --locked` xanh toàn bộ.
...
- W0 Stabilize baseline + rebaseline OCP-only: `DONE`
...
- Gate/Step:
  - W0 (re-open)
- Why:
  - Re-open W0 theo yêu cầu triển khai lại từ trạng thái chưa done, dùng lại phạm vi W0 đã khóa để kiểm chứng baseline OCP-only.
- Scope:
...
- Implemented:
  ...
  - Sửa `tools/ci_w0_entry.ps1` để chạy đúng test mục tiêu `m5_conformance` ở package `ocp-sdk`.
...
- Test results:
  - PASS:
    ...
    - `cargo clippy --workspace --all-targets -- -D warnings`
    - `tools/ci_ocp_lane.ps1`
    - `cargo test -p ocp-sdk --test m5_conformance` (4/4)
    - `tools/ci_w0_entry.ps1`
    - `tools/ci_ocp_quarantine.ps1` (report: `target/ocp/quarantine/report.json`)
    - `cargo metadata --no-deps --format-version 1`
  - FAIL đã xử lý:
    - `cargo test -p ocp-cli --test m5_conformance` (không có test target trong package `ocp-cli`).
- Notes/risks:
  - Checklist W0 hiện đã chạy đúng theo codebase hiện tại; `m5_conformance` thuộc `ocp-sdk`, không phải `ocp-cli`.
  - `ci_ocp_quarantine.ps1` được thiết kế non-blocking (ghi report), không thay đổi dữ liệu ngoài `target/`.
...
- Scope:
  ...
  - Bổ sung guard cấm `std.net.poll` ở user OCP path.
...
- Expected tests:
  - `cargo check --workspace`
  - `cargo test -p ocp-runtime-core --test m1_language`
  ...
  - `cargo fmt -- --check`
  - `cargo run -p ocp-cli -- run projects/ocp/apps/mini-server --locked --reactor --ticks 128 --runtime deterministic --socket-listen 127.0.0.1:19091 --runtime-report target/ocp/w1/miniserver_det.json`
  - `cargo run -p ocp-cli -- run projects/ocp/apps/mini-server --locked --reactor --ticks 128 --runtime throughput --socket-listen 127.0.0.1:19092 --runtime-report target/ocp/w1/miniserver_thr.json`
  - `powershell -ExecutionPolicy Bypass -File tools/ci_ocp_lane.ps1`
- Exit criteria:
...
- Implemented:
  ...
  - Cập nhật registry/type/runtime semantics cho W1:
    ...
    - typecheck reject trực tiếp `std.net.poll` ở user OCP path
  - Cập nhật demo `mini-server`:
    - thêm `[runtime]` budget fields trong `Ocp.toml`
    - thêm flow `std.net.listen` + `std.net.reply` + `commit(reply_res)` trong `src/main.ocp`
  - Bổ sung test W1:
...
- Files changed:
  ...
  - `projects/ocp/crates/ocp-runtime-core/tests/m1_language.rs`
  - `projects/ocp/crates/ocp-sdk/tests/w1_runtime.rs`
  - `tests/ocp_registry.rs`
  - `projects/ocp/apps/mini-server/Ocp.toml`
  - `projects/ocp/apps/mini-server/src/main.ocp`
  - `tools/ci_ocp_lane.ps1`
  - `OCP-MVP-PLAN-v0.4.md`
- Commands run:
  - `cargo check --workspace`
  ...
  - `cargo test -p ocp-cli`
  - `cargo test --test ocp_registry`
  - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings` (lần đầu fail do `clippy::ptr_arg` trong test helper)
  - `cargo fmt` (format các file mới sửa)
  - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings` (sau fix, pass)
  - `cargo fmt -- --check`
  - `cargo run -p ocp-cli -- run projects/ocp/apps/mini-server --locked --reactor --ticks 128 --runtime deterministic --socket-listen 127.0.0.1:19091 --runtime-report target/ocp/w1/miniserver_det.json`
  - `cargo run -p ocp-cli -- run projects/ocp/apps/mini-server --locked --reactor --ticks 128 --runtime throughput --socket-listen 127.0.0.1:19092 --runtime-report target/ocp/w1/miniserver_thr.json`
  - `powershell -ExecutionPolicy Bypass -File tools/ci_ocp_lane.ps1`
...
- Test results:
  - PASS:
    - `cargo check --workspace`
    - `cargo test -p ocp-runtime-core --test m1_language` (2/2)
    - `cargo test -p ocp-sdk --test w1_runtime` (2/2)
    - `cargo test -p ocp-cli` (3/3)
    - `cargo test --test ocp_registry` (8/8)
    - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings` (sau fix)
    - `cargo fmt -- --check`
    - mini-server runtime commands deterministic + throughput đều pass, report file được ghi:
      - `target/ocp/w1/miniserver_det.json`
      - `target/ocp/w1/miniserver_thr.json`
    - `tools/ci_ocp_lane.ps1` pass
    - `cargo test -p ocp-runtime-core -p ocp-sdk -p ocp-cli` pass
  - FAIL đã xử lý:
...
- Notes/risks:
  ...
  - `std.net.poll` đã bị chặn ở user OCP path theo đúng scope lock W1.
...
- Scope:
  - Bổ sung IAM capability enforcement ở `ocp-sdk` cho chế độ `--locked`.
  ...
  - Nối lane CI W2 trong `tools/ci_ocp_lane.ps1`.
- Expected tests:
  - `cargo check --workspace`
  - `cargo test -p ocp-sdk --test w2_permissions`
  - `cargo test -p ocp-sdk --test w2_audit`
  - `cargo test -p ocp-sdk --test m3_packaging`
  - `cargo test -p ocp-cli`
  - `cargo test --test ocp_registry`
  - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings`
  - `cargo fmt -- --check`
...
- Gate/Step:
  - W2 (re-open, code delta)
- Implemented:
  - Thêm mô hình quyền IAM trong `ocp-sdk`:
    - `PermissionRules`, `ProjectPermissions`, `PermissionDecision`.
    - Parser từ `Ocp.toml` cho `[permissions.package]` và `[permissions.module.*]`.
...
- Files changed:
  ...
  - `projects/ocp/crates/ocp-cli/src/main.rs`
  - `projects/ocp/crates/ocp-runtime-core/src/lib.rs`
  - `projects/ocp/crates/ocp-sdk/tests/w2_permissions.rs`
  - `projects/ocp/crates/ocp-sdk/tests/w2_audit.rs`
  - `projects/ocp/crates/ocp-sdk/tests/m3_packaging.rs`
  - `projects/ocp/crates/ocp-sdk/tests/w1_runtime.rs`
  - `projects/ocp/apps/hello-cli/Ocp.toml`
  - `projects/ocp/apps/web-fetch/Ocp.toml`
  - `projects/ocp/apps/scheduler/Ocp.toml`
  - `projects/ocp/apps/composer-demo/Ocp.toml`
  - `projects/ocp/apps/mini-server/Ocp.toml`
  - `tools/ci_ocp_lane.ps1`
  - `OCP-MVP-PLAN-v0.4.md`
- Commands run:
...
- Test results:
  - PASS:
    ...
    - fmt check pass
    - `tools/ci_ocp_lane.ps1` pass
    - package tests (`ocp-runtime-core`, `ocp-sdk`, `ocp-cli`) pass
- Notes/risks:
...
- Scope:
  - Thêm module pipeline trong `ocp-runtime-core` (`ir`, `bytecode`, `source_map`, `vm`).
...
- Expected tests:
  - `cargo test -p ocp-runtime-core --test w3_bytecode`
  - `cargo test -p ocp-sdk --test w3_engine`
  - `cargo test -p ocp-cli`
  - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings`
  - `cargo fmt -- --check`
  - `powershell -ExecutionPolicy Bypass -File tools/ci_ocp_lane.ps1`
- Exit criteria:
  ...
  - `ocp run --engine bytecode|dual` hoạt động.
...
- Gate/Step:
  - W3 (re-open, code delta)
- Implemented:
  - Thêm pipeline W3 trong `ocp-runtime-core`:
  ...
  - Nối CLI:
    - thêm `--engine interpreter|bytecode|dual` cho `ocp run` non-reactor.
    - guard `--engine` không dùng cho reactor path hiện tại.
    - thêm test CLI cho bytecode/dual.
  - Bổ sung test W3:
    - `projects/ocp/crates/ocp-runtime-core/tests/w3_bytecode.rs`
    - `projects/ocp/crates/ocp-sdk/tests/w3_engine.rs`
  - Cập nhật lane:
...
- Files changed:
  ...
  - `projects/ocp/crates/ocp-runtime-core/src/source_map.rs`
...
- Commands run:
  ...
  - `cargo test -p ocp-sdk --test w3_engine`
  - `cargo test -p ocp-cli`
  - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings` (lần đầu fail, lần sau pass)
  - `powershell -ExecutionPolicy Bypass -File tools/ci_ocp_lane.ps1`
- Test results:
  - PASS:
    - `w3_bytecode`: 3/3
...
- Scope:
  - Thêm `.ocppkg` single-file artifact trong SDK.
...
- Expected tests:
  - `cargo test -p ocp-sdk --test w4_supply`
  - `cargo test -p ocp-cli`
  - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings`
  - `cargo fmt -- --check`
  - `powershell -ExecutionPolicy Bypass -File tools/ci_ocp_lane.ps1`
- Exit criteria:
  - Build mặc định xuất `.ocppkg`.
...
- Gate/Step:
  - W4 (re-open, code delta)
- Implemented:
  - Thêm W4 supply-chain API trong `ocp-sdk`:
    - `sync_deps_lock_v2` (đồng thời `sync_deps_lock_v1` giờ ghi thêm `deps.lock.v2`)
    - `build_ocppkg_with_lock` / `build_ocppkg`
  ...
  - Thêm lệnh CLI:
    ...
    - `ocp run <artifact.ocppkg> --engine ...` (offline artifact run)
  - Bổ sung test W4:
    - `projects/ocp/crates/ocp-sdk/tests/w4_supply.rs`
    - test CLI `w4_cli_build_publish_fetch_verify_and_run_artifact_pass` trong `ocp-cli/src/main.rs`
  - Cập nhật lane:
    - `tools/ci_ocp_lane.ps1` thêm `cargo test -p ocp-sdk --test w4_supply`
- Files changed:
  - `projects/ocp/crates/ocp-sdk/src/lib.rs`
  - `projects/ocp/crates/ocp-sdk/tests/w4_supply.rs`
...
- Commands run:
  ...
  - `cargo test -p ocp-cli`
  - `cargo fmt -- --check` (lần đầu fail)
  - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings` (lần đầu fail)
  - `cargo fmt`
  - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings`
  - `powershell -ExecutionPolicy Bypass -File tools/ci_ocp_lane.ps1`
- Test results:
...
- Scope:
  - Thêm dependency crypto thật cho `ocp-sdk`: `blake3`, `ed25519-dalek`, `base64`.
...
- Gate/Step:
  - W4 (crypto completion)
- Implemented:
  - Thêm dependency crypto chuẩn trong `ocp-sdk`:
    - `base64`
    - `blake3`
    - `ed25519-dalek`
...
- Files changed:
  ...
  - `OCP-MVP-PLAN-v0.4.md`
- Commands run:
  - `cargo test -p ocp-sdk --test w4_supply` (lần đầu timeout do sandbox không truy cập `crates.io`)
  - `cargo test -p ocp-sdk --test w4_supply` (rerun ngoài sandbox để tải dependency, pass)
  - `cargo test -p ocp-cli`
  - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings`
  - `cargo fmt`
  - `cargo fmt -- --check`
...
- Expected tests:
  ...
  - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings`
  - `cargo fmt -- --check`
  - `cargo run -p ocp-cli -- run projects/ocp/app-ocp --locked`
  - `cargo run -p ocp-cli -- run projects/ocp/apps/mini-server --locked --reactor --ticks 32`
  - `powershell -ExecutionPolicy Bypass -File tools/ci_ocp_lane.ps1`
- Exit criteria:
...
- Gate/Step:
  - W5
- Implemented:
  - Bổ sung model + API trace/profile trong `ocp-sdk`:
  ...
  - Bổ sung command surface W5 trong `ocp-cli`:
    - `ocp trace run`, `ocp trace view`.
    - `ocp profile run`, `ocp profile view`.
    - hỗ trợ cả non-reactor và reactor path cho `trace/profile run`.
  - Bổ sung test W5 đúng lane trong `ocp-cli`:
...
- Files changed:
  ...
  - `OCP-MVP-PLAN-v0.4.md`
- Commands run:
  - `cargo test -p ocp-cli w5_`
  - `cargo test -p ocp-runtime-core -p ocp-sdk -p ocp-cli`
  - `cargo clippy -p ocp-sdk -p ocp-cli --all-targets -- -D warnings`
  - `cargo fmt`
  - `cargo fmt -- --check`
  - `cargo run -p ocp-cli -- trace run projects/ocp/app-ocp --locked --engine dual --out target/ocp/w5/app.trace.locked.jsonl`
  - `cargo run -p ocp-cli -- profile run projects/ocp/app-ocp --locked --engine dual --out target/ocp/w5/app.profile.locked.json`
  - `cargo run -p ocp-cli -- trace run projects/ocp/app-ocp --engine dual --out target/ocp/w5/app.trace.jsonl`
  - `cargo run -p ocp-cli -- trace view target/ocp/w5/app.trace.jsonl --tail 10 --json`
  - `cargo run -p ocp-cli -- profile run projects/ocp/app-ocp --engine dual --out target/ocp/w5/app.profile.json`
  - `cargo run -p ocp-cli -- profile view target/ocp/w5/app.profile.json --top 10 --json`
  - `cargo run -p ocp-cli -- trace run projects/ocp/apps/mini-server --reactor --ticks 8 --runtime deterministic --engine dual --out target/ocp/w5/mini.det.trace.jsonl`
  - `cargo run -p ocp-cli -- profile run projects/ocp/apps/mini-server --reactor --ticks 8 --runtime deterministic --engine dual --out target/ocp/w5/mini.det.profile.json`
- Test results:
  - PASS:
    - `cargo test -p ocp-cli w5_`: 3/3 pass.
    - `cargo test -p ocp-runtime-core -p ocp-sdk -p ocp-cli`: pass toàn lane.
    - `cargo clippy -p ocp-sdk -p ocp-cli --all-targets -- -D warnings`: pass.
    - `cargo fmt -- --check`: pass.
    - CLI `trace/profile run` cho `app-ocp --locked`: pass.
    - CLI `trace/profile run/view` cho app + reactor mini-server: pass.
- Notes/risks:
  - Trace file W5 lưu theo dòng deterministic (có thể xem lại bằng `ocp trace view --json`), không lộ raw args/payload text; phần nhạy cảm chỉ giữ hash.
...
- Scope:
  ...
  - Bổ sung CLI command `ocp plugin lock sync` và `ocp plugin verify`.
...
- Expected tests:
  - `cargo test -p ocp-cli w6_`
  - `cargo test -p ocp-runtime-core -p ocp-sdk -p ocp-cli`
  - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings`
  - `cargo fmt -- --check`
  - `cargo run -p ocp-cli -- plugin lock sync projects/ocp/apps/plugin-demo`
  - `cargo run -p ocp-cli -- plugin verify projects/ocp/apps/plugin-demo`
  - `cargo run -p ocp-cli -- run projects/ocp/apps/plugin-demo --locked --engine dual`
- Exit criteria:
...
- Implemented:
  ...
  - SDK:
    ...
    - Export W6 APIs qua `ocp-sdk/src/lib.rs`.
    - Thêm enforcement `--locked` cho các path `check/run/test/build/build_ocppkg` và trace/profile run:
      - Nếu có `custom.*` thì bắt buộc có `plugins.lock.v1` hợp lệ và key phải resolve được plugin.
      - Set env runtime plugin (`OCP_PLUGIN_LOCK_PATH`, `OCP_PLUGIN_ROOT`) cho engine path.
  - CLI:
    - Thêm command:
      - `ocp plugin lock sync <project_dir>`
      - `ocp plugin verify <project_dir>`
    - Cập nhật help usage.
    - Bổ sung test W6 trong `ocp-cli/src/main.rs`.
  - Demo app:
    - Thêm `projects/ocp/apps/plugin-demo` gồm:
      - `Ocp.toml`, `src/main.ocp`, `tests/smoke.ocp`
...
- Files changed:
  ...
  - `src/ocp/registry.rs`
  - `src/ocp/exec.rs`
  - `projects/ocp/crates/ocp-sdk/src/w6.rs`
  - `projects/ocp/crates/ocp-sdk/src/lib.rs`
  - `projects/ocp/crates/ocp-cli/src/main.rs`
  - `projects/ocp/apps/plugin-demo/Ocp.toml`
  - `projects/ocp/apps/plugin-demo/src/main.ocp`
  - `projects/ocp/apps/plugin-demo/tests/smoke.ocp`
  - `projects/ocp/apps/plugin-demo/plugins/index.toml`
  - `projects/ocp/apps/plugin-demo/plugins/bin/echo_plugin.ps1`
  - `projects/ocp/apps/plugin-demo/deps.lock`
  - `projects/ocp/apps/plugin-demo/plugins.lock.v1`
  - `OCP-MVP-PLAN-v0.4.md`
- Commands run:
  - `cargo test -p ocp-cli w6_`
  - `cargo test -p ocp-runtime-core -p ocp-sdk -p ocp-cli`
  ...
  - `cargo fmt`
  - `cargo fmt -- --check`
  - `cargo run -p ocp-cli -- lock sync projects/ocp/apps/plugin-demo`
  - `cargo run -p ocp-cli -- plugin lock sync projects/ocp/apps/plugin-demo`
  - `cargo run -p ocp-cli -- plugin verify projects/ocp/apps/plugin-demo`
  - `cargo run -p ocp-cli -- check projects/ocp/apps/plugin-demo --locked --json`
  - `cargo run -p ocp-cli -- run projects/ocp/apps/plugin-demo --locked --engine dual`
  - `cargo run -p ocp-cli -- test projects/ocp/apps/plugin-demo --locked`
  - `cargo run -p ocp-cli -- trace run projects/ocp/apps/plugin-demo --locked --engine dual --out target/ocp/w6/plugin-demo.trace.jsonl`
  - `cargo run -p ocp-cli -- trace view target/ocp/w6/plugin-demo.trace.jsonl --json`
  - `cargo run -p ocp-cli -- profile run projects/ocp/apps/plugin-demo --locked --engine dual --out target/ocp/w6/plugin-demo.profile.json`
- Test results:
  - PASS:
    - `cargo test -p ocp-cli w6_`: 2/2 pass.
    - `cargo test -p ocp-runtime-core -p ocp-sdk -p ocp-cli`: pass toàn lane.
...
- Expected tests:
  ...
  - `cargo test -p ocp-cli w7_`
  - `cargo test -p ocp-runtime-core -p ocp-sdk -p ocp-cli`
  - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings`
  - `cargo fmt -- --check`
  - `cargo run -p ocp-cli -- check projects/ocp/apps/tls-client --locked --json`
  - `cargo run -p ocp-cli -- run projects/ocp/apps/tls-client --locked`
  - `cargo run -p ocp-cli -- check projects/ocp/apps/sqlite-app --locked --json`
  - `cargo run -p ocp-cli -- run projects/ocp/apps/sqlite-app --locked`
  - `cargo run -p ocp-cli -- test projects/ocp/apps/sqlite-app --locked`
  - `cargo run -p ocp-cli -- build projects/ocp/apps/sqlite-app --locked --json`
  - `powershell -ExecutionPolicy Bypass -File tools/ci_ocp_lane.ps1`
  - `powershell -ExecutionPolicy Bypass -File tools/ci_ocp_quarantine.ps1`
- Exit criteria:
...
- Implemented:
  ...
  - Tests:
    - Mở rộng `tests/ocp_stdlib.rs` với test W7 cho ctx-required, TLS payload, DB read/write split, commit policy, UI/Game tick payload.
    - Bổ sung test unit W7 trong `ocp-cli/src/main.rs`:
      - `w7_cli_tls_client_locked_flow_pass`
      - `w7_cli_sqlite_app_locked_flow_pass`
  - Demo apps:
    - Thêm `projects/ocp/apps/tls-client` (manifest, source, smoke test, deps.lock).
    - Thêm `projects/ocp/apps/sqlite-app` (manifest, source, smoke test, deps.lock).
  - CI lane:
    - Cập nhật `tools/ci_ocp_lane.ps1` để thêm hai app W7 vào vòng `lock/check/run/test/build --locked`.
- Files changed:
  - `src/ocp/registry.rs`
  - `src/ocp/exec.rs`
  - `tests/ocp_stdlib.rs`
  - `projects/ocp/crates/ocp-cli/src/main.rs`
  - `tools/ci_ocp_lane.ps1`
  - `projects/ocp/apps/tls-client/Ocp.toml`
  - `projects/ocp/apps/tls-client/src/main.ocp`
  - `projects/ocp/apps/tls-client/tests/smoke.ocp`
  - `projects/ocp/apps/tls-client/deps.lock`
  - `projects/ocp/apps/sqlite-app/Ocp.toml`
  - `projects/ocp/apps/sqlite-app/src/main.ocp`
  - `projects/ocp/apps/sqlite-app/tests/smoke.ocp`
  - `projects/ocp/apps/sqlite-app/deps.lock`
  - `OCP-MVP-PLAN-v0.4.md`
- Commands run:
  - `cargo test --test ocp_stdlib`
  - `cargo test -p ocp-cli w7_`
...
- Test results:
  - PASS:
    - `cargo test --test ocp_stdlib`: 11/11 pass.
    - `cargo test -p ocp-cli w7_`: 2/2 pass.
    - `check/run/test/build --locked` cho `tls-client` và `sqlite-app`: pass.
    - `cargo test -p ocp-runtime-core -p ocp-sdk -p ocp-cli`: pass toàn lane.
    - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings`: pass.
    - `cargo fmt -- --check`: pass.
    - `tools/ci_ocp_lane.ps1`: pass (bao gồm hai app W7 mới).
    - `tools/ci_ocp_quarantine.ps1`: pass.
- Notes/risks:
  - Đã mở local-real adapter path dạng env-gated:
    - `OCP_W7_TLS_LOCAL_REAL=1` cho TLS socket probe path (`std.tls.connect` + `std.tls.handshake`).
    - `OCP_W7_DB_LOCAL_REAL=1` cho DB local-real path dùng SQLite engine thật qua Python `sqlite3` (giữ hợp đồng `std.db.query_int` + commit apply-once của `std.db.exec`).
...
- Scope:
  - Sửa `src/ocp/exec.rs` để bỏ phụ thuộc ngoài không bắt buộc (`rusqlite`, `blake3`) khỏi W7 local-real path.
  ...
  - Sửa test W7 tương ứng trong `tests/ocp_stdlib.rs`.
- Expected tests:
  - `cargo test --test ocp_stdlib`
  - `cargo test -p ocp-cli w7_`
- Exit criteria:
...
- Test results:
  - PASS:
    - `cargo test --test ocp_stdlib`: 13/13 pass.
    - `cargo test -p ocp-cli w7_`: 2/2 pass.
...
- Expected tests:
  ...
  - `cargo run -p ocp-cli -- compose projects/ocp/apps/composer-demo --phenotype projects/ocp/apps/composer-demo/phenotype.toml --registry projects/ocp/apps/composer-demo/registry --locked --json`
  - `cargo run -p ocp-cli -- verify projects/ocp/apps/composer-demo --phenotype projects/ocp/apps/composer-demo/phenotype.toml --registry projects/ocp/apps/composer-demo/registry --locked --json`
  - `powershell -ExecutionPolicy Bypass -File tools/ci_ocp_lane.ps1`
  - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings`
  - `cargo fmt -- --check`
  - `cargo test`
...
- Implemented:
  - Bổ sung code delta W8 tại SDK composer:
    ...
    - Ghi cache marker `target/ocp/composer/cache.v1`.
...
- Test results:
  - PASS:
    - `cargo test -p ocp-sdk --test m4_composer` pass (4/4).
    ...
    - `cargo test` pass.
  - FAIL tạm thời đã xử lý:
    - compose với path `app-ocp/phenotype.toml` fail vì path không tồn tại.
...
- Scope:
  ...
  - Thêm surface `ocp test --conformance` trong CLI với exit code contract W9 (9/10/11/12).
...
- Expected tests:
  ...
  - `cargo fmt -- --check`
- Exit criteria:
  - `ocp test --conformance --locked` pass đủ 10/10.
...
- Implemented:
  ...
  - Export W9 API tại:
    - `projects/ocp/crates/ocp-sdk/src/lib.rs`
  - Mở rộng CLI:
    - thêm mode `ocp test --conformance ...` trong `projects/ocp/crates/ocp-cli/src/main.rs`
    - validate `--runtime`, `--engine`
    - enforce locked signed flow (`--sign-key` hoặc `OCP_SIGN_KEY_PATH`, `--signer-id`)
  ...
  - Thêm artifact W9:
    - `projects/ocp/conformance/conformance.v1.toml` (10 scenario)
    - app mới:
      - `projects/ocp/apps/tcp-jsonl-server-real/**`
      - `projects/ocp/apps/ui-demo/**`
    - security placeholders:
      - `projects/ocp/security/trust.store.toml`
      - `projects/ocp/security/dev-root-1.signing.key.toml`
  - Thêm test W9:
    - `projects/ocp/crates/ocp-sdk/tests/w9_conformance.rs`
    - unit tests W9 trong `projects/ocp/crates/ocp-cli/src/main.rs`:
      - `w9_cli_conformance_requires_sign_key_when_locked`
      - `w9_cli_conformance_single_scenario_pass`
  - Cập nhật lane scripts:
    - `tools/ci_ocp_lane.ps1` thêm blocking W9 conformance step
    - `tools/ci_ocp_quarantine.ps1` thêm throughput smoke cho W9 (`engine=bytecode`)
- Files changed:
  - `projects/ocp/crates/ocp-sdk/src/w9.rs`
  - `projects/ocp/crates/ocp-sdk/src/lib.rs`
  - `projects/ocp/crates/ocp-sdk/tests/w9_conformance.rs`
  - `projects/ocp/crates/ocp-cli/src/main.rs`
  - `projects/ocp/conformance/conformance.v1.toml`
  - `projects/ocp/apps/tcp-jsonl-server-real/Ocp.toml`
  - `projects/ocp/apps/tcp-jsonl-server-real/deps.lock`
  - `projects/ocp/apps/tcp-jsonl-server-real/src/main.ocp`
  - `projects/ocp/apps/tcp-jsonl-server-real/tests/smoke.ocp`
  - `projects/ocp/apps/ui-demo/Ocp.toml`
  - `projects/ocp/apps/ui-demo/deps.lock`
  - `projects/ocp/apps/ui-demo/src/main.ocp`
  - `projects/ocp/apps/ui-demo/tests/smoke.ocp`
  - `projects/ocp/security/trust.store.toml`
  - `projects/ocp/security/dev-root-1.signing.key.toml`
  - `tools/ci_ocp_lane.ps1`
  - `tools/ci_ocp_quarantine.ps1`
...
- Test results:
  - PASS:
    - `cargo test -p ocp-sdk --test w9_conformance` pass (2/2)
    - `cargo test -p ocp-cli w9_cli_` pass (2/2)
    - `ocp test --conformance ...` pass (10/10)
    - `tools/ci_ocp_lane.ps1` pass
    - `cargo clippy ... -D warnings` pass
    - `cargo fmt -- --check` pass
  - FAIL tạm thời đã xử lý:
...
- Notes/risks:
  ...
  - Manifest W9 đang dùng path tương đối từ repo root (`projects/ocp/apps/...`) để ổn định khi chạy từ root.
...
- Test results:
  - PASS:
    ...
    - `cargo fmt -- --check`: PASS.
    - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings`: PASS.
    - `tools/ci_ocp_lane.ps1`: PASS toàn bộ lane e2e.
    - `cargo test -p ocp-sdk --test w9_conformance`: PASS (4/4).
    - `cargo test -p ocp-cli w9_cli_`: PASS (2/2).
    - `ocp test --conformance --locked ...`: PASS (10/10), `required_digest=10deff71c24ef729`.
- Notes/risks:
  - Không phát hiện drift thực thi giữa lock thiết kế v0.4 và code/toolchain hiện tại.
  - Trust/sign placeholder ở `projects/ocp/security/*` vẫn là local-flow evidence, chưa phải production key management.
