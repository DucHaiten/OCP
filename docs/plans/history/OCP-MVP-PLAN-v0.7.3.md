# OCP v0.7.3 — Expansion Plan (Consumer-grade UI/Game + Shadow Primitive + Engines/Modules)
...
Mục tiêu v0.7.3: biến OCP thành nền làm **tool app/game** dễ khoe, dễ dùng, ít code, có “wow” mà ngôn ngữ phổ thông làm được nhưng tốn infrastructure:
...
- `ocp init mini-game` tạo project **≤ 7 file** (bao gồm 1–2 assets nhỏ nếu cần).
- Script OCP (không tính engine/packs): **≤ 400 LOC**.
- `ocp run` => game chạy (loop + input + draw) và tạo replay artifacts.
- `ocp replay` reproduces signature (lane locked, input recorded).

**KPI-2: Shadow-preview template**
- `ocp init shadow-preview` tạo project **≤ 7 file**.
...
- Engine config override qua `ocp.toml` hoặc record config trong OCP.
...
- OCP không làm widget framework phức tạp; OCP chỉ xuất “draw commands”.
- Layout/widgets (nếu có) nằm ở engine layer (OCP modules), không phải core.
...
- Source-of-truth cho `std.ui.frame_info` trong lane locked:
  ...
  - nếu thiếu field thì fallback từ `[engine.ui]` trong `ocp.toml`,
...
- `std.ui.frame_info`
  ...
  - locked lane rule:
    - `theme/locale/w/h/scale` lấy từ config deterministic (`ocp.toml`/engine config),
    - không đọc trực tiếp host runtime values trong lane locked.
- `std.ui.input`
...
- Engine API là OCP module functions + record config.
...
Rules:
- only enabled if `OCP_QUARANTINE=1`
...
Scope:
- `ocp init mini-game`
- `ocp init shadow-preview`
- (optional) `ocp init tool-ui`
...
- Implemented:
  ...
  - Test gate A:
    ...
    - mở rộng `tests/ocp_manifest_fs_kv_time.rs` thêm 2 tests cho `std_ui`.
- Files changed:
  - `src/ocp/diag.rs`
  - `src/ocp/audit.rs`
  - `src/ocp/registry.rs`
...
- Test results:
  - Targeted tests (must-pass for gate):
    ...
    - PASS: `ui_replay` 3/3.
  - Regression tests (supporting only):
    - PASS: `ocp_manifest_fs_kv_time` 7/7.
...
- Implemented:
  ...
  - Bổ sung test targeted gate B:
    ...
    - mở rộng `tests/ocp_manifest_fs_kv_time.rs` cho permissions `std_game`.
    - cập nhật `tests/ocp_stdlib.rs` để tương thích payload `std.game.tick_info`.
- Files changed:
  - `src/ocp/diag.rs`
  - `src/ocp/audit.rs`
  - `src/ocp/registry.rs`
...
- Test results:
  - Targeted tests (must-pass for gate):
    - `PASS` `std_game` (5/5)
    - `PASS` `ocp_manifest_fs_kv_time` (9/9)
    - `PASS` `ocp_stdlib` (13/13)
  - Regression tests (supporting only):
    - `PASS` full `cargo test` toàn workspace
...
- Implemented:
  ...
  - Bổ sung test targeted gate C:
    ...
    - mở rộng `tests/ocp_manifest_fs_kv_time.rs` cho permissions `std_shadow`.
- Files changed:
  - `src/ocp/diag.rs`
  - `src/ocp/audit.rs`
  - `src/ocp/registry.rs`
...
- Test results:
  - Targeted tests (must-pass for gate):
    - `PASS` `std_shadow` (5/5)
    - `PASS` `ocp_manifest_fs_kv_time` (11/11)
  - Regression tests (supporting only):
    - `PASS` full `cargo test` toàn workspace
    - `PASS` `cargo clippy --all-targets -- -D warnings`
    - `PASS` `cargo fmt -- --check`
...
- Scope:
  ...
  - Targeted tests gate D:
    - `tests/engine_ui.rs`
    - `tests/engine_game.rs`
    - mở rộng `tests/ocp_manifest_fs_kv_time.rs` cho deny-path của `engine.*` khi thiếu permissions tương ứng.
- Expected tests:
  - `cargo test --test engine_ui`
  - `cargo test --test engine_game`
  - `cargo test --test ocp_manifest_fs_kv_time`
...
- Implemented:
  ...
  - Bổ sung targeted tests gate D:
    ...
    - mở rộng `tests/ocp_manifest_fs_kv_time.rs` thêm 3 tests deny-path cho `engine.*` permissions.
- Files changed:
  - `src/ocp/exec.rs`
  - `src/ocp/registry.rs`
  - `projects/ocp/crates/ocp-sdk/src/lib.rs`
...
- Test results:
  - Targeted tests (must-pass for gate):
    ...
    - `PASS` `ocp_manifest_fs_kv_time` (14/14, gồm 3 test mới cho `engine.*`)
  - Regression tests (supporting only):
    - `PASS` full `cargo test` toàn workspace
    - `PASS` `cargo clippy --all-targets -- -D warnings`
    - `PASS` `cargo fmt -- --check`
...
- Scope:
  ...
  - Bổ sung test E2E:
    - `tests/cli_mini_game_e2e.rs`
    - `tests/cli_shadow_preview_e2e.rs`
  - Kiểm chứng `ocp run` tạo artifact bundle và `ocp replay` signature match cho cả hai template.
- Expected tests:
  - `cargo test --test cli_mini_game_e2e`
  - `cargo test --test cli_shadow_preview_e2e`
  - `cargo test`
  ...
  - `cargo fmt -- --check`
- Exit criteria:
  - `ocp init <dir> --template mini-game` và `shadow-preview` chạy thành công.
  - Cả hai template chạy `ocp run` thành công, tạo `.ocp_artifacts/<run_id>`.
  - `ocp replay <artifact_dir>` pass signature match cho cả hai template.
...
- Implemented:
  ...
  - Bổ sung auto-sync deps lock sau khi apply template:
    - `sync_deps_lock_v1(root)` được gọi ngay trong flow `ocp init --template ...` để tránh replay fail do lock mismatch.
  ...
  - E2E assertions:
    ...
    - `ocp run` tạo artifact bundle (`audit.jsonl`, `signature.txt`, `replay.toml`).
    - `ocp replay` pass signature match cho artifact vừa tạo.
- Files changed:
...
- Scope:
  - Thêm gate runtime/CLI: lane `quarantine` chỉ chạy khi `OCP_QUARANTINE=1`.
...
- Expected tests:
  ...
  - `cargo fmt -- --check`
- Exit criteria:
  - Lane `quarantine` fail-closed khi thiếu `OCP_QUARANTINE=1`.
...
- Implemented:
  - Hoàn tất lane gate `quarantine` theo scope tối thiểu v0.7.3:
    ...
    - enforce `quarantine` chỉ chạy khi `OCP_QUARANTINE=1` cho cả `ocp run` (project path) và `ocp replay`.
