# OCP v0.15 — Supply-chain Hardening + LTS Rehearsal (Security-by-Design Distribution)
...
Mục tiêu v0.15: đưa OCP tới mức **production readiness** ở góc độ supply-chain + governance:
...
- File code trọng yếu đã thay đổi:
  - `projects/ocp/crates/ocp-sdk/src/attestation_v15.rs`
...
### 1.1 North Star
- OCP có thể dùng trong môi trường “security-conscious”:
...
- Canonical workflow:
  ...
  - `ocp perm approve <diff_report>`
- `ocp perm review` (nếu giữ) chỉ là alias tổng hợp của `snapshot + diff`.
...
- Canonical commands:
  ...
  - `ocp perm approve <diff_report>`
- Alias compatibility:
  - `ocp perm review` (nếu tồn tại) phải forward sang flow canonical, không tạo semantics riêng.
- Policy gates:
...
- `ocp lts check` verifies project compatibility.

G) **CLI compatibility contract (LOCKED)**
- Canonical commands của v0.15:
  - `ocp lock sign|verify`
  - `ocp perm snapshot|diff|approve`
  - `ocp build --attest`
  - `ocp verify --attest|--repro`
  - `ocp lts check|report`
- Alias compatibility:
  - `ocp perm review` là alias additive.
...
- Trust/lock/permission policy parser + conformance runner wiring:
  - crate: `ocp-sdk`
  - path: `projects/ocp/crates/ocp-sdk/src/*`
...
- CLI commands:
  - `ocp lock sign|verify`
  - `ocp perm snapshot|diff|approve|review(alias)`
  - `ocp build --attest`, `ocp verify --attest|--repro`, `ocp lts check|report`
  - crate: `ocp-cli`
...
- `ocp lock sign --key <keyid> --lock deps.lock.v3`
- `ocp lock verify --lock deps.lock.v3`
- `ocp.lock` nếu có chỉ là export/read-only view:
...
- `ocp perm approve` generates approval entry.
- Enforcement hook (LOCKED):
  - trong lane `locked_v071`, `ocp build`, `ocp run`, và `ocp lts check` phải:
...
- implement canonical commands:
  ...
  - `ocp perm diff <old> <new>`
  - `ocp perm approve <diff_report>`
- `ocp perm review` chỉ là alias compatibility
...
- PASS:
  - `cargo test` pass.
  - conformance suite v0.14 pass (`ocp test --conformance run --suite all`).
...
- Gate/Step: 15-A
- Implemented:
  - mở rộng trust engine trong `ocp-sdk`:
...
- Gate/Step: 15-B
- Why:
  - khóa signed lockfile contract theo v0.15 với SoT `deps.lock.v3 + deps.lock.v3.sig` và CLI `ocp lock sign|verify`.
- Scope:
  ...
  - bổ sung CLI `ocp lock sign` và `ocp lock verify`;
...
- Scope:
  ...
  - Thêm CLI surface `ocp perm ...` (kèm alias `ocp perm review`).
...
- Implemented:
  ...
  - Mở rộng CLI:
    ...
    - `ocp perm diff`
    - `ocp perm approve`
    - `ocp perm review` (alias của diff flow)
...
- Scope:
  ...
  - Mở rộng CLI:
    - `ocp build --attest [--attest-key <keyid>]`
    - `ocp verify --attest <artifact_dir>`
    - `ocp verify --repro <artifact_dir>`
  - Giữ nguyên path cũ `ocp verify <project_dir> --phenotype ...` để không phá backward compatibility.
...
- Gate/Step: 15-D
- Implemented:
  - Thêm module mới `attestation_v15` trong `ocp-sdk`:
    - canonical JSON + `sha256-v1` cho build manifest.
    - tạo `target/ocp/attestation/build_manifest.json`.
...
- Gate/Step: 15-E
- Why:
  - khóa `ocp lts check/report` thành entrypoint machine-checkable cho Exit Contract v0.15, gom trust/lock/permission/conformance/upgrade-check vào một báo cáo deterministic.
- Scope:
  - mở rộng CLI `ocp lts check` và `ocp lts report`;
  - `lts check` phải:
...
- Expected tests:
  ...
  - `cargo fmt -- --check`
- Exit criteria:
  - `ocp lts check` pass khi toàn bộ gates nội bộ đạt;
...
- Implemented:
  - mở rộng CLI `ocp-cli`:
    - `ocp lts check <project_dir> [--manifest ... --target-runtime ... --out ... --json]`;
    - `ocp lts report <report_file> [--json]`.
  - triển khai pipeline `lts check`:
    ...
    - tổng hợp report `ocp.lts_check.v1`, deterministic, có `ok/error_code/risks/gates`.
...
- Implemented:
  ...
  - hardening vòng 2 cho chữ ký cassette:
    - ràng buộc chữ ký theo project-local secret (`.ocp_signing/cassette_sign.key.v15`) để chặn forge từ dữ liệu public,
