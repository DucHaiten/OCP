# OCP v0.12 — Shadow Scheduling + Incremental Reuse (Bounded Parallel Realities Engine)
...
- Legacy bridge:
  - nếu còn code path `src/ocp/*`, chỉ dùng làm lớp tương thích trong giai đoạn chuyển tiếp, không mở rộng semantics mới tại đó.
- Rule:
...
DX requirement:
- `ocp trace diff` can highlight scheduling divergence between runs.
...
- Expected tests:
  - `cargo test --test shadow_checkpoints`
  - `cargo test --test std_shadow --test ocp_manifest_fs_kv_time --test pack_schema_conformance_consumer`
  - `cargo test`
  - `cargo clippy --all-targets -- -D warnings`
  - `cargo fmt -- --check`
- Exit criteria:
...
- Test results:
  ...
  - Regression tests (supporting only):
    - `std_shadow`: `PASS` (5/5)
    - `ocp_manifest_fs_kv_time`: `PASS` (14/14)
...
- Scope:
  - Runtime:
    ...
    - thêm cờ runtime:
      - `OCP_STD_SHADOW_MEMOIZE_DETERMINISTIC_OBSERVE`
      - `OCP_STD_SHADOW_MEMO_OBSERVE_KEY`
      - `OCP_STD_SHADOW_MEMO_OBSERVE_COST_STEPS`
      - `OCP_STD_SHADOW_CHECKPOINT_REUSE`
  - Registry:
...
- Expected tests:
  - `cargo test --test shadow_memo`
  - `cargo test --test shadow_checkpoints --test std_shadow --test ocp_registry --test pack_schema_conformance_consumer`
  - `cargo test`
  - `cargo clippy --all-targets -- -D warnings`
  - `cargo fmt -- --check`
- Exit criteria:
...
- Implemented:
  - `projects/ocp/crates/ocp-cli/src/main.rs`:
    - cập nhật generator template `shadow-preview`:
      ...
      - thêm dataset variants bounded (`N=8`) và config policy/caps/scoring deterministic trong `src/main.ocp`.
...
- Test results:
  ...
  - Kết luận gate:
    - `DONE`
- Notes/risks:
  - KPI benchmark hiện dựa trên env toggle runtime (`OCP_STD_SHADOW_MEMOIZE_DETERMINISTIC_OBSERVE`, `OCP_STD_SHADOW_CHECKPOINT_REUSE`) và được assert trực tiếp trong e2e test để tránh done giả.
