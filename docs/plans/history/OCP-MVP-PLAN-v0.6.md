# OCP MVP PLAN v0.6 (Stabilization-Only)
...
- Trạng thái canonical đã chốt tại `projects/ocp/OCP-PLAN.md`: `OPEN=0` cho `001..017`.
- Unified release gate đã PASS trên clean checkout tại commit `1de5a285e7f16456cba0d2a39cef19c0435749ad`.
- Evidence signoff cuối: `projects/ocp/release/evidence/u10-final-signoff.md`.
- File này được giữ làm hồ sơ lịch sử của v0.6; trạng thái điều hành active chỉ đọc từ `OCP-PLAN.md`.
...
- Status: `DONE`
- Evidence:
  - `git ls-files "projects/ocp/crates/ocp-cli/target/**"` => `221` files tracked.
  - Ví dụ: `projects/ocp/crates/ocp-cli/target/ocp/v5/w3/reports/shadow_compare.run.json`
  - Ví dụ: `projects/ocp/crates/ocp-cli/target/ocp/v5/w3/reports/shadow_compare.reactor.json`
- Impact:
  - Dirty worktree sau mỗi lần run/test.
  - Dễ drift evidence, khó phân biệt code change và runtime artifacts.
- Exit criteria:
  1. Toàn bộ files dưới `projects/ocp/crates/ocp-cli/target/**` được untrack.
...
- Status: `DONE`
- Evidence:
  - SDK có gate: `projects/ocp/crates/ocp-sdk/src/v5_w6_organs.rs:21` (`OCP_RELEASE_GRADE`).
  - Lane script chưa set/check env `OCP_RELEASE_GRADE`.
  - `tools/ci_ocp_lane.ps1:30` chỉ in “release-grade” theo signer inputs.
- Impact:
  - Hardening release-grade có thể không được bật thật trong lane chính.
- Exit criteria:
  1. Lane blocking set rõ `OCP_RELEASE_GRADE=1` (hoặc flag tương đương có enforce thật).
...
- Status: `DONE`
- Evidence:
  - `tools/ci_ocp_quarantine.ps1:148` gọi:
    - `ocp test --conformance --locked --runtime throughput ... --json`
...
- Status: `DONE`
- Evidence:
  - File tracked: `projects/ocp/security/dev-root-1.signing.key.toml`
  - Chứa private key field: `projects/ocp/security/dev-root-1.signing.key.toml:3`
- Impact:
...
- Status: `DONE`
- Evidence:
  - `projects/ocp/OCP-MVP-PLAN-v0.1.md` tham chiếu nhiều path kiểu:
    - `tests/ocp_toy_programs.rs` (vd dòng 307/378/1371)
    - `tests/ocp_pilot.rs` (vd dòng 488/560)
  ...
  - Trong tree hiện tại, các file này không nằm cùng base path với plan v0.1:
    - tồn tại ở root/archive hoặc path khác (`projects/ocp/OCP-V0_1-GRAMMAR.md`, `docs/GATE-E-BENCHMARK-PROTOCOL.md`, `reports/gate-e-roi-v0.1.md`, `_archive/...`).
- Impact:
...
- Status: `DONE`
- Evidence:
  - `projects/ocp/OCP-MVP-PLAN-v0.1.md` còn các đoạn:
...
- Status: `DONE`
- Evidence:
  - `projects/ocp/OCP-MVP-PLAN-v0.2.md` tham chiếu nhiều path theo legacy-root:
    - `docs/OCP-USER-GUIDE.md` (dòng 414/423)
    - `src/ocp/*.rs` (dòng 427-431)
    - `tests/ocp_*.rs`, `soak_compare` (nhiều dòng trong matrix)
  - Một số path hiện đã chuyển vùng hoặc chỉ còn trong archive:
    - user guide canonical đang ở `projects/ocp/docs/OCP-USER-GUIDE.md` (root `docs/OCP-USER-GUIDE.md` chỉ là file MOVED).
- Impact:
...
- Status: `DONE`
- Evidence:
  - `projects/ocp/OCP-MVP-PLAN-v0.2.md` còn các cụm deferred:
...
### V6-ISSUE-010 - Lệch contract CLI trong plan v0.3 (`ocp doc`/`ocp lsp`)
- Severity: `MAJOR`
- Status: `DONE`
- Evidence:
  - `projects/ocp/OCP-MVP-PLAN-v0.3.md:87-98` ghi `CLI bắt buộc`, gồm:
    - `ocp doc`
    - `ocp lsp`
  - CLI hiện tại không có hai lệnh này:
    - `cargo run -p ocp-cli` in usage chỉ gồm `init|check|run|trace|profile|test|fmt|...`, không có `doc|lsp`.
- Impact:
...
- Status: `DONE`
- Evidence:
  - Nhiều command/log còn path legacy hoặc mơ hồ sau khi chuyển sang `projects/ocp/`, ví dụ:
    - `cargo run -p ocp-cli -- compose app-ocp --phenotype app-ocp/phenotype.toml --locked --json` (`projects/ocp/OCP-MVP-PLAN-v0.3.md:815`)
    - `cargo run -p ocp-cli -- verify app-ocp --phenotype app-ocp/phenotype.toml --locked --json` (`projects/ocp/OCP-MVP-PLAN-v0.3.md:816`)
    - file range không hợp lệ: `projects/ocp/OCP-MVP-PLAN-v0.1..v0.3.md` (`projects/ocp/OCP-MVP-PLAN-v0.3.md:363`)
...
- Exit criteria:
  ...
  2. Chuẩn hóa path examples về format thống nhất (`projects/ocp/...`) hoặc gắn nhãn lịch sử.
...
- Status: `DONE`
- Evidence:
  - `projects/ocp/OCP-MVP-PLAN-v0.3.md:270-277` có mục defer sang v0.4:
...
- Status: `DONE`
- Evidence:
  - `projects/ocp/OCP-MVP-PLAN-v0.4.md:433` ghi lane mặc định signed/release-grade theo repo signer baseline.
  - `projects/ocp/OCP-MVP-PLAN-v0.4.md:522` ghi lock rằng sign key không default từ repo path.
  - `projects/ocp/OCP-MVP-PLAN-v0.4.md:559` dùng command evidence với `--sign-key projects/ocp/security/dev-root-1.signing.key.toml`.
  - Script hiện tại:
    - `tools/ci_ocp_lane.ps1:14-18` chỉ lấy sign key từ env/flag, không auto default repo key.
    - `tools/ci_ocp_release.ps1:35-37` mặc định từ chối fixture key repo trong release lane.
- Impact:
...
- Status: `DONE`
- Evidence:
  - `projects/ocp/OCP-MVP-PLAN-v0.4.md:540`/`559` bám `conformance.v1.toml`.
  - Mặc định hiện tại ở SDK là v5:
    - `projects/ocp/crates/ocp-sdk/src/w9.rs:157-166` (`OCP_CONFORMANCE_DEFAULT`, fallback `conformance.v5.toml`).
- Impact:
...
- Status: `DONE`
- Evidence:
  - `projects/ocp/OCP-MVP-PLAN-v0.4.md:91` còn ghi “Single-actor bounded mailbox cho W1, multi-actor defer.”
...
- Status: `DONE`
- Evidence:
  - `projects/ocp/OCP-MVP-PLAN-v0.5.md:1021` ghi pass lane với:
    - `-SignKey projects/ocp/security/dev-root-1.signing.key.toml`
  - `projects/ocp/OCP-MVP-PLAN-v0.5.md:1071` ghi pass conformance với:
    - `--sign-key projects/ocp/security/dev-root-1.signing.key.toml`
- Impact:
...
### V6-ISSUE-017 - CI lane đang generate/delete `cosmos` cho `app-ocp` tại runtime (baseline không pin trong repo)
- Severity: `MAJOR`
- Status: `DONE`
- Evidence:
  - `projects/ocp/OCP-MVP-PLAN-v0.5.md:746` ghi rõ `cosmos.toml/cosmos.lock.v1` không commit mặc định cho `app-ocp`, lane tự `cosmos init`.
  - `tools/ci_ocp_lane.ps1:94-95` xóa `app-ocp/cosmos.toml` và `app-ocp/cosmos.lock.v1` trước run.
  - `tools/ci_ocp_lane.ps1:116-118` sinh lại `cosmos init` + `cosmos lock sync`.
  - `tools/ci_ocp_lane.ps1:143-144` xóa lại sau run.
- Impact:
...
- Evidence:
  - Khi rerun matrix v0.1, nhiều suite fail compile `E0063`:
    ...
    - `tests/ocp_pilot.rs`
  - Nguyên nhân: `ExecConfig` hiện có thêm field `commit_policy` (`src/ocp/budget.rs`) nhưng test literals vẫn dùng dạng cũ chỉ có `step_cap`.
- Impact:
...
- Exit criteria:
  ...
  2. Rerun matrix v0.1 pass đầy đủ:
     ...
     - `cargo fmt -- --check`.
  3. Ghi nhận thay đổi vào `OCP-MVP-PLAN-v0.1.md` và giữ dấu vết trong v0.6.
- Resolution (2026-03-03):
  ...
  - Đã rerun matrix và pass:
    ...
    - `cargo clippy --all-targets -- -D warnings`
  - `cargo fmt -- --check`
  - Đã cập nhật bổ sung audit entry vào `OCP-MVP-PLAN-v0.1.md`.
...
- Evidence:
  - Matrix v0.2 rerun PASS trong cùng phiên:
    - `cargo test --test ocp_parser` (4/4)
    - `cargo test --test ocp_typecheck` (6/6)
    - `cargo test --test ocp_exec` (9/9)
    - `cargo test --test ocp_toy_programs` (3/3)
    - `cargo test --test ocp_pilot` (4/4)
    - `cargo test --bin soak_compare` (1/1)
    - `cargo test` (full suite PASS)
    - `cargo clippy --all-targets -- -D warnings` (PASS)
    - `cargo fmt -- --check` (PASS)
  - Đối chiếu code trục khóa v0.2:
    - `src/ocp/exec.rs` có caps bounded `condition`/`entangle` (`CONDITION_*`, `ENTANGLE_*`).
    - `src/ocp/diag.rs` và `src/ocp/exec.rs` giữ canonical `X-*`/`R-*` + `root_reason`.
    - `tests/ocp_ctx_validation.rs` assert `R-CTX-INVALID` + `RC-CTX-INVALID`.
  - Chuẩn hóa tài liệu:
    - `OCP-MVP-PLAN-v0.2.md` đã bổ sung planning freeze bị thiếu cho V2-B..V2-E.
...
- Exit criteria:
  ...
  4. Có log cập nhật tương ứng trong `OCP-MVP-PLAN-v0.2.md`.
- Resolution (2026-03-04):
  - Đã chạy pass:
    - `cargo test --test ocp_confidence` (4/4)
    - `cargo test` (full suite pass)
    - `cargo clippy --all-targets -- -D warnings` (pass)
    - `cargo fmt -- --check` (pass sau `cargo fmt`)
  - Đã cập nhật entry `2026-03-04 - V0.6 confidence hardening append` trong `OCP-MVP-PLAN-v0.2.md`.
...
- Exit criteria:
  ...
  2. Có entry re-verify trong `OCP-MVP-PLAN-v0.3.md`.
  3. Không phát sinh regression mới ở lane e2e và conformance.
- Resolution (2026-03-04):
  - Đã đạt đủ 3 tiêu chí trên.
  - Đã cập nhật entry `2026-03-04 - V0.6 audit v0.3 consistency re-verify` trong `OCP-MVP-PLAN-v0.3.md`.
...
- Evidence:
  ...
  - Chạy thêm W9 explicit:
    - `cargo test -p ocp-sdk --test w9_conformance` (4/4)
    - `cargo test -p ocp-cli w9_cli_` (2/2)
    - `cargo run -p ocp-cli -- test --conformance --locked --runtime deterministic --engine dual --manifest projects/ocp/conformance/conformance.v1.toml --out target/ocp/w9/reports/conformance_report.json --trust-store projects/ocp/security/trust.store.toml --signer-id dev-root-1 --sign-key projects/ocp/security/dev-root-1.signing.key.toml --json` (10/10, `required_digest=10deff71c24ef729`)
- Impact:
...
- Exit criteria:
  ...
  3. Có entry re-verify mới trong `OCP-MVP-PLAN-v0.4.md`.
- Resolution (2026-03-04):
  - Đã đạt đủ 3 tiêu chí trên.
  - Đã cập nhật entry `2026-03-04 - V0.6 audit v0.4 consistency re-verify` trong `OCP-MVP-PLAN-v0.4.md`.
...
- Evidence:
  - Đã rerun đầy đủ nhóm lệnh v0.5 hiện hành:
    ...
    - `cargo clippy -p ocp-runtime-core -p ocp-sdk -p ocp-cli --all-targets -- -D warnings`
    - `cargo fmt -- --check`
    - `cargo test -p ocp-sdk --test v5_w2_domain_resolution` (6/6)
    - `cargo test -p ocp-sdk --test v5_w3_shadow` (6/6)
    - `cargo test -p ocp-sdk --test v5_w4_hive` (3/3)
    - `cargo run -p ocp-cli -- test --conformance --locked --runtime deterministic --engine dual --manifest projects/ocp/conformance/conformance.v1.toml --out target/ocp/w9/reports/conformance_report.json --trust-store projects/ocp/security/trust.store.toml --signer-id dev-root-1 --sign-key projects/ocp/security/dev-root-1.signing.key.toml --json` (10/10, `required_digest=10deff71c24ef729`)
    - `cargo run -p ocp-cli -- test --conformance --locked --runtime deterministic --engine dual --manifest projects/ocp/conformance/conformance.v5.toml --out target/ocp/w9/reports/conformance_report.v5.json --trust-store projects/ocp/security/trust.store.toml --signer-id dev-root-1 --sign-key projects/ocp/security/dev-root-1.signing.key.toml --json` (5/5, `required_digest=9e97cc112c567170`)
    - `powershell -ExecutionPolicy Bypass -File tools/ci_ocp_lane.ps1` (PASS)
    - `powershell -ExecutionPolicy Bypass -File tools/ci_ocp_quarantine.ps1` (PASS)
  - Đã tái hiện drift trong matrix cũ:
    - `-p ocp-runtime-rt` fail vì package không tồn tại.
    - conformance locked command thiếu signer/trust fail `W9-CONFORMANCE-SIGN-REQUIRED`.
  - Đã cập nhật thủ công `OCP-MVP-PLAN-v0.5.md` section `Validation matrix v0.5` để phản ánh command thực chạy.
- Impact:
...
- Exit criteria:
  ...
  3. Có entry audit mới trong `OCP-MVP-PLAN-v0.5.md` theo cặp planning/closeout.
- Resolution (2026-03-04):
  - Đã đạt đủ 3 tiêu chí trên.
  - Đã cập nhật entry:
