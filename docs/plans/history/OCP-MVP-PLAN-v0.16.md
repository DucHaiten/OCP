# OCP v0.16 — Productization Audit + Unified Readiness (Pre-v1.0 Hard Freeze)
...
Mục tiêu v0.16: **không thêm tính năng/ngữ nghĩa mới**; thay vào đó triển khai một vòng kiểm tra sản phẩm hóa tổng thể để xác nhận OCP đã là một hệ thống thống nhất, dùng được thực tế, đủ điều kiện bước sang v1.0.
...
- Kết quả cần chứng minh:
  - toàn hệ OCP vận hành nhất quán xuyên suốt các phiên bản,
...
- Artifacts:
  ...
  - `target/ocp/w16/history/historical_evidence_report.json`
  - `target/ocp/w16/contracts/contract_inventory_completeness_report.json`
- Pass:
...
- Artifacts:
  ...
  - `target/ocp/w16/conformance/unified_conformance_report.txt`
- Pass:
...
- Artifacts:
  ...
  - `target/ocp/w16/determinism/cache_signature_invariance_report.json`
  - `target/ocp/w16/determinism/cassette_offline_invariance_report.json`
- Pass:
...
- Artifacts:
  ...
  - `target/ocp/w16/security/repo_hygiene_report.json`
  - `target/ocp/w16/security/license_inventory.json`
- Pass:
...
- `target/ocp/w16/contracts/language_contract_snapshot.json`
- `target/ocp/w16/contracts/runtime_contract_snapshot.json`
- `target/ocp/w16/contracts/trace_contract_snapshot.json`
- `target/ocp/w16/contracts/supplychain_contract_snapshot.json`
- `target/ocp/w16/contracts/contract_inventory.json`
- `target/ocp/w16/contracts/contract_freeze_report.json`
...
### 6.3 Outputs
- `target/ocp/w16/determinism/determinism_soak_report.json`
- `target/ocp/w16/determinism/replay_invariant_report.json`
- `target/ocp/w16/determinism/cache_signature_invariance_report.json`
- `target/ocp/w16/determinism/cassette_offline_invariance_report.json`
...
### 7.3 Outputs
- `target/ocp/w16/compat/compat_rehearsal_report.json`
- `target/ocp/w16/compat/migration_diff_report.json`
...
### 8.3 Outputs
- `target/ocp/w16/perf/perf_budget_report.json`
- `target/ocp/w16/perf/perf_trend_vs_v015.json`
- `target/ocp/w16/perf/perf_baseline_verify_report.json`
...
### 9.3 Outputs
- `target/ocp/w16/security/security_chain_report.json`
- `target/ocp/w16/security/lts_strict_report.json`
- `target/ocp/w16/security/license_inventory.json`
- `target/ocp/w16/security/sbom.spdx.json` (optional, nếu pipeline hỗ trợ)
...
- RC phải chạy được chuỗi thao tác người dùng mới trên clean checkout:
  1. `ocp init <template>`
  2. `ocp lock sync` + `ocp lock verify`
  3. `ocp perm snapshot` + `ocp perm diff` + `ocp perm approve` (khi có diff)
  4. `ocp build --attest`
  5. `ocp verify --attest`
  6. `ocp run` + `ocp replay`
  7. `ocp dbg` smoke script (không IO mới)
...
- `target/ocp/w16/rc/release_readiness_checklist.json`
...
- Scope:
  ...
  - Tạo SoT files:
    ...
    - `contracts/history/evidence_index.v1.sig`
    - `contracts/required_contracts.v1.json`
  - Sinh report artifacts Gate 16-A trong `target/ocp/w16/contracts|history|meta`.
- Expected tests:
...
- Scope:
  - Thêm code delta thật tại `tests/conformance_runner.rs` để:
    ...
    - xuất `target/ocp/w16/conformance/unified_conformance_report.json`,
    - xuất `target/ocp/w16/conformance/unified_conformance_report.txt`,
...
- Implemented:
  ...
  - Test mới thực hiện:
    ...
    - sinh `target/ocp/w16/conformance/unified_conformance_report.json`,
    - sinh `target/ocp/w16/conformance/unified_conformance_report.txt`,
...
- Scope:
  - Bổ sung code delta thật cho `16-C`:
    - thêm soak report v0.16 từ `tests/ocp_determinism.rs`,
    ...
    - sinh artifacts determinism tại `target/ocp/w16/determinism/*`,
    - xử lý ổn định regression song song ở conformance runner (`projects/ocp/crates/ocp-sdk/src/w9.rs`) để full `cargo test` chạy nhất quán.
- Expected tests:
  - `cargo test --test ocp_determinism`
  - `cargo test --test quarantine_gate`
  - `cargo test --test ui_replay`
...
- Gate/Step: 16-C
- Implemented:
  - Mở rộng `tests/ocp_determinism.rs` với test soak `v16_determinism_soak_signature_and_trace_stable`, sinh `target/ocp/w16/determinism/determinism_soak_report.json`.
  - Thêm `tests/cache_signature_invariance.rs` để kiểm chứng cache on/off trong lane locked vẫn giữ signature và trace bất biến, sinh `target/ocp/w16/determinism/cache_signature_invariance_report.json`.
  - Thêm `tests/cassette_replay_offline_invariance.rs` để kiểm chứng quarantine record -> replay offline giữ signature bất biến, sinh:
    - `target/ocp/w16/determinism/cassette_offline_invariance_report.json`
    - `target/ocp/w16/determinism/replay_invariant_report.json`
  - Vá race song song trong conformance runtime workspace tại `projects/ocp/crates/ocp-sdk/src/w9.rs` bằng runtime root tách theo process id (`target/ocp/w9/runtime/<pid>`), loại bỏ đụng độ copy/delete khi full suite chạy đồng thời.
- Files changed:
  - `tests/ocp_determinism.rs`
  - `tests/cache_signature_invariance.rs`
  - `tests/cassette_replay_offline_invariance.rs`
  ...
  - `OCP-MVP-PLAN-v0.16.md`
- Commands run:
  - `cargo test --test ocp_determinism --test quarantine_gate --test ui_replay --test cache_signature_invariance --test cassette_replay_offline_invariance`
  - `cargo test --test ocp_determinism --test quarantine_gate --test ui_replay --test cache_signature_invariance --test cassette_replay_offline_invariance`
  - `cargo test`
  - `cargo test`
  - `cargo test --test lts_check`
...
- Notes/risks:
  ...
  - Artifacts determinism v0.16 đã được sinh đầy đủ tại `target/ocp/w16/determinism/`.
...
- Notes/risks:
  ...
  - Artifact gate đã sinh đầy đủ dưới `target/ocp/w16/compat/`.
...
- Design alignment:
  - `FULL`
- Notes/risks:
  - Baseline v0.15 đang pin theo fixture ổn định `projects/ocp/apps/hello-cli` với mode `interpreter`; nếu thay fixture baseline trong tương lai phải cập nhật đồng bộ bundle + signature.
...
- Scope:
  ...
  - Ghi đầy đủ output bảo mật bắt buộc cho `target/ocp/w16/security/*`.
- Expected tests:
  - `cargo test --test trust_policy`
  - `cargo test --test lock_sign`
  - `cargo test --test perm_review`
...
- Scope:
  ...
  - Đảm bảo các report RC bắt buộc được tạo dưới `target/ocp/w16/rc/`.
...
- Implemented:
  - Thêm test `v1_user_journey_smoke` chạy đầy đủ chuỗi RC user journey:
    - `ocp init --template tool-cli`
    - `ocp lock sync/sign/verify`
    - `ocp perm snapshot/diff/approve`
    - `ocp build --attest` + `ocp verify --attest`
    - `ocp run` + `ocp replay`
    - `ocp dbg --script` smoke
