# OCL v0.19 — Editor Productization (VSCode-grade) + LSP/DAP Shipproof
...
- Mục tiêu phiên bản:
  - biến OCL thành ngôn ngữ editor-ready chuẩn VSCode-grade (LSP/DAP shipproof).
...
- File code trọng yếu dự kiến sẽ thay đổi:
  ...
  - `projects/ocp-ocl/crates/ocl-lsp/*`
  - `projects/ocp-ocl/crates/ocl-dap/*` (theo lock debug)
  - `projects/ocp-ocl/crates/ocl-sdk/src/lib.rs`
  - `projects/ocp-ocl/crates/ocl-cli/src/main.rs`
  - `contracts/editor/*`
  - `tests/v19_*.rs`
  - `OCP-OCL-MVP-PLAN-v0.19.md`
...
- LSP server:
  - crate: `ocl-lsp`
  - path: `projects/ocp-ocl/crates/ocl-lsp/src/*`
- DAP adapter:
  - crate: `ocl-dap`
  - path: `projects/ocp-ocl/crates/ocl-dap/src/*`
- Shared contracts/serialization/taxonomy bridges:
  - crate: `ocl-sdk`
  - path: `projects/ocp-ocl/crates/ocl-sdk/src/lib.rs`
- CLI bridge commands (`ocl fmt/check/doctor/fix/debug`) cho editor workflows:
  - crate: `ocl-cli`
...
- Mọi gate v0.19 phải sinh:
  - `target/ocl/w19/meta/run_manifest.json`
...
- `editor-ci` phải bao phủ:
  ...
  - xuất artifacts vào `target/ocl/w19/*`
...
### 1.1 North Star
- Mở file `.ocl` trong VSCode phải có trải nghiệm tương đương ngôn ngữ trưởng thành:
  - highlight đúng,
...
- PASS khi:
  - `.ocl` được nhận diện đúng `languageId=ocp-ocl`
...
- PASS khi:
  - `ocl fmt` và LSP formatting đồng nhất output
...
- LSP server chạy native process (`ocl-lsp`) và gọi trực tiếp parser/typechecker từ runtime crates.
- Không dùng shell-out `ocl check` cho đường realtime chính.

### 2.6 Formatter contract (LOCKED)
- Formatter canonical là một:
  - `ocl fmt` (CLI) và LSP formatting phải chung một engine.
...
- Bắt buộc có handshake version giữa:
  ...
  - `ocl-dap`
- Nguồn sự thật:
  - `contracts/editor/ocl_version_compat_matrix.v1.json`
- Rule:
...
- Bootstrap path phải khóa deterministic:
  - `globalStorage/ocp-ocl/<version>/<platform>/`.
...
- Rule:
  - workspace untrusted => không spawn `ocl-lsp`/`ocl-dap`, chỉ cho phép editor shell tĩnh (grammar/snippets).
...
- Trace artifact path scheme deterministic:
  - `.ocl_artifacts/editor_dbg/<workspace_hash>/<run_id>/`
...
- `contracts/editor/ocl_language_profile.v1.json`
- `contracts/editor/ocl_textmate_grammar.v1.json`
- `contracts/editor/ocl_semantic_tokens.v1.json`
- `contracts/editor/ocl_lsp_capabilities.v1.json`
- `contracts/editor/ocl_diagnostics_mapping.v1.json`
- `contracts/editor/ocl_formatting_contract.v1.json`
- `contracts/editor/ocl_code_actions_contract.v1.json`
- `contracts/editor/ocl_debug_contract.v1.json`
- `contracts/editor/ocl_settings_schema.v1.json`
- `contracts/editor/ocl_version_compat_matrix.v1.json`
- `contracts/editor/ocl_binary_bootstrap_policy.v1.json`
...
- Quy ước tên file chữ ký là duy nhất:
  - `<sot_file>.sig` (ví dụ: `ocl_language_profile.v1.json.sig`)
...
- Aggregate report:
  - `target/ocl/w19/rc/platform_matrix_report.json`
- Provenance report:
  - `target/ocl/w19/rc/platform_matrix_provenance_report.json`
- Rule:
...
Artifacts:
- `target/ocl/w19/editor/language_profile_report.json`
- `target/ocl/w19/editor/highlight_golden_report.json`
- `target/ocl/w19/contracts/editor_contract_inventory_report.json`
- `target/ocl/w19/contracts/editor_inventory_sync_report.json`
- `target/ocl/w19/contracts/editor_sot_signature_report.json`
- `target/ocl/w19/contracts/editor_public_surface_report.json`
- `target/ocl/w19/contracts/editor_versioning_policy_report.json`
- `target/ocl/w19/community/editor_docs_readiness_report.json`
- `target/ocl/w19/editor/settings_schema_report.json`
- `target/ocl/w19/editor/editor_assets_manifest_report.json`
Exit criteria:
- `.ocl` nhận diện đúng languageId.
...
- startup handshake extension<->lsp theo `ocl_version_compat_matrix.v1.json`.
...
Artifacts:
- `target/ocl/w19/lsp/diagnostics_report.json`
- `target/ocl/w19/lsp/symbols_report.json`
- `target/ocl/w19/lsp/version_handshake_report.json`
- `target/ocl/w19/lsp/runtime_resilience_report.json`
- `target/ocl/w19/security/editor_privacy_hygiene_report.json`
- `target/ocl/w19/editor/semantic_tokens_golden_report.json`
- `target/ocl/w19/security/workspace_trust_report.json`
- `target/ocl/w19/security/workspace_trust_semantic_tokens_report.json`
Exit criteria:
...
Artifacts:
- `target/ocl/w19/lsp/navigation_report.json`
- `target/ocl/w19/lsp/multiroot_workspace_report.json`
Exit criteria:
...
Artifacts:
- `target/ocl/w19/editor/formatting_report.json`
- `target/ocl/w19/editor/code_actions_report.json`
- `target/ocl/w19/editor/cli_bridge_report.json`
- `target/ocl/w19/editor/code_action_apply_contract_report.json`
Exit criteria:
...
Artifacts:
- `target/ocl/w19/debug/dap_smoke_report.json`
- `target/ocl/w19/debug/dap_trace_mapping_report.json`
- `target/ocl/w19/debug/dap_trace_acquisition_report.json`
Exit criteria:
...
Artifacts:
- `target/ocl/w19/release/editor_release_manifest.json`
- `target/ocl/w19/release/editor_release_manifest.sig`
- `target/ocl/w19/release/vsix_verify_report.json`
- `target/ocl/w19/release/binary_bootstrap_report.json`
- `target/ocl/w19/release/binary_recovery_report.json`
- `target/ocl/w19/release/publish_channel_report.json`
- `target/ocl/w19/release/rollback_rehearsal_report.json`
- `target/ocl/w19/release/vsix_size_report.json`
- `target/ocl/w19/release/publish_prerequisites_report.json`
- `target/ocl/w19/release/node_supplychain_report.json`
- `target/ocl/w19/release/packaging_toolchain_report.json`
- `target/ocl/w19/release/bundled_tooling_presence_report.json`
- `target/ocl/w19/security/signing_trust_root_report.json`
- `target/ocl/w19/release/bootstrap_retention_report.json`
Exit criteria:
...
- release manifest phải pin đầy đủ hash/signature cho `ocl-cli`/`ocl-lsp`/`ocl-dap`.
...
- `cargo fmt -- --check`
Artifacts:
- `target/ocl/w19/rc/editor_release_readiness_report.json`
- `target/ocl/w19/rc/golden_user_journey_editor_report.json`
- `target/ocl/w19/rc/platform_matrix_report.json`
- `target/ocl/w19/rc/platform_matrix_provenance_report.json`
- `target/ocl/w19/perf/editor_perf_budget_report.json`
- `target/ocl/w19/perf/editor_perf_protocol_report.json`
- `target/ocl/w19/rc/ci_harness_report.json`
- `target/ocl/w19/community/editor_ci_entrypoint_report.json`
Exit criteria:
...
- VSCode extension install được, `.ocl` recognized, highlight + format + diagnostics realtime hoạt động.
- LSP supports:
...
- version compatibility matrix:
  - extension<->lsp<->dap handshake pass theo `ocl_version_compat_matrix.v1.json`.
...
- publish prerequisites + supply-chain:
  ...
  - VSIX size guard pass cho mọi channel đã khóa.
  - bundled tooling presence pass (ocl-cli/ocl-lsp/ocl-dap từ bundle).
...
- `cargo xtask editor-ci`
- `pnpm --dir editor/vscode/ocp-ocl install --frozen-lockfile`
- `pnpm --dir editor/vscode/ocp-ocl run test:integration`
- `pnpm --dir editor/vscode/ocp-ocl run package`
...
- Scope:
  - Tạo skeleton VSCode extension `editor/vscode/ocp-ocl/*`.
  - Tạo SoT `contracts/editor/*` theo danh mục v0.19.
  - Tạo fixtures highlight và docs cộng đồng bắt buộc.
  - Thêm targeted tests v19-A và report outputs trong `target/ocl/w19/*`.
...
- Gate/Step: 19-A
- Implemented:
  - Tạo VSCode extension shell cho OCL:
...
- Files changed:
  - `editor/vscode/ocp-ocl/*` (skeleton extension files cho gate 19-A).
  ...
  - `projects/ocp-ocl/crates/ocl-sdk/src/lib.rs`
  - `projects/ocp-ocl/crates/ocl-sdk/src/w19.rs`
...
- Scope:
  - Bổ sung hàm core 19-B trong `ocl-sdk/w19` cho:
    - diagnostics mapping,
    - symbols extraction,
    - version handshake,
...
- Expected tests:
 ...
 - Exit criteria:
  ...
  - Có đủ artifacts:
    - `target/ocl/w19/lsp/diagnostics_report.json`
    - `target/ocl/w19/lsp/symbols_report.json`
    - `target/ocl/w19/lsp/diagnostics_taxonomy_mapping_report.json`
    - `target/ocl/w19/lsp/version_handshake_report.json`
    - `target/ocl/w19/lsp/runtime_resilience_report.json`
    - `target/ocl/w19/security/editor_privacy_hygiene_report.json`
    - `target/ocl/w19/editor/semantic_tokens_golden_report.json`
...
- Gate/Step: 19-B
- Implemented:
  - Bổ sung core functions cho 19-B trong `ocl-sdk/w19`:
  ...
  - Export đầy đủ các hàm/types 19-B từ `ocl-sdk/src/lib.rs`.
  ...
  - Sửa compile blockers thực tế:
    - thêm dependency `serde` cho `ocl-sdk` (phục vụ serialization report structs),
...
- Files changed:
  ...
  - `projects/ocp-ocl/crates/ocl-sdk/src/lib.rs`
  - `projects/ocp-ocl/crates/ocl-sdk/src/w19.rs`
  - `tests/fixtures/v19/lsp/diagnostics_error.ocl`
  - `tests/fixtures/v19/lsp/diagnostics_ok.ocl`
  - `tests/fixtures/v19/lsp/symbols.ocl`
  - `tests/fixtures/v19/lsp/semantic_tokens.ocl`
  - `tests/v19_lsp_diagnostics.rs`
  - `tests/v19_lsp_symbols.rs`
  - `tests/v19_diagnostics_taxonomy_mapping.rs`
  - `tests/v19_lsp_version_handshake.rs`
...
- Design alignment:
  - `FULL`
- Notes/risks:
  - Gate có 1 lỗi compile thật ở vòng chạy đầu (`serde` chưa khai báo trong `ocl-sdk`, và nhánh `LetPattern` không export qua `ocl-runtime-core`), đã vá dứt điểm trước khi chốt.
  - Gate có 1 lỗi fixture parser thật ở `symbols.ocl` (thiếu `;` sau `struct/enum` declaration), đã sửa và re-run test liên quan.
...
- Scope:
  - Bổ sung API 19-C trong `ocl-sdk/w19`:
    - hover,
    - completion,
...
- Gate/Step: 19-C
- Implemented:
  - Bổ sung logic 19-C trong `ocl-sdk/w19`:
  ...
  - Re-export đầy đủ API 19-C từ `ocl-sdk/src/lib.rs`.
  - Thêm fixtures:
    - `tests/fixtures/v19/lsp/navigation.ocl`
    - `tests/fixtures/v19/multiroot/root_a/shared.ocl`
    - `tests/fixtures/v19/multiroot/root_z/shared.ocl`
...
- Scope:
  - Bổ sung formatter shared contract API trong `ocl-sdk/w19` để CLI/LSP dùng chung engine.
...
- Gate/Step: 19-D
- Implemented:
  - Bổ sung logic 19-D trong `ocl-sdk/w19`:
  ...
  - Re-export đầy đủ API 19-D từ `ocl-sdk/src/lib.rs`.
  ...
  - Sửa lỗi compile test lần chạy đầu:
    - bỏ import trực tiếp `ocl_runtime_core` trong test integration `v19_formatting_unified`.
- Files changed:
  - `projects/ocp-ocl/crates/ocl-sdk/src/w19.rs`
  - `projects/ocp-ocl/crates/ocl-sdk/src/lib.rs`
  - `tests/v19_formatting_unified.rs`
...
- Scope:
  - Bổ sung API debug 19-E trong `ocl-sdk/w19`:
...
- Expected tests:
 ...
 - Exit criteria:
  ...
  - DAP launch/breakpoint/step/mapping/acquisition bám đúng `ocl_debug_contract.v1.json`.
...
- Gate/Step: 19-E
- Implemented:
  - Bổ sung logic 19-E trong `ocl-sdk/w19`:
  ...
  - Re-export đầy đủ API/types debug từ `ocl-sdk/src/lib.rs`.
  - Thêm fixture debug:
    - `tests/fixtures/v19/dap/session.ocl`
  ...
  - Sửa lỗi fixture parse ở vòng chạy đầu:
    - bỏ nhánh `if` không thuộc grammar parse hiện tại trong `tests/fixtures/v19/dap/session.ocl`.
- Files changed:
  - `projects/ocp-ocl/crates/ocl-sdk/src/w19.rs`
  - `projects/ocp-ocl/crates/ocl-sdk/src/lib.rs`
  - `tests/fixtures/v19/dap/session.ocl`
...
- Scope:
  - Bổ sung helper packaging/release trong `ocl-sdk/w19`:
...
- Expected tests:
 ...
 - Exit criteria:
  ...
  - Có đủ artifacts:
    - `target/ocl/w19/release/editor_release_manifest.json`
    - `target/ocl/w19/release/editor_release_manifest.json.sig`
    - `target/ocl/w19/release/vsix_verify_report.json`
    - `target/ocl/w19/release/binary_bootstrap_report.json`
    - `target/ocl/w19/release/binary_recovery_report.json`
    - `target/ocl/w19/release/publish_channel_report.json`
...
- Gate/Step: 19-F
- Implemented:
  - Bổ sung logic 19-F trong `ocl-sdk/w19`:
  ...
  - Re-export đầy đủ API 19-F từ `ocl-sdk/src/lib.rs`.
  ...
  - Thêm fixture release-debug phụ trợ:
    - `tests/fixtures/v19/dap/session.ocl` được giữ tương thích parse.
...
- Design alignment:
  - `FULL`
- Notes/risks:
  - `editor_release_manifest.prev.json(.sig)` được tạo trong rollback rehearsal để mô phỏng rollback có kiểm soát; đây là artifact test tạm trong `target/ocl/w19/release`.
...
- Implemented:
  ...
  - Bổ sung script integration cho editor package:
    - `editor/vscode/ocp-ocl/scripts/test-integration.mjs`
...
### 2026-03-06 — 19-G Verification Snapshot (icon `.ocl` chữ O xanh)
- Date: 2026-03-06
- Gate/Step: 19-G
- Implemented:
  - Tinh chỉnh icon file `.ocl` về đúng yêu cầu thị giác: chỉ còn chữ `O` xanh lục, nền trong suốt, không còn hình bao quanh.
  - Cập nhật lại hash trong contract assets editor để khớp icon mới.
  - Đồng bộ lại chữ ký SoT sau khi hash thay đổi.
- Files changed:
  - `editor/vscode/ocp-ocl/icons/ocp-ocl.svg`
  - `contracts/editor/editor_assets_manifest.v1.json`
  - `contracts/editor/editor_assets_manifest.v1.json.sig`
- Commands run:
  - `Get-FileHash -Algorithm SHA256 editor/vscode/ocp-ocl/icons/ocp-ocl.svg`
  - `cargo test --test v19_editor_assets_manifest`
  - `cargo test --test v19_editor_sot_signature_verify`
...
- Design alignment:
  - `FULL`
- Notes/risks:
  - VSCode cần `Developer: Reload Window` và chọn `File Icon Theme = OCP OCL Icons` để thấy icon mới ngay.
...
- Implemented:
  ...
  - Rà lại toàn bộ `target/ocl/w19/*.json` và xác nhận không còn report thiếu cặp run manifest fields theo rule v0.19.
- Files changed:
  - `tests/v19_platform_matrix_aggregate.rs`
  - `editor/vscode/ocp-ocl/scripts/test-integration.mjs`
  - `target/ocl/w19/rc/os/win-x64/vscode_integration_report.json`
  - `target/ocl/w19/rc/os/linux-x64/vscode_integration_report.json`
  - `target/ocl/w19/rc/os/macos-arm64/vscode_integration_report.json`
  - `target/ocl/w19/rc/vscode_integration_cli_report.json`
- Commands run:
  - `cargo test --test v19_platform_matrix_aggregate`
  - `cargo test --test v19_platform_matrix_provenance`
  - `cargo test --test v19_vscode_integration`
  - `corepack pnpm --dir editor/vscode/ocp-ocl run test:integration`
  - `Test-Path target/ocl/w19/rc/os/<profile>/vscode_integration_report.json` (3 profiles)
  - kiểm tra toàn bộ report JSON dưới `target/ocl/w19` để xác nhận run-manifest linkage.
- Test results:
  - Targeted tests (must-pass for gate):
    - `PASS`:
      - `v19_platform_matrix_aggregate`
...
- Implemented:
  ...
  - Chuẩn hóa lại report contract:
    ...
    - toàn bộ report JSON dưới `target/ocl/w19` có đủ `run_manifest_ref` + `run_manifest_sha256` (ngoại lệ hợp lệ: run manifest/release manifest tự thân).
...
- Commands run:
  ...
  - quét artifact paths trong plan + quét toàn bộ `target/ocl/w19/*.json` để xác nhận completeness/linkage.
- Test results:
  - Targeted tests (must-pass for gate):
    - `PASS`:
      - `v19_platform_matrix_aggregate`
...
- Chỉ mở v1.0 public release khi:
  ...
  - `target/ocl/w19/rc/editor_release_readiness_report.json` pass
