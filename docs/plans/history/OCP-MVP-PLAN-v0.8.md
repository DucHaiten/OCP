# OCP v0.8 — Quarantine + Cassette Record/Replay for Non-deterministic IO (Deterministic-by-Recording)
...
Mục tiêu v0.8: mở rộng OCP để làm **app/tool có network / process / wallclock** mà vẫn giữ được “điểm mạnh độc đáo”:
...
- Lane `quarantine` cho phép IO nondet **nhưng bắt buộc**:
  - được cấp quyền rõ trong `ocp.toml`,
...
- Cùng script + cùng cassette => `ocp replay` reproduces:
...
- Không set env `OCP_QUARANTINE=1` => mọi capability nondet trả `DEFERRED(RC-QUARANTINE-REQUIRED)` + hint.
...
- Mỗi template có:
  - `ocp run --record` tạo cassette
  - `ocp replay` chạy offline và signature ổn định
...
- Bật bằng:
  - `ocp.toml: [project] lane="quarantine"`
...
Directory:
- `./.ocp_artifacts/<run_id>/cassette/`
  - `cassette.jsonl`
  - `cassette_index.json`
  - `cassette_meta.toml`

Precedence (khóa cứng):
- Canonical cassette cho replay luôn nằm trong `./.ocp_artifacts/<run_id>/cassette/`.
...
### 10.1 Run flags
- `ocp run --record` (forces quarantine.mode=record)
- `ocp run --replay` (forces quarantine.mode=replay)
- `ocp cassette inspect <dir>` (optional)
- `ocp cassette redact <dir> --rules ...` (optional)

Precedence matrix (LOCKED):
- Flags chỉ override `quarantine.mode`, không override `lane`.
- `lane` luôn đọc từ `ocp.toml`.
...
- Mọi run lane `quarantine` (record hoặc replay) đều bắt buộc `OCP_QUARANTINE=1`.

### 10.2 Template init
- `ocp init tool-http`
- `ocp init tool-proc`
- `ocp init tool-wallclock`

Each template includes:
- `ocp.toml` quarantine + permissions
- `main.ocp`
- `README.md`
...
- Scope:
  - Mở rộng `ocp-cli` để:
    ...
    - tạo cassette bundle chuẩn trong `.ocp_artifacts/<run_id>/cassette/`,
    - validate cassette bundle khi `ocp replay`,
...
- Expected tests:
  ...
  - `cargo fmt -- --check`
- Exit criteria:
  - Quarantine lane bắt buộc `OCP_QUARANTINE=1`.
...
- Gate/Step:
  - 8-A
- Implemented:
  - Nâng `projects/ocp/crates/ocp-cli/src/main.rs`:
...
- Implemented:
  ...
  - CLI/cassette:
    - bổ sung env bridge runtime (`OCP_PROJECT_LANE`, `OCP_QUARANTINE_MODE`, cassette paths) cho run/replay.
...
- Implemented:
  ...
  - Permission contract:
    ...
    - CLI bridge env runtime cho std.net.http:
      - `OCP_STD_NET_ALLOW_HOSTS`,
      - `OCP_STD_NET_ALLOW_METHODS`,
      - `OCP_STD_NET_TIMEOUT_MS`,
      - `OCP_STD_NET_MAX_BODY_BYTES`.
  - Tests:
...
- Scope:
  - Mở rộng `ocp init` cho 3 template:
    - `tool-http`
    - `tool-proc`
    - `tool-wallclock`
...
- Exit criteria:
  ...
  - Không vi phạm lane gate (`OCP_QUARANTINE=1` vẫn bắt buộc).
...
- Implemented:
  - CLI template surface:
    - thêm `tool-http`, `tool-proc`, `tool-wallclock` vào `ocp init`.
    - thêm generator cho 3 template với:
      - `Ocp.toml` lane `quarantine`,
      - permissions tối thiểu theo pack,
      - `src/main.ocp`,
      - `README.md`.
  - Runtime/CLI wiring để template chạy thật:
    - bổ sung bridge env `std_fs` từ `[permissions.std_fs]` trong manifest vào runtime env (`OCP_STD_FS_ROOT`, allowlists, max caps).
  ...
  - Tests:
    ...
    - cả 3 test đều verify:
      ...
      - run/replay fail khi thiếu `OCP_QUARANTINE=1`,
