...
This document explains the OCP-OCL trademark and branding policy in user-readable language for users, contributors, and parties who may want to distribute derivative builds.

It does not replace the root source file:
- `TRADEMARK.md`

This file mainly helps answer practical questions such as:
- When is a build considered an official OCP-OCL build?
- Can a fork say that it is based on OCP-OCL?
- Can someone use the OCP-OCL name or logo for a separate release?

## Short Summary

If you only need the quick version:

1. Not every build using OCP-OCL source is an official build.
2. Only a build inside the official release scope and the official trust chain should be called an `Official OCL Build`.
3. A fork is allowed to say clearly that it is based on OCP-OCL.
4. A fork must not present itself as if it were the official OCP-OCL build.
...
- the trademark policy answers “may I present this build as the official OCP-OCL build?”.

## 2) When is a build considered official?

A build should only be considered an `Official OCL Build` when:
...
- say clearly that it is based on OCP-OCL;
...
- call itself `Official OCL Build`;
- call itself `Official OCP-OCL`;
...
## 5) May the name “OCP-OCL” be used?

Yes, but it must be used correctly.

Allowed:
- referencing OCP-OCL in order to describe project origin;
- stating that your product is based on OCP-OCL;
...
- using `OCP-OCL` as the primary brand of a fork without making the fork or community-build status clear.

## 6) May the OCP-OCL logo or branding be used?

The current safe understanding is:
- do not use the OCP-OCL logo or branding in a way that makes a fork or separate build look official;
...
That means:
- an extension published by a different publisher should not be presented as the official OCP-OCL extension unless it truly belongs to the official release chain.

## 8) How does trademark policy relate to the trust chain?

In OCP-OCL, trademark does not stand apart from release integrity.
...
- you may fork and build from OCP-OCL;
- you may say clearly that your build is based on OCP-OCL;
- but only a build inside the official release and the official trust chain should be called an `Official OCL Build`.
