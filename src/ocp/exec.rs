...
use crate::ocp::diag::{DiagPhase, Diagnostic, ErrorCode, ReasonCode};
use crate::ocp::registry::{Cacheability, CapabilityRegistry, DeterminismClass};
use crate::ocp::result_kind::{Result4, ResultKind};
use crate::ocp::schema::{validate_schema_value, SchemaType};
...
impl Executor {
    ...
    fn eval_expr(&mut self, expr: &Expr) -> Result<Value, Diagnostic> {
        let cache_disabled = env::var("OCP_CACHE_DISABLE")
            .ok()
            .as_deref()
            .map(|v| v == "1")
            .unwrap_or(false);
...
fn runtime_lane_from_env_v08() -> String {
    env::var("OCP_PROJECT_LANE").unwrap_or_else(|_| "locked_v071".to_string())
}

fn quarantine_mode_from_env_v08() -> String {
    env::var("OCP_QUARANTINE_MODE").unwrap_or_else(|_| "record".to_string())
}

fn append_wallclock_record_v08(call_id: u64, unix_ms: i64, iso: &str) -> Result<(), ReasonCode> {
    let Some(path) = env::var("OCP_V08_WALLCLOCK_RECORD_PATH").ok() else {
        return Ok(());
    };
...
fn load_wallclock_from_cassette_v08(call_id: u64) -> Result<(i64, String), ReasonCode> {
    let index_path = env::var("OCP_V08_CASSETTE_INDEX_PATH").ok();
    let jsonl_path = env::var("OCP_V08_CASSETTE_JSONL_PATH").ok();
...
fn net_http_allow_hosts() -> Vec<String> {
    env::var("OCP_STD_NET_ALLOW_HOSTS")
        .unwrap_or_default()
        .split(',')
...
fn net_http_allow_methods() -> Vec<String> {
    env::var("OCP_STD_NET_ALLOW_METHODS")
        .unwrap_or_default()
        .split(',')
        .map(|v| v.trim().to_ascii_uppercase())
        .filter(|v| !v.is_empty())
...
fn net_http_timeout_ms() -> u64 {
    env::var("OCP_STD_NET_TIMEOUT_MS")
        .ok()
...
fn net_http_max_body_bytes() -> usize {
    env::var("OCP_STD_NET_MAX_BODY_BYTES")
        .ok()
...
fn mock_http_response_v08(
    ...
    headers.insert("x-ocp-mock".to_string(), "v08".to_string());
...
fn append_net_http_record_v08(row: NetHttpRecordAppendV08<'_>) -> Result<(), ReasonCode> {
    let Some(path) = env::var("OCP_V08_HTTP_RECORD_PATH").ok() else {
        return Ok(());
    };
    if path.trim().is_empty() {
        return Ok(());
...
fn proc_allow_bins() -> Vec<String> {
    env::var("OCP_STD_PROC_ALLOW_BINS")
        .unwrap_or_default()
        .split(',')
...
fn proc_timeout_ms() -> u64 {
    env::var("OCP_STD_PROC_TIMEOUT_MS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(5_000)
...
fn proc_max_stdout_bytes() -> usize {
    env::var("OCP_STD_PROC_MAX_STDOUT_BYTES")
        .ok()
        .and_then(|v| v.parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(1_048_576)
...
fn proc_max_stderr_bytes() -> usize {
    env::var("OCP_STD_PROC_MAX_STDERR_BYTES")
        .ok()
        .and_then(|v| v.parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(1_048_576)
...
fn append_proc_record_v08(
    ...
    let Some(path) = env::var("OCP_V08_PROC_RECORD_PATH").ok() else {
        return Ok(());
    };
    if path.trim().is_empty() {
        return Ok(());
...
fn kv_store_path() -> PathBuf {
    if let Ok(raw) = env::var("OCP_STD_KV_PATH") {
        let trimmed = raw.trim();
        if !trimmed.is_empty() {
            return PathBuf::from(trimmed);
        }
    }
    PathBuf::from(".ocp_state/kv.json")
}

fn kv_max_keys() -> usize {
    env::var("OCP_STD_KV_MAX_KEYS")
        .ok()
...
fn kv_max_value_bytes() -> usize {
    env::var("OCP_STD_KV_MAX_VALUE_BYTES")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(65_536)
...
fn fs_sandbox_root() -> PathBuf {
    if let Ok(raw) = env::var("OCP_STD_FS_ROOT") {
        let trimmed = raw.trim();
        if !trimmed.is_empty() {
            return PathBuf::from(trimmed);
        }
...
fn fs_max_read_bytes() -> usize {
    env::var("OCP_STD_FS_MAX_READ_BYTES")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(1_048_576)
...
fn fs_max_write_bytes() -> usize {
    env::var("OCP_STD_FS_MAX_WRITE_BYTES")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(1_048_576)
...
fn fs_max_list_entries() -> usize {
    env::var("OCP_STD_FS_MAX_LIST_ENTRIES")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(500)
...
fn ui_max_draw_cmds() -> usize {
    env::var("OCP_STD_UI_MAX_DRAW_CMDS")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(5_000)
...
fn ui_max_input_events() -> usize {
    env::var("OCP_STD_UI_MAX_INPUT_EVENTS")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(500)
...
fn game_enabled() -> bool {
    match env::var("OCP_STD_GAME_ENABLED") {
        Ok(raw) => !matches!(
...
fn game_fixed_dt_ms() -> i64 {
    env::var("OCP_STD_GAME_FIXED_DT_MS")
        .ok()
...
fn game_rng_max_count() -> usize {
    env::var("OCP_STD_GAME_RNG_MAX_COUNT")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(1024)
...
fn game_state_delta_max_bytes() -> usize {
    env::var("OCP_STD_GAME_STATE_DELTA_MAX_BYTES")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(65_536)
...
fn game_rng_streams() -> Vec<String> {
    let raw = env::var("OCP_STD_GAME_RNG_STREAMS").unwrap_or_else(|_| "main,loot".to_string());
    raw.split(',')
        .map(|v| v.trim())
        .filter(|v| !v.is_empty())
        .map(|v| v.to_string())
...
fn game_base_seed() -> u64 {
    env::var("OCP_STD_GAME_BASE_SEED")
        .ok()
...
fn shadow_enabled() -> bool {
    match env::var("OCP_STD_SHADOW_ENABLED") {
        Ok(raw) => !matches!(
            raw.trim().to_ascii_lowercase().as_str(),
            "0" | "false" | "no"
        ),
...
fn shadow_max_branches() -> usize {
    env::var("OCP_STD_SHADOW_MAX_BRANCHES")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(8)
...
fn shadow_branch_step_cap() -> usize {
    env::var("OCP_STD_SHADOW_BRANCH_STEP_CAP")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(5_000)
...
fn shadow_branch_budget_cap() -> usize {
    env::var("OCP_STD_SHADOW_BRANCH_BUDGET_CAP")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(200_000)
...
fn shadow_max_diff_keys() -> usize {
    env::var("OCP_STD_SHADOW_MAX_DIFF_KEYS")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(2_000)
...
fn shadow_max_report_bytes() -> usize {
    env::var("OCP_STD_SHADOW_MAX_REPORT_BYTES")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(262_144)
...
fn shadow_checkpoint_every() -> usize {
    env::var("OCP_STD_SHADOW_CHECKPOINT_EVERY")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(200)
...
fn shadow_checkpoint_cache_max_entries() -> usize {
    env::var("OCP_STD_SHADOW_CHECKPOINT_CACHE_MAX_ENTRIES")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(2_048)
...
fn shadow_checkpoint_reuse_enabled() -> bool {
    match env::var("OCP_STD_SHADOW_CHECKPOINT_REUSE") {
        Ok(raw) => !matches!(
            raw.trim().to_ascii_lowercase().as_str(),
            "0" | "false" | "no"
        ),
...
fn shadow_memoize_deterministic_observe_enabled() -> bool {
    match env::var("OCP_STD_SHADOW_MEMOIZE_DETERMINISTIC_OBSERVE") {
        Ok(raw) => !matches!(
            raw.trim().to_ascii_lowercase().as_str(),
            "0" | "false" | "no"
        ),
...
fn shadow_memo_observe_key() -> String {
    let raw = env::var("OCP_STD_SHADOW_MEMO_OBSERVE_KEY")
...
fn shadow_scheduler_policy_allow() -> Vec<String> {
    let raw = env::var("OCP_STD_SHADOW_SCHEDULER_POLICY_ALLOW")
...
fn shadow_max_rounds() -> usize {
    env::var("OCP_STD_SHADOW_MAX_ROUNDS")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(50)
...
fn shadow_global_step_cap_max() -> usize {
    env::var("OCP_STD_SHADOW_GLOBAL_STEP_CAP")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
...
fn shadow_global_budget_cap_max() -> usize {
    env::var("OCP_STD_SHADOW_GLOBAL_BUDGET_CAP")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
...
fn shadow_beam_width_max() -> usize {
    env::var("OCP_STD_SHADOW_BEAM_WIDTH_MAX")
        .ok()
        .and_then(|v| v.trim().parse::<usize>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(8)
...
fn shadow_memo_observe_cost_steps() -> i64 {
    env::var("OCP_STD_SHADOW_MEMO_OBSERVE_COST_STEPS")
        .ok()
        .and_then(|v| v.trim().parse::<i64>().ok())
        .filter(|v| *v > 0)
        .unwrap_or(64)
...
fn fs_allow_patterns_for_action(action: &str) -> Vec<String> {
    let var_name = match action {
        "read" => "OCP_STD_FS_ALLOW_READ",
        "list" => "OCP_STD_FS_ALLOW_LIST",
        "write" => "OCP_STD_FS_ALLOW_WRITE",
        "remove" => "OCP_STD_FS_ALLOW_REMOVE",
        "rename" => "OCP_STD_FS_ALLOW_RENAME",
        _ => return Vec::new(),
    };
...
fn tls_local_real_enabled() -> bool {
    flag_enabled("OCP_W7_TLS_LOCAL_REAL")
}

fn db_local_real_enabled() -> bool {
    flag_enabled("OCP_W7_DB_LOCAL_REAL")
}

fn tls_local_timeout() -> Duration {
    let millis = env::var("OCP_W7_TLS_TIMEOUT_MS")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(1_500);
...
fn load_plugin_lock_entries_from_env() -> Result<Vec<PluginLockEntryRuntime>, ReasonCode> {
    let lock_path = env::var("OCP_PLUGIN_LOCK_PATH").map_err(|_| ReasonCode::PluginUnavailable)?;
...
fn invoke_plugin_jsonl(
    ...
    if let Ok(root) = env::var("OCP_PLUGIN_ROOT") {
